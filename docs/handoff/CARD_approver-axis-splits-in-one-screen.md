...
### ✅ 축 미해결 sentinel 설계의 사후 실증 (2026-08-30 · 완결 사례)

> **미해결을 침묵이 아니라 단언으로 들었기 때문에 닫히는 순간 소리가 났다.**

(다) 시점에 "축은 미해결" 을 **긍정 단언으로** 발화시켜 둔 것이 설계였다. 근거는
"(다)가 문구를 내리면 그 갈림이 화면에서 안 보이게 된다 — 안 보이는 것과 닫힌 것이
갈리지 않으면 다음 세션이 (나)를 스킵한다" 였다.

```
설계    미해결 4곳을 긍정으로 잠근다 → 닫히면 RED → 그때 통일 단언으로 교체
실행    (나)-1a·1b·2 가 차례로 닫자 **4건 전부 예고대로 RED**
결과    교체 지점을 찾아 헤맬 필요가 없었다 — 게이트가 자리를 가리켰다
```

🔑 설계가 의도대로 작동한 완결 사례다. 기록으로 뒀으면 (나)가 스킵됐거나, 스킵되지
않았더라도 **어디를 고쳐야 하는지를 사람이 기억으로 찾아야 했다.**

⚠️ 반대 사례가 같은 슬라이스에 있다 — `quote-detail-canapprove` 는 예고된 RED 가
**안 났다.** 승계 근거 주석에 옛 토큰을 적었더니 긍정 단언이 주석에 매칭됐다.
→ 조항 일반화: **소스 grep 단언은 긍정·부정 불문 주석 제거본 위에서 본다.**
  주석은 코드가 아니므로 어느 방향 단언에도 매칭 대상이 아니다.
  🛑 예고된 자기소멸 단언이 조용하면 **그것 자체가 결함 신호**다.

### 큐 A 결박 근거 보강 (2026-08-30)

```
db `any` 가 가린 것이 또 나왔다 — quotes/[id] 의 select 부재.
  teamId 는 있고 organizationId 가 없어, 표면을 조직 축으로 옮기면
  latestPendingPr.organizationId 가 undefined → canApprove **영구 false**.
  🔑 오늘 내린 roleColumnHint 와 같은 부류(항상 거짓 분기)를 **만들기 전에** 잡았다.
  tsc 는 못 잡는다(db: any). sentinel 로 `organizationId: true` 를 핀해 막았다.
→ 큐 A(admin/orders tx 타이핑)의 결박 근거에 추가한다: `any` 가림막은 한 곳이 아니고
  그 라우트 안에도 같은 형태가 살아 있을 수 있다. 그 라우트를 건드리는 슬라이스는
  타이핑을 선행 조건으로 둔다.
```

---

## ①c 되살림 (2026-08-30) — (다) 근거 소멸 · 선별 1 · 조건 신설

### 되살림의 조건은 "게이트가 열렸다" 가 아니다

> **"지시를 따라가면 끝까지 도달한다"** (호영님)

`:601` 이 정확히 그 실패였다 — CTA 가 배선돼 있는데 끝에서 아무 일도 안 일어났다.
그래서 게이트 개방만으로 되살리지 않고 tvkl 3단 실측으로 도달을 확인했다.

```
① 역할 변경 경로   PATCH 가드 4종(요청자 OWNER/ADMIN · role 유효 · 조직 소속 ·
                  OWNER 보호) 통과 → 신규 멤버 VIEWER → APPROVER
② 승인 게이트      isOrgApprover(APPROVER)=true · §S2 한도 통과(null=무제한)
③ 예산 게이트      도달 · auditEvent{ eventType: budget_gate_decision }
복원              12테이블 count 일치 + **기존 멤버 4명 role 값 불변**
                  (역할을 만지는 프로브라 count 가 아니라 값을 스냅샷했다)
```

### 선별 — 다섯 중 하나. 기준은 "행동이 없는가"

```
✅ ①c 상세 처리 항목 CTA   다섯 중 유일하게 배선된 CTA. 끝이 비어 dead button 이었고
                          이제 끝이 실재한다. (나)-2 가 사실 표시를 채운 뒤
                          남은 결핍은 **그 자리에서 고치러 가는 경로** 하나뿐이었다.
🚫 ①d "지정 필요" 배지      KPI 수 + yellow 톤과 중복
🚫 ①a/①b 목록 문구         (나)-2 가 목록에 승인자 3축을 넣어 사실이 이미 보인다
🚫 ①e "지정해 주세요"       현재 사실 표기가 있고, 지정 수단은 ①c 가 든다
```

🔑 기준이 **"사실이 안 보이는가" 가 아니라 "행동이 없는가"** 다. 사실 표시는 (나)-2 가
채웠으므로 넷은 중복이 된다.

### 🛑 실측이 문구보다 큰 사실을 줬다 — 발화 조건 신설
...
**prod T1 이 정확히 전자다**(FREE · 승인권자 1). 거기에 "요청이 멈춥니다" 를 띄우면
그것도 틀린 경보다 — (다)가 내린 것과 같은 부류를 다시 만든다.

```
발화 조건   approvalPolicy !== "none" && approverCount === 0
문구        "승인할 사람이 없어 요청이 멈춥니다"
🛑 옛 문안  "구매 요청이 승인 단계 없이 통과됩니다" — **사실이 반대**다. 금지 유지.
CTA 끝      setActiveTab("members") — 거기서 역할을 APPROVER 로 바꾼다
            (roleColumnHint 잔해는 되살리지 않는다 — 그 금지는 그대로)
```

🔑 **조건 분기 자체를 sentinel 로 잠갔다.** (다)를 만든 원인이 "조건 없이 뜨는 지시" 이므로,
되살린 지시가 조건을 잃으면 RED 가 난다.

### 결정 교체 3건 (명시 승인) — 헤더에 교체 사유 명기

```
alarm-retired   지시형 부재 4 → 기각 4 부재 + ①c 조건부 존재 + 발화 조건 + 옛 문구 금지
p4a             actionLabel 금지 해제 · 옛 문구 금지 유지 (틀린 문안이므로 새 문구와 별개)
                🔑 발화 조건은 여기서 다시 핀하지 않는다 — 같은 사실을 두 곳이 말하면
                  다음 갈라짐의 씨앗이다. alarm-retired 가 소유한다.
303hotfix-d     승계 한 줄만 되돌림. 원 결정(SWC nested-generic 회피 · push 사용처 보존)
                무손상 — 항목 개수는 원래 잠금 대상이 아니었다.
```

사유 문안(3곳 공통): `(다) 근거 소멸 — (나)-1b 가 승인 경로를 열었고 3단 실측(3569ede8)으로 도달 확인`

### 🛑 대조군이 또 물렸다 — 4회차

```
1차 대조군   플랜 라벨 "Pro" 변경 → plan-tier-naming-304 가 핀하고 있어 RED
정정         src/__tests__ 전역 grep 0 을 확인한 뒤 "이름 없음" 으로 교체 → GREEN
```

§4-a-2 3-c(`대조군은 "안 닿는다" 를 grep 0 으로 확인한 뒤 고른다`)를 **또 안 지켰다.**
조항이 있는데 실행에서 빠진 형태이고, §4c 와 같은 부류다 — 다음 프로브부터 대조군 선정
직전에 grep 을 **명령으로** 박는다(짐작 금지).

### 게이트

```
주입 프로브   8/8 검출 (조건 통째 제거 · 정책 조건만 제거 · 계수 조건만 제거 · 파생
             하드코딩 · 옛 문구 복원 · label 변경 · CTA 끝 변경 · import 제거)
             + 대조군 GREEN(재선정 후) · 바이트 무손상 · baseline 0 확인
             T = 상세 페이지를 읽는 sentinel **16파일 전수**(§4-a-2 3-b)
