...
## 착수 순서 — isSelectable 이 먼저다 (로컬 세션 판정 2026-08-24)

```
먼저   isSelectable 차단 (:3546 · :3558 · :3573)
       표시가 틀린 건 오정보지만 일괄 선택은 **실행**이다.
       두 축이 같은 함수를 쓴다는 것이 결함의 원인이므로 분리 자체가 수정의 일부다.
       아래 문법 판정을 기다리는 동안에도 선택 축은 막을 수 있다.
나중   RailState 문법 판정 (아래 측정 항목)
```
