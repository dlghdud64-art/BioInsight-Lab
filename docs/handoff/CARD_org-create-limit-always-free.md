...
**사례 1 — 축: 계수 대상 (2026-08-29 · 클로드).**
배선 확인이 답을 냈고("구조 맞음"), 그 답을 근거로 슬라이스를 잘랐는데,
계수 축이 하나(유래 여부)뿐이라 두 번째 축(값이 어느 rung 에 착지하는가)이 안 보였다.
→ B1a 가 값 중립인 줄 알았으나 실은 TEAM 에 ∞ 를 주는 선택이었다.

**사례 2 — 축: 단위 (2026-08-29 · 호영님).**
세션 인계 블록의 `tsc 27` 한 줄을 보고 tsc baseline 을 27 로 잡았으나,
그 27 은 **vitest 통과 테스트 수**였다(`3 files · 27 tests`). 숫자만 있고 **단위가 없는 값**을
근거로 썼다. 실제 tsc baseline 은 그 시점에 측정된 적이 없었다.
→ 대조 지시는 옳았고 결론도 우연히 같았으나(실측 tsc 도 27), **근거가 다른 축이었다.**

**사례 3 — 축: 열림 여부 (2026-08-29 · 클로드).**
묶음 오독을 정정하면서 `C1(plan 응답 실기)` 를 열린 항목처럼 두 문서에 박았다.
**같은 문서 안에 §C1 자기 정정("응답 확장이 필요 없었다 · API 변경 0")이 이미 있었다.**
map 축은 쟀는데 **그 항목이 아직 열려 있는지는 안 셌고**, 앞 묶음 문구의 표현을 물려썼다.
→ 닫힌 항목이 열린 것으로 판정 대기 내내 살아 있었다.

🔑 세 사례의 공통 형태: **값은 맞는데 그 값이 무엇의 값인지가 안 세어졌다.**
사례 1 은 세는 대상이, 사례 2 는 단위가, 사례 3 은 시점(열림 여부)이 빠졌다.
축 하나로 센 확인은 확인이 아니다.

⚠️ §2b 는 이 저장소에 **정의가 없다.** 이 절과 카드 상단이 유일한 참조다.
   조문 체계(HANDOFF_2026-08-26 의 §1~§7 · 4-b · 4-c 선례)로 보면 §2 §verification-loss
   아래 **2-b** 자리인데, 조문 신설은 판정 사안이라 미착수 — 위치 지시 대기.
