...
🛑 **받은 어림수도 재사용하지 않는다.** 사무국이 이 어림수를 그대로 받아 여러 턴 인용했다.
   내가 만든 어림수가 남의 근거가 되어 돌아왔다 — §7 형태 하나 더다.

```
어림수를 만든 쪽   검증을 미룬다 ("나중에 세지")
어림수를 받은 쪽   센 값으로 읽는다 (물결표는 인용에서 떨어진다)
→ 아무도 안 센 수가 **양쪽에서 근거로 쓰인다**
```

받았을 때의 규칙: 인용하기 전에 **세거나, 어림수임을 같이 인용한다.**
