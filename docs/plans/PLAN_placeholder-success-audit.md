...
## 세 번째 사례 — `$transaction` 안의 전역 `db` (2026-08-15)

감사 호출 15건이 `$transaction` 콜백 **안**에 있는데, 그중 **10건이 전역 `db`** 를 쓴다.
Prisma 에서 이건 **별도 커넥션 · 트랜잭션 밖** 실행이다.

```
await db.$transaction(async (tx) => {
  await tx.inventory.update(...)      // 트랜잭션 안
  await createAuditLog({ ... })       // ← 전역 db. 트랜잭션 밖. 롤백돼도 남는다
})
```

**읽는 사람은 편입으로 읽고, 실제로는 아니다.** 미편입보다 나쁘다 —
미편입은 최소한 미편입으로 보인다.

| 사례 | 형태 |
|---|---|
| 1 | `getOrganizationById` 팬텀 파라미터 — 받지만 쓰지 않는다 |
| 2 | `enforceAction` 조직 게이트 `(a)≡(b)` — 검사하지만 항등이다 |
| 3 | **`$transaction` 안의 전역 `db`** — 감싸여 보이지만 밖이다 |

> 셋 다 **충족된 것처럼 읽히는** 형태다. 코드 독해로는 통과하고 실측에서만 갈린다.

### 처리 — (가)/(나) 무관 전건

범위 논의 대상이 아니다. 커밋 1b 에서 10건 전건 교정한다.
**편입 불가한 건이 있으면 편입하지 말고 주석으로 미편입을 명시한다. 조용히 두지 않는다.**
