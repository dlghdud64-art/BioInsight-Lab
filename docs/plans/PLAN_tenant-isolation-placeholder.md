...
- **Status:** 🔒 **A트랙 종료** (2026-08-15, 호영님 승인) · fail-open 확정 → 유출 2건 수정 완료
  - ⚠️ **종료 문구는 §9.23 과 함께 읽는다** — A트랙 GREEN 은 "org 축 회귀 차단 + org 축 실측분
    유출 0" 이지 **테넌트 격리 완료가 아니다**. 닫히지 않은 축: scopeKey · 전역 카탈로그 · 귀속
...
### 판정 성립의 **두 형태** (2026-08-15 보강)

위 문장은 **차단형**만 상정했다. 그 기준을 무시형에 그대로 적용하면
**성립하는 판정을 판정 불가로 버린다.**

| 형태 | 교차 결과 | 판정 성립 조건 | 실례 |
|---|---|---|---|
| **차단형** | 교차가 **차단**됨(403/404) | **대조군 통과 필요** — 안 그러면 다른 이유로 막힌 것과 구분 불가 | `dispatch-batch`(교차 422 "권한 없음" / 대조 200+쓰기) |
| **무시형** | 교차가 **통과**하되 주입이 **무시**됨 | **대조군 불요** — 한 프로브가 "경로 착지"와 "주입 무시"를 동시에 증명 | `ingestion`(교차 201 · 생성 행 org = 세션 org), `protocol/bom` 수정 후 |

무시형의 판정 근거는 **응답이 아니라 생성된 row 의 귀속**이다.
교차 입력(orgB)이 결과에 반영되지 않았다는 것이 곧 org 판별의 증거다.

### 🛑 프로브 **자동 판정은 4분류를 강제**한다 (2026-08-15 추가)

3차 마지막 배치에서 스크립트가 **바디 검증 400** 을 `✅ 교차 차단` 으로 자동 판정했다.
바디 검증 층 정지는 **org 축 미도달**이므로 판정 불가인데, 판정 로직이 층을 구분하지 않아
**미도달을 차단으로 읽었다.**

이것은 세션 **첫 턴 §0** 에서 경계한 것("500 을 403 으로 볼 뻔했다")과 **정확히 같은 형태**이며,
**마지막 배치에서 다시 나왔다.**

> **조항: 프로브 스크립트의 자동 판정은 4분류(미들웨어 / 역할게이트 / 라우트 검사 /
> 바디·선결·드리프트) 를 먼저 확정한 뒤에만 ✅/🔴 를 붙인다.**
> 층이 `라우트 검사` 가 아니면 org 판정을 **자동으로 내리지 않는다** — 판정 불가로 둔다.
> "차단됐다"는 결과값 하나로 ✅ 를 찍는 판정기는 이 세션에서 두 번 틀렸다.
...
## 9.22 3차 잔여 3건 재개 — 완료 (2026-08-15)

새 선언 파일(`DECLARATION_wave3b.json`) 선작성. **감사·로그 파생 11테이블을 기본 항목으로**
선언에 넣었다(6차 동형 교정). 정지 없음.

| 프로브 | 결과 | 판정 |
|---|---|---|
| `ops-sync` | 200 · `AiActionItem` 생성(org=null) · 바디 org 주입 무시 · orgB 증감 0 | **✅ org 축 대상 아님** (클라 org 지정 수단 0 · 조회 전량 userId 축) |
| `cadence-governance` POST | 200 `{"success":true}` · **전역 변경 0** | ⚠️ **org 축 판정 불가** — 귀속 대상 행 자체가 안 생긴다 + 🔴 **신규 발견**(아래) |
| `bottleneck-remediation` POST | 400 `remediation 필수` | ⚠️ **판정 불가** — 바디 검증 층에서 정지, org 축 미도달 |

### 🔴 신규 발견 — `cadence-governance` 성공 위장 (테넌트 축 아님)

200 `success:true` 인데 **아무것도 저장되지 않는다.** 원인 두 겹:
드리프트(`entityType` 필수 누락 + `ActivityType` enum 에 `CADENCE_*` 부재) +
`createActivityLog` 의 `catch` 가 예외를 삼킨다. 그런데 라우트는
`enforcement.complete({ status:'logged' })` 를 부르고 성공을 반환한다.
**감사 기록 0인데 감사 봉투는 'logged'.** → §drift §1.9 · §placeholder-success 교차.
**수정 금지 — 목록만.**

### 첫 시도 정정 (자기 교정)

1차 시도에서 두 라우트가 **바디 검증(400)** 에 막혔는데 스크립트가 이를
"✅ 교차 차단(orgB 증감 0)"으로 자동 판정했다. **틀렸다** — 바디 검증 층에서 멈춘 것은
org 축 미도달이므로 판정 불가다. 유효 바디를 찾아 재측정했고, 그 결과가 위 표다.
(§measurement-layer-blindness §1 — "차단됐다"만으로 층을 가르지 않는다)

### 이관 요건 문장 (다음 사람이 바로 착수하도록)

- `bottleneck-remediation` POST: **`action:'create'` + `remediation` 객체의 정확한 스키마**가 필요하다.
  `{title, description}` 형태는 거부됨 — 서비스 계층 타입 확인 후 재시도
- `cadence-governance` POST: **드리프트 해소(enum `CADENCE_*` 추가 + `entityType` 전달) 후**
  재측정해야 org 축 판정이 가능하다

## 9.23 A트랙 종료 판정 — **닫을 수 있다** (기준안 적용 결과)

### 종료 조건 대조

| 조건 | 상태 |
|---|---|
| org 축 유출 **0 실증** | ✅ 발견 2건(읽기 `organizations/[id]/members` · 쓰기 `protocol/bom`) **전건 수정 완료 + 재측정 확인** |
| 잔여 전건 **명시적 이관** | ✅ 아래 표 |
...
| 잔여 종류 | 건 | 이관처 | 조건 |
|---|---|---|---|
| **드리프트 잠김** | 읽기 10경로 + 쓰기 `cadence`·`quotes/[id]/status` PATCH 등 | §drift-masks-isolation | 각 경로 해소 조건에 **"대조군 착지 확인"** 포함. **이관은 위임이지 면제가 아니다** |
| **대조군 미통과** | `bulk-po`(선택된 회신 보유 견적) · `organizations`(요금제 한도) · `user-budgets/[id]/check` · `bottleneck`(remediation 스키마) | 후속 백로그 | **요건 문장 동봉 완료** |
| **미측정 축** | scopeKey · 전역 카탈로그 · 귀속 | 카드 3종 분리 완료 | 종료 문구에 "닫히지 않은 축" 명시 |
| **축 다름 제외** | 읽기 9 · 쓰기 7 | 분모 유지 | 커버리지에서 빼지 않는다 |

### 최종 분수 (분모 정의 포함)

```
읽기        99/107   분모 = 테넌트 접촉 · 미들웨어 미커버 GET 라우트 전체
                     유출 1건 발견 → 수정 완료 · 판정 불가 11(드리프트 10 · 대조군 1)
쓰기 org 축  5/11    분모 = org 축 대상 쓰기 핸들러
                     판정: dispatch-batch · protocol/bom · ingestion · ops-sync(대상 아님) · quote-lists/items(guestKey 축)
                     유출 1건 발견 → 수정 완료 · 판정 불가 2(cadence · bottleneck)
scopeKey 축  0       §scopekey-axis-unmeasured — **A트랙이 닫지 않는다**
```

### 종료 문구 (닫을 때 반드시 함께 간다)

> **A트랙 GREEN 은 "org 축 격리 회귀 차단 + org 축 실측분 유출 0" 이다.
> 테넌트 격리 완료가 아니다. 닫히지 않은 축: scopeKey · 전역 카탈로그 · 귀속.
> 판정 불가 건은 드리프트 트랙에 조건부 이관됐으며, 그 트랙이 닫히기 전까지 org 축도 완결이 아니다.
> 드리프트 수정 시 대조군 착지 확인이 누락되면 격리가 미완인 채로 GREEN 이 된다 —
> 이관은 위임이지 면제가 아니다.**
