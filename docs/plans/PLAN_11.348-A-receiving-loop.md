...
---
## 부록 — A-2 구현 완료 (Claude, 2026-06-03, push 대기)
- ✅ **회신 수신 완성.** 신규 3파일: `api/receiving/[token]`(GET, snapshot freeze + 만료/종결 가드), `api/receiving/[token]/response`(POST, zod + snapshot orderItemId 검증 + status AWAITING_REPLY→PENDING_REVIEW), `receiving/[token]/page.tsx`(폼 — 실수량/LOT/유효기간/메모).
- ✅ **재사용**: 견적 `/vendor/[token]` + `/api/vendor-requests/[token]` GET/POST 패턴 미러. token 검증·rate-limit 동일.
- ✅ **불변(§11.336)**: 회신 제출 = ReceivingDraft(Item)만. ProductInventory/InventoryRestock 미변경(sentinel 강제). 입고 확정은 A-4.
- ✅ **migration 0**: 항목 교체를 upsert 대신 deleteMany+create(트랜잭션) → A-3 모델에 @@unique 불필요, 신규 migration 회피.
- ✅ sentinel `receiving-reply-form-348a2` 5/5 green.
- **다음:** A-4(연구소 승인 → InventoryRestock(delivery-sync) + PO 매칭 + status APPROVED + 이중입고 가드 restockSyncedAt). 회신 도착 알림 동반.

---
## 부록 — A-4 구현 완료 (Claude, 2026-06-03, push 대기)
- ✅ **승인/반려 backend.** `api/receiving-drafts/[id]/approve`(POST) + `.../reject`(POST). 승인 시 공급사 회신(LOT·실수량) 기준 ProductInventory 증분 + 품목별 InventoryRestock + Order DELIVERED + 입고안 APPROVED/restockSyncedAt. 반려는 재고 무변경.
- ✅ **다중 가드(canonical 보호)**: auth+권한 / status PENDING_REVIEW / 이중입고 ①restockSyncedAt ②order.status DELIVERED / productId+실수량 없으면 422 / 전체 $transaction.
- ✅ **delivery-sync 미재사용 결정**: 그건 order-item+단일 LOT 기반 → 공급사 회신 품목별 실수량·LOT 반영 못 함. draft item 기반 직접 restock(LOT 보존).
- ✅ **migration 0**: enum도 기존 INGESTION_RECEIVED 재사용(전용 audit enum 회피).
- ✅ sentinel `receiving-approve-348a4` 5/5 green.
- ⚠️ **A-4b 미구현(후속)**: 연구소 리뷰 UI(PENDING_REVIEW 목록·상세→승인/반려 surface) + 회신 도착 알림. 현재 backend만 — 승인 진입점 UI 없음.
- **다음 후보:** A-4b(리뷰 UI — receiving 워크벤치) 또는 A-5(현장 QR/스캔 접합).

---
## 부록 — A-4b 구현 완료 (Claude, 2026-06-03, push 대기)
- ✅ **폐루프 실사용 완성.** `GET /api/receiving-drafts`(스코프 목록) + `<ReceivingReviewPanel>`(승인·입고/반려, csrfFetch→A-4) + `dashboard/receiving` 랜딩 **same-canvas 마운트**(신규 페이지 X, ops-store 무수정). 0건 시 패널 숨김.
- ✅ **CSRF 갭 수정(A-2 후속)**: `/api/receiving/[token]/response`(공급사 공개 POST)가 registry 미등록 → CSRF로 차단될 잠재 결함. exempt(public_token_auth) 등록 + csrf-batch10.test 정합(8→9). 58/58 green.
- ✅ sentinel `receiving-review-panel-348a4b` 5/5 green.
- ⚠️ `csrf-batch10-node.mjs`(수동 harness)는 미포함 — HEAD가 CRLF + highRisk 단언 2건 pre-existing drift(A-4b 무관). 별도 위생 트랙.
- ⚠️ 회신 도착 알림(연구소) 미구현 — 후속.
- **A-1~A-4b 완료 = 발주→회신링크→공급사 폼→검증대기→승인→재고확정 폐루프 동작.** 남은 A-5(현장 QR/스캔 접합)만.

---
## 부록 — A-5 구현 완료 + §11.348-A 폐루프 종결 (Claude, 2026-06-03, push 대기)
- ✅ **현장 접합.** 승인(A-4) 시 생성된 InventoryRestock 의 inventoryId 를 응답 `restockedItems` 로 반환 → 패널이 LabelPrintModal(§11.355-B) 자동 오픈(QR=inventoryId). 승인 분기 한정(반려는 라벨 없음).
- ✅ **재사용**: LabelPrintModal + 스캔(§11.349) + 차감(§11.355-D) 그대로. id=inventoryId 로 스캔·차감 정합.
- ✅ sentinel `receiving-field-label-348a5` 3/3 + 회귀(A-4 5/5, A-4b 5/5) green. migration 0.
- ✅ **폐루프 완결**: 발주 발송(A-1) → 회신링크 → 공급사 폼(A-2) → 검증대기 → 승인(A-4) → 검토 UI(A-4b) → 현장 QR(A-5) → 스캔→차감(§11.355).

## §11.348-A 전체 상태 (2026-06-03)
| Phase | 내용 | 상태 |
| :-- | :-- | :-- |
| A-3 | ReceivingDraft 모델 | ✅ DB 적용 완료 |
| A-1 | 발주 회신 링크 + reply-to | ✅ push 대기(코드) |
| A-2 | 공급사 회신 폼/라우트 | ✅ push 대기(코드) |
| A-4 | 승인/반려 → 재고확정 | ✅ push 대기(코드) |
| A-4b | 리뷰 UI + CSRF exempt | ✅ push 대기(코드) |
| A-5 | 현장 QR 라벨 접합 | ✅ push 대기(코드) |

**잔여(후속, 선택):** 회신 도착 알림(연구소 push/메일), 라벨 재출력 진입점, §11.348-FALLBACK(OCR), §11.348-B(COA·B-1 파일 인프라 = PLAN_11.348-B1 별도).

---
## 부록 — 후속 소형 2종 완료 (Claude, 2026-06-03, push 대기)
- ✅ **회신 도착 알림(A-2-notify)**: 회신 제출(PENDING_REVIEW) 시 연구소(draft.userId + 조직 OWNER/ADMIN)에 `VENDOR_REPLIED` dispatch + push. 전체 graceful try/catch, 재고 mutation 0 유지. (신규 NotificationEventType 회피 — drift sentinel 보호.)
- ✅ **라벨 재출력(A-5b)**: A-4b 패널에서 승인 후 모달 닫아도 방금 확정분 재출력 버튼(in-session, 신규 API/뷰 0).
- ✅ sentinel `receiving-followups-348a-notify-reprint` 3/3 + 회귀 13/13. migration 0.
- **잔여(별도 세션, Large)**: §11.348-B-1 파일 인프라(`PLAN_11.348-B1`), §11.348-FALLBACK(OCR), APPROVED 영구 재출력 뷰.
