...
- Status: [x] **P4a Complete** · [x] **P4b Complete** (2026-08-24)
...
### 착수 전 실측이 범위를 줄였다 — 세 번째

```
핸드오프 §4 "역할 = 인라인 드롭다운(역할 색 점 + 셰브런)"
실측       :1058 shadcn <Select> + onValueChange → updateRoleMutation 이 **이미 있었다**
           본인 관리자 행도 canEditRole=false 로 이미 편집 불가였다
```
→ 남은 것만 채웠다. §4 전체를 새로 만들 뻔했다.

```
✅ 이미 있던 것   인라인 Select · mutation 직결 · 본인 행 편집 불가
➕ 채운 것        역할 색 점(ROLE_DOT · 트리거와 옵션이 같은 map)
                  "✓ 저장됨" 1.5초 — toast 만으로는 **어느 행이** 저장됐는지 안 보인다
                  "본인 역할 변경 불가" 캡션
                  초대 대기 행 재발송·취소를 ⋮ 밖 인라인 버튼으로
                  승인자 지정 딥링크 → 멤버 탭 + 역할 열 강조
```

### 실측이 문서를 정정 — 3건째

```
핸드오프 §5  "승인자 지정 버튼 → 멤버 탭 + **역할 드롭다운 오픈 상태**로 딥링크"
🛑 열 수 없다. 승인자 0명 상황에서 **누구를 승인자로 만들지는 사용자가 정한다** —
   열어야 할 드롭다운이 하나로 정해지지 않는다. 아무 행이나 열면 그 사람을 지목하는 셈이다.
→ 역할 열 전체를 강조해 **어디를 볼지만** 말한다. 선택은 사용자에게 남긴다.
```

sentinel: `src/__tests__/dashboard/organizations-member-role-inline-p4b.test.ts` (9건)

## §sweep-widen-then-filter 보강 (로컬 세션 지적 2026-08-24)

```
대상은 "교체" 만이 아니다 — **필드 추가 · 포맷 변경**도 sweep 대상이다.
