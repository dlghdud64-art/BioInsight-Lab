...
- **Status:** 🔄 In Progress (Phase 0 실측 완료 · Phase 1 대기)
- **Started:** 2026-09-14
...
- [ ] §sentinel-inversion: `quote-followup-send-wiring-363.test.ts` 명제 판정 → Phase 1 착수 시 기록
  - 명제 "compare_not_ready 의 다음 조치 = 추가 회신 확보(발송 단일점)" 는 **비교 축에서는 여전히 참** · "발주 불가" 함의만 빠진다 → 승계 예상

**✋ Quality Gate:** 모순 0 · 우선순위 기록 · **Rollback:** 계획만

### Phase 1: Contract & Failing Tests
- Status: [ ] Pending
- [ ] 🔴 `__tests__/quotes/quote-readiness-single-source.test.ts` · 필수 핀 「회신 1건 → 발주 가능 · 비교 불가」 · 행렬 · 복사본 0 · BLOCKED_STATES
- [ ] 🔴 RED 확인(함수 부재) · 363 핀 승계 판정 기록
- [ ] RED 는 로컬에서 확인만 한다 · RED 테스트를 단독 커밋하지 않는다(원장 증가 = exit 1) · Phase 2 와 한 커밋

**✋ Quality Gate:** RED 가 실제 · 기존 GREEN 불변 · tsc 불변 · **Rollback:** 테스트 파일 revert

### Phase 2: Core Logic
- Status: [ ] Pending
- [ ] 🟢 `resolveQuoteReadiness({ status, vendorRequests, portalResponses })` → `{ respondedCount, convertibleCount, compare: {ready, label}, po: {ready, label}, uiState }`
- [ ] 🔵 `deriveUiState`(contract) 위임 · 문턱 상수 `COMPARE_MIN_RESPONSES = 2` · `PO_MIN_RESPONSES = 1` 한 곳
- [ ] 단위 GREEN · 주입 프로브
...
### Phase 3: UI Wiring
- Status: [ ] Pending
- [ ] 대시보드 `deriveRailState`·`:154/:171/:406/:636` 카운트 → 함수 사용 · `BLOCKED_STATES` 에서 compare_not_ready 제외 · compare_not_ready 브리핑 문구: 비교 「불가 · 회신 1건」 · 발주 「가능 · 단일 견적」 · blocker 「차단 없음(비교만 불가)」 계열
- [ ] 상세 `canConvert` → 함수 `po.ready` · KPI 「구매 전환」 라벨 동일 출처
- [ ] 상세 received 탭: 수신 회신 섹션을 입력 폼 **위로** · 폼 제목 「새 회신 직접 입력」 · 기본값 「대기」 는 폼 안 설명으로만
- [ ] 문구는 em dash 금지(·) · 기존 핀 옛 값 sweep
