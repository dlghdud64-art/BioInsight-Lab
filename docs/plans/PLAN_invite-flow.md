...
⏳ **후속 트랙 — 알림 화면 가짜 데이터 (③ · ①②보다 무겁다)**
`dashboard/notifications/page.tsx` 의 `INITIAL_NOTIFICATIONS` **하드코딩 20건**
(`useState(INITIAL_NOTIFICATIONS)` · 쿼리 0). 그중 `:189` —
`구독 갱신 완료 — Business 플랜` · `다음 결제일: 2026-04-18. 정상 갱신되었습니다`.
🛑 **우선순위 근거(Cowork QA):** ①②는 *"일어날 일"* 을 잘못 말한다(미래형).
③은 *"일어난 일"* 을 **단언**한다(과거형) — 결제도 갱신도 없었고, 사용자가 쓰지도 않는
Business 플랜이다. **과거형 거짓이 더 무겁다**: 사용자가 검증할 방법이 없고,
결제 문의가 들어오면 **이 화면이 근거로 인용된다.**
한 건만 고치면 나머지 19건 가짜 데이터가 남으므로 알림 화면 정직화 트랙으로 함께 잡는다.
...
- `maxOrganizations: 1` 이라 두 번째 조직도 만들 수 없다.
→ **Free 계정으로는 초대 생성 성공에 구조적으로 도달 불가**다. 좌석 게이트 발화·차단은
  검증됐지만(초대 목록 0 유지 = 게이트가 create 앞), 성공 경로 —
...
🛑 **부정 단언만 두면 침묵으로 통과한다 — "사실을 말하는지" 도 함께 본다.**
   위 규칙(호출 형태를 세지 말 것)과 **짝으로 읽는다.** 거짓 문구를 `not.toMatch` 로 막으면
   구현자가 그 문구를 **지우기만 해도** GREEN 이다. 침묵은 정직이 아니다 —
   결제 약속을 지우고 아무 말도 안 하면 사용자는 여전히 무슨 일이 일어나는지 모른다.
   → 부정 단언 옆에 **양성 단언**(대체 문구가 실재하는지)을 반드시 둔다.
   실측 2026-09-05(§checkout-two-paths): 프로브 T2(사실 문구 삭제)가 그 구멍을 드러냈다.

🛑 **범위를 밝히지 않은 "0" 은 틀린 0이다.**
   실측 2026-09-05: "렌더 잔재 0" 보고가 `checkout-utils`·`CheckoutDialog` **범위에서만** 참이었고,
   같은 거짓 문구가 `billing/page.tsx`·`settings/plans/page.tsx` 에 **3곳 더** 있었다.
   게다가 그 셋은 `Subscription.currentPeriodEnd`(플랜 변경이 `now+30일` 로 만들어 넣는 값)를
   공유해, **업그레이드하는 순간 동시에 발화**하는 구조였다.
   → 계수·부재 보고에는 **대상 집합을 함께 적는다.** 센티널도 파일을 열거해 고정한다.

🛑 **상수를 참조하는 단언은 그 상수가 사라져도 통과한다** (2026-09-05 · 저장소 전역).
   `toBe(E.X)` 는 멤버 삭제 시 `undefined === undefined` 다. 개수 단언은 **삭제**만 잡고
   **값 변경**(`"MODEL"` → `"model"`)은 못 잡는다 — 그때 DB 에는 다른 문자열이 저장된다.
   → 값 집합을 **리터럴로** 고정하고, 개별 비교도 리터럴로 병기한다.
   정식 조항은 **CLAUDE.md `### 상수를 참조하는 단언`** 에 있다(전역 규칙이라 그쪽이 정본 —
   계획서에만 두면 다른 트랙이 안 읽는다).

🔑 **두 세션이 서로 다른 지점에서 멈춘 것이 결과를 갈랐다** (구조 기록, Cowork QA 제안).
   §checkout-two-paths 에서 봉인 오판을 막은 것은 어느 한쪽의 판단이 아니다 —
   ① 구현 세션이 **봉인 직전에 멈춰** 착수 전 확인을 물었고,
   ② QA 세션이 그 물음에 **전제를 측정하라는 요건**을 붙였다.
   둘 중 하나만 있었으면 계측(`api/health` billing 축)이 만들어지지 않았고, 죽은 경로를
   유일한 대안으로 삼아 나머지까지 봉인했을 것이다. **판정보다 판정의 전제를 재는 단계**가
   결과를 갈랐고, 그 단계는 두 세션의 정지점이 어긋났기 때문에 생겼다.
