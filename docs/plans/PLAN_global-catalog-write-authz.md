...
> 상시 제약: **SDS·제품 정보는 공급사·관리자만 등록한다.**

`Product` 에 스코프 컬럼이 없다는 것은 이 제약을 **데이터 모델이 뒷받침하지 않는다**는 뜻이다.
한 조직 사용자가 고친 MSDS URL·보관조건·안전메모는 **다른 조직이 보는 화면에 그대로 나간다.**

- 테넌트 축이 아니므로 §tenant-isolation A트랙 종료로는 **닫히지 않는다**
- 그러나 결과는 더 나쁠 수 있다 — 유출은 남의 데이터를 **보는** 것이고,
  이쪽은 남이 보는 데이터를 **바꾸는** 것이다
- 안전정보라 틀리면 **취급·보관 판단이 달라진다**

즉 "공급사·관리자만 등록"이 지금 어디서 강제되는지가 이 카드의 핵심 질문이다.
역할게이트(`sensitive_data_import` 등)만으로 강제된다면, 그 역할은 **조직 축이 아니라
전역 축**(`User.role`)이므로 §authorization-dual-axis 와 같은 자리에서 만난다.
