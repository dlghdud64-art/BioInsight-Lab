...
- **Status:** ⏳ Pending
- **Started:** 2026-07-21
- **Last Updated:** 2026-07-21 (Phase 1–3 완료 — operator 일괄 게이트 대기)
...
### Phase 4: 5a 발송 검토 시트
**Goal:** 스텝퍼 정합 · 경고 통합 히어로 · 접기 · 푸터 위계 · 케이스 칩(RFQ+품목).
- Status: [ ] Pending

**🔴 RED:** 5a sentinel — 가짜 체크 금지(막힘=warning·이후=대기·체크=실완료)·경고 1개·접힘 기본·비활성 사유 인라인·"직접 전달용" 라벨. 기존 dispatch sentinel 회귀 가드 동반
**🟢 GREEN:** `vendor-dispatch-workbench.tsx` — P0 판정표대로 공유/분기 적용
**🔵 REFACTOR:** 중복 경고 제거·긴 스크롤 축소

**✋ Quality Gate:** **데스크탑 발송 플로우 무회귀**(264h5·279d 등 전건)·fake success 0·다운로드 폴백 = 공급사 0곳 상태 한정 노출
**Rollback:** 단일 파일 revert

### Phase 5: 스모크 · 롤아웃
**Goal:** QA 체크리스트 전수 · 375px · 44px · 문서 종결.
- Status: [ ] Pending

**🔴 RED:** 스모크 경로 정의(s2 케이스 → 리마인더 발송 → 활동 로그 확인 / s1 → 발송 검토 → 전송)
**🟢 GREEN:** 하네스 전 파일 원문 실행 + operator 게이트(build·full vitest·커밋)
**🔵 REFACTOR:** notes·핸드오프 갱신

**✋ Quality Gate:** QA 13항 전수 통과·baseline-delta 0·롤백 문서화
**Rollback:** phase 별 문서화된 개별 revert
...
- Overall completion: 67% (Phase 0–3/6)
- Current phase: **Phase 3 ✅ → operator 일괄 게이트(P1–P3) 대기 → 이후 Phase 4(5a)**
- Current blocker: 없음
- Next validation step: operator — `npm run build` → full vitest baseline-delta 판정 → 커밋
  (add 대상 10파일: 소스 6 = mobile-quotes-view · priority-recommendation-card · supplier-avatars ·
  reminder-targets(신규) · mobile-reminder-sheet(신규) · quotes/page + 테스트 3 = p1 · p3 ·
  reminder-targets unit + PLAN 1)
  ⚠️ baseline 참고: `quotes-mobile-density-p3` "sticky" 어서션은 **기존 drift**(P1 게이트 노트 참조)
