...
### 1a 접힘 행 — 2회 반복 (공용 컴포넌트 확정 · 2026-08-16 정정)
...
| ~~3~~ | ~~**국내 규제기관 포털**~~ | ~~`6개 링크`~~ | 🛑 **은퇴 — v2 §5 승계 (2026-08-09 승인)** |

🛑 **계획 정정의 재정정 (2026-08-16):** 위 "규제 포털 = 접힘 행 3번째 ·
`<CollapsedEmptyRow>` 추출은 확정 요구사항" 은 **폐기됐다.**
`docs/specs/소싱 제품 상세 핸드오프 v2.md` §5 가 규제 포털을 접힘 행에서 빼고
**주요 2기관 버튼 + `더보기 N개 기관 ›`** 으로 교체했다.

```
<CollapsedRow> 사용 = 3회 아님 · **2회** (상세 스펙 · SDS)
  상세 스펙   page.tsx:663-681   (2026-08-16 실측 · 이미 구현됨)
  SDS         page.tsx:849-873   (동일)
  규제 포털    page.tsx:961-1002  (2기관 버튼 + 더보기 — CollapsedRow 아님)
```

⚠️ 계약⑥ = `PAGE_CODE.not.toMatch(/CollapsedRow label="국내 규제기관 포털"/)` — **역방향 잠금**이다.
   정정 전 서술대로 3번째 접힘 행을 만들면 **게이트가 RED 를 낸다.** 이 잠금은 부활 차단 자산이므로 존치.
...
**🔴 RED:** 계약③⑤⑥ + 대체품 위치 계약
**🟢 GREEN:**
0. **`<CollapsedRow>` 공용 컴포넌트 선작성** — §0-B 접힘 행 토큰(`p 11/13`·`#e2e8f0`·`r10`·`#fafbfc`·`▸`·라벨 `#475569`·상태 `#94a3b8`·액션 `#2563eb`). **2회 사용 확정**
   (상세 스펙 · SDS. 규제 포털은 v21 §5 로 **은퇴** — 3번째를 만들면 **계약⑥ RED**)
...
> 🛑 **아래 진행률·현재 Phase 는 수기 집계다. 정본은 §11 Phase Checklist 다.**
>
> 손으로 쓰는 값이라 **반드시 낡는다.** 체크리스트는 sha·사유가 붙어 갈리면 드러나지만
> 집계값은 근거가 없어 조용히 갈린다. 실제로 커밋 `2122d903` body 는 "Progress 100%",
> 이 줄은 90% 로 갈려 있다.
>
> **이 수치를 착수 판단 근거로 쓰지 말 것** (2026-08-16 · 이 줄과 체크리스트로 2회 오판).

- Overall completion: **90%** (3b 완료 `ab0e4e2d` · **48/48 GREEN** · F10 EXIT 0)
  ⚠️ 수기 집계 · 커밋 `2122d903` 은 100% 라 적었다 — 상충. 정본은 체크리스트
- Current phase: **Phase 4 — 런타임 스모크 · 계약 위생 · 문서**
  ⚠️ 수기 집계 · Phase 4 상태 자체가 상충이다(체크리스트 참조)
- Current blocker: **없음**
...
- [x] Phase 0 complete — **sha 미상 (git 추적 실패 — 산출물이 `2122d903` 에 일괄 진입)**
  - 사유: sandbox → 호영님 환경 → push 워크플로. Phase 0~3 기간 산출물이 커밋되지 않았고
    Phase 4 종결 커밋(`2122d903`, 2026-07-26)에 PLAN·sentinel 이 함께 들어왔다.
    🛑 "추적 실패" 가 **확정 상태**다. 대기 항목이 아니다 — 다시 묻지 말 것.
- [x] Phase 1 complete (2026-07-25 · 26 RED / 회귀 0 6 PASS · false-GREEN 1건 봉합)
  - **sha 미상 (git 추적 실패 — `product-detail-refinement.test.ts` 최초 진입이 `2122d903`)**
    Phase 0 과 같은 사유. 확정 상태다.
...
- [ ] Phase 4 complete
  - ⚠️ **상태 상충 — 어느 쪽도 채택하지 않음.** 셋이 갈린다:
    ```
    커밋 2122d903 body   "PLAN: Progress 100% + Phase 4 스모크 판정표"
                         런타임 스모크 실행 기록: S1·S3·S4·S6 육안 PASS
                         S2·S5 는 admin·100% 데이터 부재로 **정적 승계**(증거 등급 구분)
    PLAN §11             Overall completion 90% · Current phase: Phase 4
    이 체크리스트          [ ] 미완
    ```
  - **Phase 4 항목 실측으로만 갈린다 (미실시).** 하나를 고르면 세 번째 상태를 창작하는 것이다.
  - 🛑 **이 줄을 근거로 착수/미착수를 판단하지 말 것.**
    2026-08-16 에 두 번 이 줄로 움직였다 — ① Phase 3 착수 오판, ② 이 줄에
    "프로덕션 로그인 차단" 이라는 **실측 근거 0 의 사유를 역산해 기입**(삭제됨).
    후자가 더 비싸다: 착수 오판은 실측하면 드러나지만, 거짓 사유는 **문서에 근거로 박혀**
    다음 세션이 실측 없이 인용한다 — 오판이 근거로 승격되는 형태다.
- [ ] Phase 5 complete — **미착수 사유: Phase 4 종료 선행.** QA 7항 런타임 확인 +
  회수 경로 확정이 스모크 결과에 의존한다. Phase 4 상태가 상충이므로 이 줄도 함께 유보.
...
- [2026-08-16] 🛑 **위 항목 폐기** — v21 §5(2026-08-09 승인)가 규제 포털을 접힘 행에서 제외.
  `<CollapsedRow>` 사용 = **2회**. 계약⑥ 역방향 잠금이 3번째 생성을 차단한다.
