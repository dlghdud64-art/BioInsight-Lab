...
## 7.7 🔴 시안 ↔ 핸드오프 불일치 5건 — **회신 대기 (Phase 0 blocker)**

| # | 항목 | 시안 | 핸드오프 | 무게 |
|---|---|---|---|---|
| 1 | **재발주 합류 배너** | `재고관리에서 재발주 검토 중인 품목입니다 · 권장 9개` `[재발주안에 합류]` | §1 **언급 없음** | 🔴 **무거움** |
| 2 | buyer 편집 링크 | 주석 ④ 에만 제거 명시 | — | 🟢 상시 제약과 일치 |
| 3 | amber `#b45309` | **4회 사용** | §5 토큰 목록 **부재** | 🟢 §0-B 표에 이미 확정 |
| 4 | radius | 실제 **7~18 + 99** | §5 "카드 14–16" 만 | 🟢 |
| 5 | 검색 결과 라벨 | `4건 · 품명 시작 일치` | **누락** | 🟢 |

### 🔴 1번이 Phase 2 범위를 바꾼다

단순 누락이 아니라 **제품 상세 → 재발주안 역방향 합류 경로**를 뜻한다.
§3 이 설계한 **재발주 → 소싱 단방향**과 다른 경로이고,
**양방향 동기화 범위가 그만큼 커진다.**

> **Phase 2 트랜잭션 설계가 이 경로까지 덮어야 한다.**
> 배너 건 회신 = Phase 2 범위 확정 조건.

⚠️ 2~5 는 회신 없이 진행 가능하나 **fixture 에는 시안 실측값으로 넣는다**
(핸드오프 누락은 문서 결손이지 시안 부정이 아니다).
