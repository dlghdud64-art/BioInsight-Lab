...
- [ ] **§0-B amber hex 8토큰 전수 일치** (`#fffbeb` `#fde68a` `#92400e` `#fef3c7` `#d97706` `#78350f` `#b45309` `#a16207`)
...
회신에 명시된 두 파일이 **이 세션 컨텍스트에 도착하지 않았다**(repo·scratchpad 부재).
경로를 받는 즉시 게이트를 실행 가능 상태로 만든다.

### 수령 시 확정할 사양 (회신 기재분)

```
레이블 총 112 — 1a 51 · 1b 24 · 1c 37
섹션별 colors / radius / font_sizes 동봉
annotation_excluded 11 — 시안 주석(수정:·동작:·흐름:)은 UI 아니므로 제외
```

**자기검증 앵커: `total_label_count = 112`.**
로딩 실패 시 이 값이 안 맞아 **불일치 0 으로 위장하지 못한다** —
§7.6 의 "출력 감소도 RED" 가 실제로 걸리는 지점이다.

🟡 **확인 후 확정할 항목 (실측 아님 · 분류 판단):**
각 섹션 `labels[0]` 이 시안 자체 라벨(`제품 상세 /products/mp-… (담김 상태)` 등)이라
UI 요소가 아니다 → `labels[1:]` 슬라이스 시 **대조 대상 111**.
게이트 fixture 로딩부에 **112(앵커) 와 111(대조 대상)을 분리 기록**한다 —
앵커를 111 로 낮추면 위장 방어가 무너진다.
