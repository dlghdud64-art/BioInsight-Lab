...
- **Status:** ⏳ Pending
- **Started:** 2026-06-14
- **Last Updated:** 2026-06-14 (P2 단일 surface 구현 완료)
...
### Phase 1: Contract & Sentinel (RED) — ✅ Complete (2026-06-14)
- Status: [x] Complete
- **🔴 RED:** 단일 로그 surface 계약 sentinel(활동/감사 모드 토글) + **회귀 0**(감사 admin-gate·Part11·PDF/CSV export 보존, 활동 ActivityLog·org멤버 보존) — 구현 전 RED.
- **산출물:** `src/__tests__/regression/log-consolidation-p1.test.ts`
  - RED 계약(5): `log-mode-toggle` / `log-mode-activity` / `log-mode-audit` + 활동모드 `/api/activity-logs` 흡수 + 감사모드 `canAccessAudit` 권한분기 — 전부 audit page 부재 → RED.
  - GUARD(18): admin-gate(canAccessAudit·ADMIN·redirect·거부메시지·/api/audit-logs) / Part11(Asia/Seoul·KST·append-only) / export(handleCompliancePdf·pdf-view·autoPrint·handleCsvExport·export-trigger·sheet-pdf·sheet-csv) / 활동(/api/activity-logs·org멤버 enabled·ACTIVITY_TYPE_LABELS).
- **격리 node 검증:** RED 5개 전부 absent(test FAIL = 정상), GUARD 18개 전부 present(회귀 0). 실 vitest·push = operator-shell.
- **✋ Gate:** RED 진짜 실패 ✅, 기존 sentinel 무회귀 ✅(GUARD 패턴은 보존만 매칭). Rollback: sentinel revert.

### Phase 2: 단일 surface 구현 — ✅ Complete (2026-06-14, 코드/검증 sandbox)
- Status: [x] Complete (operator 실 build+vitest 게이트 대기)
- **🟢 GREEN:** canonical 감사 surface(`dashboard/audit/page.tsx`)를 단일 로그 route로. 활동/감사 모드 토글(`log-mode-toggle`/`-activity`/`-audit`). 각 모드 자기 모델: 활동→`/api/activity-logs`(ActivityLog), 감사→`/api/audit-logs`(AuditLog) — **데이터 병합 없음**.
- **권한 분기:** 비admin = 활동 모드만(감사 탭 `canAccessAudit` 게이트로 비노출), admin = 둘 다. 페이지 단위 wholesale redirect 제거 → 감사 모드 진입 시도 시 거부 안내 + 활동 모드 강등(admin-gate 의미 보존).
- **산출물:**
  - `src/lib/activity/activity-labels.ts` (신규) — ACTIVITY_TYPE_LABELS/ENTITY_TYPE_LABELS/COLORS/isAiActivity/isAlertActivity 단일 소스(두 surface 라벨 drift 차단).
  - `dashboard/audit/page.tsx` — 모드 토글 + 활동 모드 fetch/리스트 + 권한 분기. 감사 모드 본문(Part11·PDF/CSV export·admin-gate) 무변경 보존.
  - `dashboard/activity-logs/page.tsx` — 인라인 라벨/헬퍼 → lib import(기능 무변경, P3 redirect 전까지 유지).
  - sentinel 갱신: RED 계약 5 → GREEN, GUARD redirect 기계론 진화(`router.replace("/dashboard")` → `setMode("activity")`, 의도 동일) + 감사탭 게이트 GUARD 추가.
- **검증:** activity-labels.ts·activity-logs·sentinel = TS 파서 통과(격리 node). audit/page.tsx 는 sandbox 마운트 동기 truncation 아티팩트로 격리 파서 검증 불가 → 구조는 직접 정독으로 균형 확인. **실 build+vitest = operator 필수.**
- **✋ Gate:** 계약 5 GREEN · GUARD 무회귀(감사 컴플라이언스 보존) · build. Rollback: 활동/감사 분리 surface 복귀.

### Phase 3: 메뉴 정리 + redirect — ✅ Complete (2026-06-14, 코드/검증 sandbox)
- Status: [x] Complete (operator 실 build+vitest 게이트 대기)
- **🟢 GREEN:** sidebar 활동로그+감사 2항목 → **통합 단일 진입**("활동 · 감사 로그" → `/dashboard/audit`, 활동/감사 모드 토글). 구 route `/dashboard/activity-logs` → **server `redirect("/dashboard/audit")`** (client 페이지 전체 제거, redirect-only). dead link 0.
- **산출물:**
  - `_components/dashboard-sidebar.tsx` — adminMenuItems 2→1, 미사용 `Activity` 아이콘 import 제거(ShieldCheck 유지).
  - `dashboard/activity-logs/page.tsx` — `"use client"`+useQuery 페이지 → `redirect("/dashboard/audit")` server 컴포넌트(redirect-only).
  - sentinel(`log-consolidation-p1.test.ts`) — 활동 보존 GUARD 3개(`/api/activity-logs`·`enabled status authenticated`·`ACTIVITY_TYPE_LABELS`)를 통합 host(AUDIT_PATH)로 이전 + P3 describe 신규(sidebar 1항목·구 항목 제거·redirect·redirect-only).
- **검증:** 격리 node self-check 31 계약/가드 — 30 PASS, 1(append-only) 위양성(sandbox 마운트가 audit page를 890행에서 truncate; 실제 1075행 Read로 append-only 1055행 실재 확인, P3에서 audit page 무변경). 실질 31/31. **실 build+vitest = operator 필수.**
- **✋ Gate:** 메뉴 1항목 ✅, 구 링크 redirect ✅, dead link 0 ✅, 감사 컴플라이언스 무회귀(audit page 무변경) ✅. Rollback: sidebar 2항목 + activity-logs client 페이지 복귀.

### Phase 4: 모바일 + Smoke — ✅ Complete (코드, 2026-06-14) / 라이브 smoke = operator
- Status: [x] Complete (코드/검증 sandbox, 라이브 375px smoke operator 대기)
- **🟢 GREEN:** 통합 활동 모드에 §11.311 컴팩트 KPI 3카드(시스템 활동/AI 처리/경고·오류) 이식 — **구 activity-logs redirect 로 사라진 KPI 기능 동등성 회복**. grid-cols-3 한 줄 / p-3 / text-lg / 0건 회색·1+건 활성 / 경고 1+건 red(§11.302). 감사 모드는 §11.311b 모바일 정합 이미 완료(무변경).
- **산출물:** `dashboard/audit/page.tsx` — lib `isAiActivity`/`isAlertActivity` import + `Zap`/`AlertTriangle` 아이콘 + 활동 KPI 계산(KST 자정 기준 today/AI/경고) + KPI 그리드 JSX(활동 모드 헤더↔필터 사이). sentinel P4 describe 2건(KPI 그리드·분류 헬퍼).
- **검증:** 격리 node 7/7 (KPI testid·grid-cols-3·헬퍼·아이콘·계산). 감사 컴플라이언스 본문 무변경(append-only/Part11/export). **실 build+vitest = operator.**
- **✋ Gate:** 모바일 KPI 정합 ✅, 기능 동등성 회복 ✅, 컴플라이언스 무회귀 ✅. Rollback: KPI 블록 + import revert.
- **잔여 라이브 smoke(operator/호영님 계정):** 375px 모드 토글·활동 KPI 렌더·감사 export 동작 + 비admin OAuth 계정 활동 모드만/감사 탭 비노출 + 구 activity-logs redirect 실측.
...
- Overall: 100% 코드 (P0~P4 완료) — operator 실 build+vitest+push 게이트 + 라이브 375px/비admin smoke 잔여
- Current: P4 코드 완료. operator 게이트 통과 후 라이브 smoke 만 잔여.
