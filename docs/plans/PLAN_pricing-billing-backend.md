...
- **착수 보류 (호영님 2026-06-27):** 포트원 계정·V2 secret·SDK 미준비 → 추후 착수. 준비되면 P1(빌링키) 스캐폴드(schema BillingKey + env-gated 엔드포인트)부터. SDK 설치=operator(sandbox npm add 금지), 계정·secret=ops/CEO, 법무(약관·환불·자동결제 동의)=P4 前 필수.
