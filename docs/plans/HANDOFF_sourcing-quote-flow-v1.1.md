...
| ⑦ | **sticky `>= 2`** 🆕 | "sticky 바 `cartItems.length >= 2` 조건" | 🛑 **보류 확정 — §2 보다 먼저 할 수 없다** | §2-C 참조 |
...
문서가 `>= 2` 를 요구하는 전제는 **"레일에 `견적 진행 →` 가 있다"** 인데 그게 아직 없다.
...
둘은 중복이 아니라 상보다. 그리고 **모바일에는 "견적 진행" 진입이 애초에 없다** —
§2 구현 시 모바일 쪽도 함께 만들어야 한다(이번 실측의 부수 발견).

`>= 2` 를 지금 걸면 1건 담은 **데스크탑** 사용자의 진입이 사라진다. 모바일은 진입이
없으므로 영향 없으나, 그 사실 자체가 §2 의 목적과 반대다.

**순서 확정:**
1. §2 로 레일에 `견적 진행 →` 생성 (왕복 이후)
2. 그때 `QuoteTrayBar` 가 데스크탑에서 중복인지 실측
3. 중복이면 그때 조건 조정 — `>= 2` 일 수도, 반응형 분기일 수도
