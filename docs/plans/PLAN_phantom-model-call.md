...
### 사용자 체감 실측 (2026-08-12, 호영님 지시 — 교정 전 관측)

| 질문 | 실측 |
|---|---|
| ① 라우트 응답 | `catch` 가 잡아 **500 `{error:"Failed to update SSO config"}`**. **삼키지 않는다** |
| ② 저장 UI | `onError` → **destructive toast "SSO 설정 저장 실패"** + 서버 메시지. **거짓 성공 아님** |
| ③ 조회 실패의 렌더 | 🛑 **조용하다.** `ssoLoading`·에러 상태를 **렌더에 쓰지 않아** 화면이 `ssoEnabled=false` 기본값으로 그려진다 |
| 로컬 상태 잔존 | 새로고침하면 초기화된다(GET 이 실패하므로 채워질 값이 없다) — **거짓 영속은 없다** |

**판정: 호영님 분류의 세 번째("에러가 명확히 뜬다")에 가깝다.** 저장 경로는
§placeholder-success 가 아니다.

다만 ③ 이 **약한 거짓 표시**다 — 조회 실패를 "SSO 가 꺼져 있음" 으로 읽게 만든다.
사용자는 *설정이 없다* 고 믿는다.

### 판정 — 표면 미생성 + sentinel 짝 강제 (2026-08-12 처리)

호영님 포괄 지시: *"어느 쪽이든 6컬럼이 생기기 전까지 이 화면은 동작할 수 없으므로,
처리는 '표면 미생성 + sentinel 로 짝 강제' 가 기본"*.

⚠️ 결과별 3분기의 세 번째("그대로 두고 기록만")와 위 포괄 지시가 문면상 갈린다.
**포괄 지시를 따랐다** — 근거가 실측으로 확인됐기 때문이다:
저장뿐 아니라 **조회도 500** 이라 화면 전체가 동작 불가이고, ③ 의 거짓 표시가 남는다.

처리:
- `dashboard/settings/enterprise` 의 SSO 탭 → **폼 미생성**(스위치·입력·저장 버튼 삭제.
  disabled 아님). "SSO 는 준비 중입니다" 한 줄만 남긴다
- 항상 실패하는 GET → `enabled: false` (페이지 열 때마다 500 이 나가지 않는다)
- 배선(`ssoMutation`·`handleSaveSSO`·폼 state)은 **지우지 않는다** — 컬럼이 서면
  UI 와 함께 되살릴 대상
- **라우트 유지** (#7 초대와 같은 판단 — 생성 절반은 멀쩡하다)
- sentinel `ops/sso-phantom-surface.test.ts` (S0~S3):
  **컬럼이 생기면 S0 이 RED** 가 되어 UI 복원을 알린다. corrupt→RED 양방향 실증
