...
- **Status:** 규모 실측 (2026-08-15) · **D1 §2 착수 전 — 개발 DB 비밀번호 회전 대기**
  - 회전은 호영님 몫이며 **유일한 병목**. 통보 후 §2(치환) → §3(프로브) 를 **한 배치**로 간다
  - 🛑 대기 중 소스 변경 0 유지. **이 대기를 다른 작업으로 메우지 않는다** —
    드리프트 전수 재도출(별도 배치 예약분)도 포함해서 착수 금지
  - 근거: 미커밋 변경을 무기한 두면 **④ 정지 조건(선언 밖 변경 감지)이 무력화**된다.
    감지 장치가 내 미커밋 작업과 실제 부작용을 구분하지 못한다
