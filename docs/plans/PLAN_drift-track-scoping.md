...
3. **배치 D3 (마이그레이션 7경로)** — "필드 추가 vs 참조 제거" 설계 판단 **먼저 회신**.
   추가 쪽이면 prod DDL 4스텝
4. ~~**배치 D4 (cadence 성공 위장)**~~ → ✅ **완료 (2026-08-15)**. 감사 무결성 축 카드 신설:
   §audit-integrity-200-mask. 실패 주입 2/2 확진, 수정은 미수행(설계 판단 필요)
...
## 11. 순서 확정 (2026-08-15) — D4 → D3, D2 는 D4 다음

```
D4 ✅ 완료 — 감사 무결성 확진 (§audit-integrity-200-mask)
D2    safety/sds/bulk ×2 원인 판정 — 로그 수집 선결
D3    (나) 마이그레이션 + D1c 8경로 재프로브
```

**D4 를 먼저 한 근거는 규제 무게만이 아니다** — D3 가 8경로를 살리면
죽어 있어서 문제가 안 되던 경로들이 **감사 공백을 만들며 200 을 반환하기 시작한다**.
D1c 의 `deleteMany` 트랜잭션화와 같은 구조: 드리프트 수정이 잠재 위험을 활성화한다.

⚠️ D2 는 **아직 로그를 수집하지 못했다** — D4 배치는 대량 스윕을 하지 않았고
(풀 고갈 규칙), `safety/sds/bulk` 는 비-GET 이라 ② 스윕 대상도 아니었다.
D2 착수 시 **별도 요청**으로 로그를 수집해야 한다.
