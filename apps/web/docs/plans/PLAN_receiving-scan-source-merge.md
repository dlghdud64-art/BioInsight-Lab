...
**④ 스키마 필드 추가는 출력 예산의 소비다** (2026-09-05 · ④가 ①을 악화시켰다)

| 시점 | rawText 길이 | 결과 |
| :--- | :--- | :--- |
| ④ 이전 4품목 | 1094 | 한 번 잘림 · 한 번 통과(경계선) |
| ④ 이후 4품목 | **1534 (+40%)** | 잘림 (`finishReason=MAX_TOKENS` 실측) |

  `lotNumber`·`expiryDate` **2필드**를 늘리자 출력이 40% 길어져 **잘림 경계가 내려왔다.**
  경계선이던 4품목이 안정적 잘림으로 바뀌었다.
  자리를 만든 것은 옳았다(Lot 이 canonical 에 닿는 유일한 경로다). 대가가 이것이다.
  ⇒ 프롬프트 스키마에 필드를 더할 때는 **출력 길이 영향을 함께 잰다.**
  조치: §ocr-output-budget — thinking 예산 분리(1순위) → 상한 상향(2순위) →
  `finishReason` 감지 유지(마지막 방어선 · 20품목이 오면 또 잘린다).
