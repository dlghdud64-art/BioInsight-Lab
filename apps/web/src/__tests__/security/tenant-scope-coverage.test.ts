 ...
 *
 * ═══════════════════════════════════════════════════════
 * 🛑 이 단언의 등급 — **GREEN 은 무결 증명이 아니다**
 * ═══════════════════════════════════════════════════════
 *
 * 이 파일이 보장하는 것은 **신규 회귀 차단**까지다:
 *   "테넌트 모델을 건드리면서 소유권/조직 검사도 미들웨어 커버도 없는 핸들러가
 *    **새로** 들어오면 RED 가 된다."
 *
 * 보장하지 **않는** 것 — **현 상태 무결**:
 *   마커가 존재한다는 것은 "그 핸들러 어딘가에 검사처럼 보이는 코드가 있다"이지
 *   **그 검사가 지금 요청된 리소스를 지킨다**는 증명이 아니다. 검사가 무관한
 *   엔티티를 지키고 있어도 이 단언은 GREEN 이다.
 *   실제로 `organizations/[id]/members` GET 이 그 형태로 유출 중이었고, 이 단언은
 *   그것을 "신뢰 高" 로 분류했다 (A5 반증 스윕이 잡았다).
 *
 * 특히 `scopedWhereVar`(`where: { … userId … }`)는 최다 등장 마커이면서 가장 느슨하다.
 * 이 마커 **단독** 의존 핸들러는 정적으로는 판정 불가로 봐야 한다.
 *
 * → **현 상태 무결은 교차조직 런타임 스윕(A5)으로만 닫힌다.**
 *   이 단언이 GREEN 이라는 이유로 스윕을 생략하면 이 파일을 만든 이유가 사라진다.
 */
