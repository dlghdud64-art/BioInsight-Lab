...
describe("§quote-management-redesign P5 — 신규 표면 반응형(375px 잘림 0)", () => {
  ...
  it("우선순위 컨트롤 = 테이블 셀 터치 영역(min-h 28px, 너비는 라벨+캐럿 content)", () => {
    // §quote-priority-picker — 옛 pill(정사각 min-h/min-w 28px) → 팝오버 트리거 버튼(min-h-[28px] + px-1.5 + 라벨/캐럿).
    //   터치영역 ≥28px 보호의도 불변: 세로 min-h-[28px] 유지, 가로는 content+padding로 28px 초과(min-w 불필요).
    expect(PAGE).toMatch(/min-h-\[28px\] px-1\.5/);
  });
});
