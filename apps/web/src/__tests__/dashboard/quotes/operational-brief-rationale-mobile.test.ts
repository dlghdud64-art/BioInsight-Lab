...
describe("§11.222 — mobile bottom sheet 인과관계 정합", () => {
  it.skip("MobileOperationalBriefSheet 의 facts prop 영역에 인과관계 helper call (또는 inline emoji)", () => {
  ...
  it.skip("같은 collapse state 재사용 (desktop + mobile 동일)", () => {
  ...
  it.skip("mobile facts 의 3-row table (현재 상태 / 다음 액션 / 수신 견적) collapsed 안에 보존", () => {
  ...
  it.skip("mobile 영역에도 toggle '상세 보기' 또는 '접기' CTA 한국어", () => {
    const mobileFactsMatch = src.match(/MobileOperationalBriefSheet[\s\S]*?facts=\{[\s\S]{0,2500}/);
    expect(mobileFactsMatch?.[0]).toMatch(/상세 보기|접기|펼치기/);
  });

  it.skip("§11.222 cluster trace marker", () => {
