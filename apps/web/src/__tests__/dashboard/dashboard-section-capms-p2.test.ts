...
// ── (E) P2 격리 — page 미교체 ──────────────────────────────────────────
describe("§main-dashboard-redesign P2 (E) — page 미교체(훅 revert=무영향)", () => {
  it("dashboard page 가 P2 훅 아직 미소비(P3~5 모듈별 채택)", () => {
    expect(read("src/app/dashboard/page.tsx")).not.toMatch(/useDashboardSection/);
  });
});
