...
describe("§11.244 Phase B #4a — overview 탭 월별 차트 mockup overlay", () => {
  ...
  it("overview 빈 상태 메시지 — '발주 데이터가 쌓이면' 또는 '첫 발주 완료 후'", () => {
    /* 승계 §spend-source-honesty (2026-09-26 · 호영님 실측) — 「발주 데이터」·「첫 발주 완료」 는 제품에 없는 행위를 가리켰다.
       이 화면의 수는 PurchaseRecord 에서 오고 그 생산자는 견적의 「구매 진행 처리」 다.
       명제(빈 상태가 언제 채워지는지 문구로 알린다)는 불변 · 이름만 참인 것으로 옮겼다. */
    expect(page).toMatch(/(구매 기록이 쌓이면|첫 구매 처리 후)/);
  });
});
