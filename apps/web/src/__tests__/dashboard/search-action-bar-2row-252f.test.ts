 ...
 *
 * 🔄 부분 진화(§sourcing-counter-timing step3, 호영님 승인 2026-07-25 'b'):
 *   md+ 1줄 세그먼트 바 도입 · **모바일(md↓) 2행 보존**(md:hidden 래핑). 2행 강제는 모바일 한정으로 전환,
 *   min-h-[44px]·compare/quote 조건부 숨김·CTA 축약·전체 해제 invariant 는 모바일 블록에서 그대로 보존.
 *   전면 폐기 아님 — 아래 어서션 전건 유지 + md:hidden(모바일 2행) 앵커 추가.
 *
 * 호영님 spec:
...
describe("§11.252f #1 — 2행 구조 (비교 행 + 견적 행 독립 분리)", () => {
  ...
  it("🔄 진화 — 2행 블록 모바일 한정(md:hidden) + md+ 1줄 세그먼트 공존", () => {
    // 결정 교체(호영님 승인 2026-07-25 'b'): md+ 1줄 세그먼트 도입, 모바일 2행 보존.
    expect(code).toMatch(/md:hidden/); // 모바일 2행 블록
    expect(code).toMatch(/hidden md:flex/); // md+ 1줄 세그먼트 바
  });
});
