...
// §auth-empty-state-fix (호영님 P1) — H-A 클라이언트 에러 게이트(redirect 레이스/미발화 안전망).
//   stats fetch 실패를 신규유저 onboarding 으로 오인하지 않도록 명시 분기. onboarding 은 성공+allEmpty 일 때만.
describe("§auth-empty-state-fix — 에러 ≠ 빈데이터 분리", () => {
  it("dashboard 가 statsError 추출", () => {
    expect(PAGE).toMatch(/isError:\s*statsError/);
  });
  it("stats 실패 시 에러 분기(onboarding 낙하 차단) — rawStats zeros 폴백 전에 early return", () => {
    expect(PAGE).toMatch(/statsError && !dashboardStats/);
    expect(PAGE).toContain("세션이 만료되었거나 데이터를 불러오지 못했습니다");
    // 에러 게이트가 rawStats(zeros 폴백)·hasAnyData onboarding 분기보다 먼저 와야 함.
    const errIdx = PAGE.indexOf("statsError && !dashboardStats");
    const rawIdx = PAGE.indexOf("const rawStats = dashboardStats || {}");
    expect(errIdx).toBeGreaterThan(0);
    expect(errIdx).toBeLessThan(rawIdx);
  });
  it("만료 UX(§4) — 재시도 + 다시 로그인 CTA", () => {
    expect(PAGE).toContain("다시 로그인");
