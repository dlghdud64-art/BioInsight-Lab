...
describe("§랜딩 P1 — 갱신 라벨 존재", () => {
  it("hero 캡션 '선택안 확정'(발주 전환 제거 카피)", () => {
    expect(HERO).toMatch(/견적 요청부터 선택안 확정까지/);
  });
  it("hero KPI '회신 대기'", () => {
...
describe("§랜딩 P1 — 무회귀(랜딩 골격 보존)", () => {
  it("hero 카피 '한 화면에서 운영합니다' 보존", () => {
    expect(HERO).toMatch(/한 화면에서 운영합니다/);
  });
  ...
  it("ops-preview 선택된 건 요약(최저가·기존 거래 칩, (b) 현행 유지) 보존", () => {
    expect(OPS).toMatch(/최저가/);
    expect(OPS).toMatch(/기존 거래/);
  });
});
