...
describe("§invite-pairing I0 — 수집이 실제로 동작한다", () => {
  ...
  /* ── I0 승계 (2026-09-04, §invite-flow Phase 3-3) ────────────────────────────
   * 옛 I0 은 `acceptScreenExists === false` 를 단언하는 **알림**이었다. 화면이 생기면 RED 로
   * "UI 를 되살리고 단언을 승계하라" 고 말하는 것이 임무였고, 실제로 그렇게 발화했다.
   * 임무를 마친 알림을 `true` 로 뒤집기만 하면 도장 찍기다 — **역방향 잠금**으로 바꾼다.
   *
   * I1·I2 는 **그대로 유효하다.** 조직 상세 초대는 Phase 4 에서 열지만, 이 파일이 지키는
   * 표면은 `settings/workspace` 초대 카드이고 그쪽은 **메일 발송이 없어** 아직 닫혀 있다.
   * 그 카드를 되살리는 순간 I1·I2 가 RED 로 알린다 — 알림이 이미 그 자리에 있으므로
   * 별도 알림을 새로 만들지 않는다. */
  it("역방향 — 수락 화면이 실재한다 (지우면 링크가 다시 죽는다)", () => {
    expect(
      acceptScreenExists,
      "수락 화면을 지웠다. 지우면 초대 링크가 다시 404 가 된다(§onboarding-blocker #7).",
    ).toBe(true);
  });
});
