 ...
 *     → P4 정정(2026-07-22 실측, 호영님 승인): 위 전제 반박 — 상세 라우트 실재
 *       (quotes/[quoteId]·purchase-orders/[poId]) · overlay 스토어 호출 0건(전역 hook 만) ·
 *       hook didProcess 구조상 동일 마운트 재클릭 무시. 계약 = 실재 라우트 직행으로 진화.
...
describe("§mobile-logs P4 계약 — 활동 리스트 날짜 그룹 + 행 딥링크", () => {
  ...
  // P4 진화(호영님 승인 2026-07-22) — 구 계약(?overlay= 규약 재사용)은 P0 전제가 실측으로
  //   반박되어 교체: 행 딥링크 = 실재 라우트 직행(activityDeepLink — 매핑 실재 대상만 링크,
  //   가짜 링크/dead link 0 · 매핑 부재 도메인 = 비링크 행 수용).
  it("행 탭 딥링크 — 실재 라우트 직행(activityDeepLink), 가짜 링크 0", () => {
    const src = read(AUDIT_PATH);
    expect(src).toMatch(/data-testid="log-row-link"/);
    expect(src).toMatch(/function activityDeepLink/);
    expect(src).toMatch(/\/dashboard\/quotes\/\$\{entityId\}/);
  });
});
