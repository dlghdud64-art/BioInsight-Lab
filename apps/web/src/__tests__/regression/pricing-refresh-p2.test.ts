 ...
 *   - 2곳: quotes/inventory POST 401 직후 enforcePlanLimit → 초과 시 429 + 한도·사용량 안내.
 *     (§pricing-redesign 2026-06-27: orders(PO) 한도 폐기 → orders enforce 제거)
...
describe("§pricing-refresh P2 — enforce helper", () => {
  ...
  // §pricing-redesign (호영님 2026-06-27) — orders(PO) enforce 폐기 → 카운트 2종.
  it("카운트 2종(이번달 quote / 누적 productInventory) + 초과 throw", () => {
    ...
    expect(HELPER).not.toMatch(/db\.order\.count/);
  });
});

describe("§pricing-refresh P2 — 2곳 POST 배선(초과 429 + 안내)", () => {
  ...
  // §pricing-redesign (호영님 2026-06-27) — orders 라우트 PO enforce 제거 확인 (dead/no-op 0).
  it("orders POST — enforcePlanLimit 미호출 (PO 한도 폐기)", () => {
    expect(O).not.toMatch(/enforcePlanLimit/);
  });
});
