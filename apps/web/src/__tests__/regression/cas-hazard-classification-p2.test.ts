...
describe("§cas-hazard P2 — 표 무결성", () => {
  it("시작셋 규모(§cas-table-seed-expansion 반영, 55종 이상)", () => {
    // 36종 시작 → LabAxis 실보유 시약 유래 19종(항생제 제외) 확장. floor 상향으로 확장 회귀 보호.
    expect(Object.keys(CAS_GHS_TABLE).length).toBeGreaterThanOrEqual(55);
  });
  it("모든 key = 정규화된 CAS", () => {
