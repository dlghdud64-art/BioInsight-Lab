...
describe("§11.320 — 상태 배너 통합 (Phase 2 GREEN target)", () => {
  ...
  it("배너 상태별 분기 3 case — 정상 emerald / 만료 임박 yellow / 위험 red (§inventory-brief-sian 시안 톤 배경)", () => {
    const src = read(PATH);
...
describe("§11.320 Phase 5 — 모바일 터치 영역 ≥ 44px (CLAUDE.md §8 정합)", () => {
  ...
  it("접기/펼치기 button hit area min-h-[36px] 확장 (32px→36px 강화)", () => {
    const src = read(PATH);
    // LOT / Flow / History / Actions 접기 button 모두 min-h-[36px] 적용 (터치영역 강화)
    const expandHitArea = (src.match(/min-h-\[36px\] px-2 -mx-2 inline-flex items-center/g) || []).length;
    expect(expandHitArea).toBeGreaterThanOrEqual(3);
  });
});

describe("§11.320 Phase 5 — KPI 모바일 정합 (§11.322 Phase 2 인라인 row 전환)", () => {
  it("KPI 인라인 라벨-값 row 패턴 — 모바일 폭 ~360px/400px 안정 (잘림 0)", () => {
    const src = read(PATH);
    // §11.322 Phase 2 에서 grid-cols-3 → 인라인 row 4 (flex justify-between) 전환
    // 핵심 testid 가 flex justify-between 컨테이너 안에 있는지 확인
    expect(src).toMatch(/data-testid="inventory-context-kpi-current"[\s\S]{0,200}flex justify-between/);
    // 옛 grid-cols-3 + MetricCell 패턴 잔존 0
