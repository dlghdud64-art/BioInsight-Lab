...
describe("§safety-modal-upgrade SM-P4c — GET owner/org 스코프", () => {
  it("무파라미터 GET where에 owner/org 스코프(과다노출 해소)", () => {
    expect(ROUTE).toMatch(/where\.OR\s*=/);
    expect(ROUTE).toMatch(/userId: session\.user\.id/);
    expect(ROUTE).toMatch(/organizationId: \{ in: userOrgIds \}/);
  });
  it("명시 organizationId 는 where.organizationId 로 스코프(멤버십 게이트 후)", () => {
    expect(ROUTE).toMatch(/where\.organizationId = organizationId/);
  });
