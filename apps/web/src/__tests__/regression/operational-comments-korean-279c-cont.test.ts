...
describe("§11.279c-cont — invariant 보존 (변수/함수/타입명 lineage 보존)", () => {
  ...
  it("operator-quick-actions — 견적 발송 진입 보존 (§11.364 D-1: in-card expand 강등→Link 라우팅)", () => {
    // setIsQuoteDispatchExpanded(in-card expand) 은 §11.364 D-1 로 폐지, 발송 진입은 균질 Link 로 보존.
    expect(FILES.operatorQuickActions).toMatch(/href:\s*"\/dashboard\/quotes\?labaxisPilot=quote-dispatch"/);
    expect(FILES.operatorQuickActions).not.toMatch(/setIsQuoteDispatchExpanded/);
  });
