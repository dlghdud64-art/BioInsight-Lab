...
describe("§inventory-mobile-reorder-gate P3 — 소프트 게이트(하드 차단 금지)", () => {
  ...
  it("override 기록 — 검토 시트 표기 + 견적 초안 reason 전파(front-only success 0)", () => {
    ...
    expect(SHEET).toMatch(/params\.set\("reason", "안전 재고 미달 — 재고 운영 도우미 권장" \+ overrideNote\)/);
  });
});
...
describe("§inventory-mobile-reorder-gate — 기존 invariant 보존(회귀 0)", () => {
  ...
  it("310/p3b lock — 견적/발주 route·prefill·게이팅 보존", () => {
    expect(SHEET).toMatch(/reason: "안전 재고 미달 — 재고 운영 도우미 권장"/);
    expect(SHEET).toMatch(/prefill: "reorder-recommendation"/);
    expect(SHEET).toMatch(/disabled=\{!hasVendor \|\| !purchasingOn\}/);
