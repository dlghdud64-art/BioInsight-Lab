...
export const TASK_STATUS_BADGE: Record<TaskStatus, { label: string; color: string }> = {
  READY: { label: "준비", color: "bg-slate-800 text-slate-300" },
  REVIEW_NEEDED: { label: "검토 필요", color: "bg-amber-900/40 text-amber-800" },
  IN_PROGRESS: { label: "진행 중", color: "bg-blue-900/30 text-blue-800" },
  WAITING_RESPONSE: { label: "응답 대기", color: "bg-purple-900/30 text-purple-800" },
  ACTION_NEEDED: { label: "조치 필요", color: "bg-red-900/40 text-red-800" },
  COMPLETED: { label: "완료", color: "bg-green-900/30 text-green-800" },
  FAILED: { label: "실패", color: "bg-red-200 text-red-900" },
  BLOCKED: { label: "차단됨", color: "bg-slate-800 text-slate-200" },
};

export const APPROVAL_STATUS_BADGE: Record<ApprovalStatus, { label: string; color: string }> = {
  NOT_REQUIRED: { label: "", color: "" },
  PENDING: { label: "승인 대기", color: "bg-yellow-900/40 text-yellow-800" },
  APPROVED: { label: "승인됨", color: "bg-green-900/30 text-green-800" },
  REJECTED: { label: "거절됨", color: "bg-red-900/40 text-red-800" },
};
