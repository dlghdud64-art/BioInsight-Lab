...
// ══════════════════════════════════════════════════════════════════════════════
// P3-4: Repository-First Async Variant
// ══════════════════════════════════════════════════════════════════════════════

export async function validateBaselineAtBootFromRepo(
  runtimeState: {
    lifecycleState: string;
    releaseMode: string;
    baselineStatus: string;
  ...
  emitDiagnostic(
    ...
    "baseline-validator", "baseline-adapter", "baseline",
    "repository_to_canonical", "validateBaselineAtBootFromRepo:entry",
    {}
  );

  const checks: BaselineValidationCheck[] = [];

  // 1. canonical baseline uniqueness (still sync — uses in-memory assertSingleCanonical)
  const singleCheck = assertSingleCanonical();
  checks.push({
    name: "canonical_baseline_uniqueness",
    passed: singleCheck.valid,
  ...
  // 5. snapshot pair existence (P3-4: repo-first via canEnterActiveRuntimeFromRepo)
  if (baseline) {
    const runtimeCheck = await canEnterActiveRuntimeFromRepo(
      baseline.activeSnapshotId,
      baseline.rollbackSnapshotId
    );
    checks.push({
      name: "snapshot_pair_exists",
      passed: runtimeCheck.allowed,
