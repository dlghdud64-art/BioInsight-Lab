...
// P. Operating Loop Drift Resistance (Scenario 9)
export {
  OperatingLoopType,
  DriftVector,
  DriftHardeningClass,
  type DriftLevel,
  type DriftSignals,
  type OperatingLoopCatalogEntry,
  type ErosionScenarioResult,
  type RenewalVerdict,
  type RenewalIntegrityResult,
  type AmendmentAccumulationVerdict,
  type AmendmentAccumulationResult,
  type ObligationContinuityVerdict,
  type ObligationContinuityResult,
  type PurposeLockVerdict,
  type PurposePressureType,
  type PurposePressureScenario,
  type PurposeLockTestResult,
  type ReviewSubstanceVerdict,
  type ReviewSubstanceResult,
  type SunsetDisciplineVerdict,
  type SunsetDisciplineResult,
  type OperatingLoopVerdict,
  type LoopDimensionScore,
  type OperatingLoopScorecard,
  type PostScenario9Decision,
  type DriftHardeningItem,
  type OperatingLoopDriftSimulationResult,
  getOperatingLoopCatalog,
  getLoopCatalogEntry,
  calculateDriftLevel,
  runAllErosionScenarios,
  verifyRenewalIntegrity,
  analyzeAmendmentAccumulation,
  assessObligationContinuity,
  stressTestPurposeLock,
  assessReviewSubstance,
  assessSunsetDiscipline,
  generateOperatingLoopScorecard,
  determinePostScenario9Decision,
  addHardeningItem,
  getHardeningBacklog,
  generateHardeningBacklogFromErosion,
  runOperatingLoopDriftSimulation,
  getSimulationResults as getDriftSimulationResults,
} from "./operating-loop-drift-resistance";

export {
  type DriftCardSeverity,
  type DriftDashboardCard,
  getDriftDashboardCards,
} from "./drift-dashboard-cards";

export {
  type Scenario9MemoryCategory,
  type ConstitutionalMemoryEntry,
  recordScenario9Lessons,
  getScenario9Memory,
  getScenario9MemoryByCategory,
} from "./drift-constitutional-memory";

// ─── Section Q: Succession Integrity & Authority Transfer (Scenario 10) ────────
export {
  SuccessionScenarioType,
  TransferStep,
  SuccessionDimension,
  type SuccessionActor,
  type SuccessionRiskFactor,
  type SuccessionScenario,
  type TransferStepStatus,
  type TransferArtifact,
  type TransferStepDefinition,
  type TransferStepResult,
  type ReadinessCriterion,
  type ReadinessGateStatus,
  type SuccessorReadinessGateResult,
  type JudgmentContinuityResult,
  type JudgmentDivergence,
  type DualControlAction,
  type DualControlPolicy,
  type DualControlDecision,
  type DualControlIntegrityResult,
  type ObligationHandoffRecord,
  type ObligationHandoffResult,
  type SuccessionMemoryType,
  type MemoryHandoffRecord,
  type MemoryHandoffResult,
  type PrivilegeEscalationMethod,
  type PrivilegeEscalationSignal,
  type PrivilegeEscalationResult,
  type EmergencyTransferPolicy,
  type RegionalTransferConstraints,
  type SuccessionDimensionScore,
  type SuccessionVerdict,
  type PostScenario10Decision,
  type SuccessionScorecard,
  type SuccessionBacklogPriority,
  type SuccessionBacklogItem,
  type ScenarioSimulationResult,
  type SuccessionTimelineEntry,
  type SuccessionSimulationReport,
  runSuccessionIntegritySimulation,
  getSuccessionSimulationResults,
  getSuccessionScenarioCatalog,
  getTransferProtocolDefinition,
} from "./succession-integrity-simulation";

export {
  type SuccessionMetricStatus,
  type SuccessionDashboardMetric,
  type SuccessionDashboardCard,
  getSuccessionDashboardCards,
} from "./succession-dashboard-cards";

export {
  type SuccessionMemoryCategory,
  type SuccessionConstitutionalMemory,
  type SuccessionMemoryIntegrityResult,
  getSuccessionConstitutionalMemories,
  getSuccessionMemoriesByCategory,
  getSuccessionMemoriesByScenario,
  validateSuccessionMemoryIntegrity,
} from "./succession-constitutional-memory";

// ─── Section R: Multi-Transition Constitutional Order (Scenario 11) ────────
export {
  MultiTransitionScenarioType,
  TransitionType,
  type OverlappingTransition,
  type MultiTransitionScenario,
  type ConcurrencyPolicy,
  type ConcurrencyRule,
  type ConstitutionalRule,
  type PendingAmendmentEntry,
  type ConstitutionalVisibilityState,
  type ConstitutionalConflict,
  type RoleCollisionSeverity,
  type RoleCollision,
  type RoleCollisionResolution,
  type TransitionCoordinationEntry,
  type TransitionCoordinationOrder,
  type ArbitrationOutcome,
  type ArbitrationPolicy,
  type ObligationPileup,
  type TriagedObligation,
  type ObligationTriageResult,
  type CrisisPrecedencePolicy,
  type CrisisPrecedenceResult,
  MultiTransitionDimension,
  type MultiTransitionDimensionScore,
  type MultiTransitionVerdict,
  type MultiTransitionDecision,
  type MultiTransitionScorecard,
  type MultiTransitionBacklogPriority,
  type MultiTransitionBacklogItem,
  type MultiTransitionTimelineEvent,
  type MultiTransitionScenarioResult,
  type MultiTransitionSimulationReport,
  getMultiTransitionScenarioCatalog,
  buildConcurrencyMatrix,
  lookupConcurrencyRule,
  resolveEffectiveConstitution,
  detectRoleCollisions,
  resolveTransitionOrder,
  arbitrateLocalGlobal,
  detectObligationPileup,
  triageObligations,
  applyCrisisPrecedence,
  generateMultiTransitionScorecard,
  generateMultiTransitionBacklog,
  runMultiTransitionSimulation,
} from "./multi-transition-order-simulation";

export {
  type MultiTransitionMetricStatus,
  type MultiTransitionMetric,
  type MultiTransitionDashboardCard,
  getMultiTransitionDashboardCards,
} from "./multi-transition-dashboard-cards";

export {
  type MultiTransitionMemoryCategory,
  type MultiTransitionConstitutionalMemory,
  type MultiTransitionMemoryIntegrityResult,
  getMultiTransitionConstitutionalMemories,
  validateMultiTransitionMemoryIntegrity,
} from "./multi-transition-constitutional-memory";
