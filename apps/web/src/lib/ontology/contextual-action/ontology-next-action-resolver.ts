...
// ── Sourcing search context ──

function resolveSourcingContext(input: ContextualActionInput): ResolvedNextActionResult {
  const { counts, activeWorkWindow } = input;

  // 이미 work window가 열려 있으면 그 문맥을 우선
  if (activeWorkWindow === "request-submission") {
    return {
      nextRequiredAction: makeAction("continue_submission", "요청 제출 검토 계속", "primary",
      ...
      blockedActions: [],
      whyThisAction: "현재 제출 검토 작업 창이 열려 있습니다.",
  ...
  if (activeWorkWindow === "request-assembly") {
    return {
      ...
      blockedActions: [],
      whyThisAction: "현재 요청 조립 작업 창이 열려 있습니다.",
  ...
  if (activeWorkWindow === "compare" || activeWorkWindow === "compare-review") {
    return {
      ...
      blockedActions: [],
      whyThisAction: "현재 비교 검토 작업 창이 열려 있습니다.",
      currentStageLabel: "비교 검토",
      mode: "contextual",
    };
  }

  // 견적 후보가 있으면 → request assembly가 next
  if (counts.quoteItems > 0) {
    ...
    return {
      nextRequiredAction: makeAction("start_request_assembly", "견적 요청 조립", "primary",
      ...
      currentStageLabel: "견적 후보 선택 완료",
      mode: "contextual",
    };
  }

  // 비교 후보만 있으면 → compare가 next
  if (counts.compareIds >= 2) {
    return {
      nextRequiredAction: makeAction("start_compare", "AI 비교 분석", "primary",
        `${counts.compareIds}개 후보를 비교합니다.`, null, "compare", "sourcing"),
      availableFollowUpActions: [],
      blockedActions: [],
      whyThisAction: `비교 후보 ${counts.compareIds}개가 선택되어 있어 비교 분석이 가능합니다.`,
      currentStageLabel: "비교 후보 선택",
      mode: "contextual",
    };
  }
  ...
  if (counts.compareIds === 1) {
    return {
      nextRequiredAction: makeAction("add_compare", "비교 후보 추가", "primary",
