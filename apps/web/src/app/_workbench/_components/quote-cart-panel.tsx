...
interface QuoteCartPanelProps {
  ...
  /** §11.339 v2 4 — 외부에서 견적함 탭으로 강제 전환(하단 바 "검토 N"/"견적 N" 클릭 시). */
  forceQuoteKey?: string | null;
  /** §11.339 v2 5 — 외부에서 비교함 탭으로 강제 전환(하단 바 "비교 N" 클릭 시). */
  forceCompareKey?: string | null;
...
export function QuoteCartPanel({
  ...
  // §11.339 v2 5 — 하단 바 "비교 N" 클릭 → 비교함 탭 전환.
  useEffect(() => {
    if (forceCompareKey) setTab("compare");
  }, [forceCompareKey]);
