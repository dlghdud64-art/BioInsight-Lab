...
const ROUTE_CONFIG: Record<ReopenRoute, { label: string; icon: any; color: string; desc: string }> = {
  ...
  watch_only: { label: "Watch 유지", icon: Eye, color: "text-yellow-400", desc: "즉시 재진입 없이 모니터링" },
      ...
      <div className="bg-[#1C2028] border border-bd rounded-xl shadow-2xl w-full max-w-3xl max-h-[88vh] overflow-hidden flex flex-col">
        ...
        <div className="px-5 py-3 border-t border-bd bg-[#181E28]">
          ...
          <div className="flex gap-2">
            ...
            {!isRecorded ? <Button size="sm" className="flex-1 h-8 text-[10px] bg-blue-600 hover:bg-blue-700 text-white font-medium" onClick={recordReopen} disabled={!validation?.canRecordProcurementReentryReopen}><RefreshCw className="h-3 w-3 mr-1" />Procurement Reopen 저장</Button> : <div className="flex gap-1.5 flex-1">
              {validation?.canOpenSourcingSearchReopen && <Button size="sm" className="flex-1 h-8 text-[10px] bg-blue-600 hover:bg-blue-700 text-white font-medium" onClick={onSourcingSearchReopenHandoff}><Search className="h-3 w-3 mr-1" />Search Reopen<ArrowRight className="h-3 w-3 ml-1" /></Button>}
              ...
              {validation?.canOpenRequestReopen && <Button size="sm" className="flex-1 h-8 text-[10px] bg-emerald-600 hover:bg-emerald-700 text-white font-medium" onClick={onRequestReopenHandoff}><FileText className="h-3 w-3 mr-1" />Request<ArrowRight className="h-3 w-3 ml-1" /></Button>}
            </div>}
          </div>
        </div>
      </div>
