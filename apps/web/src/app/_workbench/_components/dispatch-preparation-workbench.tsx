        ...
        <div className="px-5 py-3 border-t border-bd bg-[#181E28]">
          ...
          <div className="flex gap-2">
            ...
            {!isRecorded ? (
              ...
              <Button size="sm" className={`flex-1 h-8 text-[10px] font-medium ${validation?.canOpenSendConfirmation ? "bg-emerald-600 hover:bg-emerald-700 text-white" : "bg-slate-700 text-slate-400"}`} onClick={handleSendConfirm} disabled={!validation?.canOpenSendConfirmation}><Mail className="h-3 w-3 mr-1" />발송 확인(Send Confirmation)<ArrowRight className="h-3 w-3 ml-1" /></Button>
            )}
          </div>
        </div>
      </div>
