      ...
      <div className="bg-[#1C2028] border border-bd rounded-xl shadow-2xl w-full max-w-3xl max-h-[88vh] overflow-hidden flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-2">
            <div className={`px-3 py-2.5 rounded-md border ${state.coverageRiskStatus === "critical" ? "border-red-500/20 bg-red-600/[0.03]" : state.coverageRiskStatus === "low" ? "border-yellow-500/20 bg-yellow-600/[0.03]" : "border-emerald-500/20 bg-emerald-600/[0.03]"}`}><div className="flex items-center gap-1.5 mb-0.5"><TrendingDown className="h-3 w-3 text-slate-500" /><span className="text-[9px] text-slate-500">커버리지</span></div><span className={`text-[11px] font-medium ${state.coverageRiskStatus === "critical" ? "text-red-300" : state.coverageRiskStatus === "low" ? "text-yellow-300" : "text-emerald-300"}`}>{state.coverageRiskStatus}</span></div>
            <div className={`px-3 py-2.5 rounded-md border ${state.expiryRiskStatus === "critical" ? "border-red-500/20 bg-red-600/[0.03]" : state.expiryRiskStatus === "warning" ? "border-yellow-500/20 bg-yellow-600/[0.03]" : "border-emerald-500/20 bg-emerald-600/[0.03]"}`}><div className="flex items-center gap-1.5 mb-0.5"><Clock className="h-3 w-3 text-slate-500" /><span className="text-[9px] text-slate-500">유효기한</span></div><span className={`text-[11px] font-medium ${state.expiryRiskStatus === "critical" ? "text-red-300" : state.expiryRiskStatus === "warning" ? "text-yellow-300" : "text-emerald-300"}`}>{state.expiryRiskStatus}</span></div>
          </div>
          <div><span className="text-[9px] font-medium text-slate-500 uppercase tracking-wider">품목별 결정</span>
            {state.candidateDecisions.length === 0 ? <Button size="sm" variant="ghost" className="w-full h-7 text-[9px] text-blue-400 hover:text-blue-300 border border-blue-500/20 mt-2" onClick={simulateDecision}>Reorder 결정 시뮬레이션</Button> : <div className="mt-2 space-y-1.5">{state.candidateDecisions.map(cd => <div key={cd.itemId} className={`flex items-center gap-3 px-3 py-2.5 rounded-md border ${cd.decisionType === "reorder_candidate" ? "border-blue-500/20 bg-blue-600/[0.03]" : "border-yellow-500/20 bg-yellow-600/[0.03]"}`}>{cd.decisionType === "reorder_candidate" ? <RefreshCw className="h-3.5 w-3.5 text-blue-400 shrink-0" /> : <Eye className="h-3.5 w-3.5 text-yellow-400 shrink-0" />}<div className="flex-1 min-w-0"><span className="text-[11px] text-slate-200 font-medium block">{cd.itemName}</span><span className="text-[9px] text-slate-500">{cd.rationale}</span></div>{cd.reorderQty > 0 && <span className="text-[10px] tabular-nums text-slate-300 font-medium shrink-0">{cd.reorderQty}개</span>}<span className={`text-[9px] px-1.5 py-0.5 rounded font-medium shrink-0 ${cd.decisionType === "reorder_candidate" ? "text-blue-400 bg-blue-600/10" : "text-yellow-400 bg-yellow-600/10"}`}>{cd.decisionType === "reorder_candidate" ? "재주문" : "Watch"}</span></div>)}</div>}
          </div>
        ...
        <div className="px-5 py-3 border-t border-bd bg-[#181E28]">
          ...
          <div className="flex gap-2">
            ...
            {!isRecorded ? <Button size="sm" className="flex-1 h-8 text-[10px] bg-blue-600 hover:bg-blue-700 text-white font-medium" onClick={recordDecision} disabled={!validation?.canRecordReorderDecisionReentry}><RefreshCw className="h-3 w-3 mr-1" />Reorder Re-entry 저장</Button> : <Button size="sm" className={`flex-1 h-8 text-[10px] font-medium ${validation?.canOpenProcurementReentryReopen ? "bg-emerald-600 hover:bg-emerald-700 text-white" : "bg-slate-700 text-slate-400"}`} onClick={onProcurementReentryReopenHandoff} disabled={!validation?.canOpenProcurementReentryReopen}><RefreshCw className="h-3 w-3 mr-1" />Procurement Re-entry Reopen<ArrowRight className="h-3 w-3 ml-1" /></Button>}
          </div>
        </div>
      </div>
    </div>
