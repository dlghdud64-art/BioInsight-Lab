      ...
      <div className="bg-[#1C2028] border border-bd rounded-xl shadow-2xl w-full max-w-3xl max-h-[88vh] overflow-hidden flex flex-col">
        ...
        <div className="px-5 py-3 border-t border-bd bg-[#181E28]">
          ...
          <div className="flex gap-2">
            ...
            {!isDecided ? <Button size="sm" className={`flex-1 h-8 text-[10px] font-medium ${state.decisionMode === "approve" ? "bg-emerald-600 hover:bg-emerald-700 text-white" : state.decisionMode === "return_for_revision" ? "bg-yellow-600 hover:bg-yellow-500 text-white" : state.decisionMode === "reject" ? "bg-red-600 hover:bg-red-500 text-white" : "bg-slate-700 text-slate-400"}`} onClick={confirmDecision} disabled={!state.decisionMode || (state.decisionMode === "approve" && !validation?.canApprove) || (state.decisionMode === "return_for_revision" && !validation?.canReturn) || (state.decisionMode === "reject" && !validation?.canReject)}>{state.decisionMode === "approve" ? <><ShieldCheck className="h-3 w-3 mr-1" />승인 확정</> : state.decisionMode === "return_for_revision" ? <><RotateCcw className="h-3 w-3 mr-1" />보완 요청</> : state.decisionMode === "reject" ? <><Ban className="h-3 w-3 mr-1" />반려</> : "결정 모드 선택"}</Button> : <div className="flex-1 text-center text-[10px] text-emerald-400 py-2">판단 완료 — {state.nextDestination}</div>}
          </div>
        </div>
      </div>
    </div>
