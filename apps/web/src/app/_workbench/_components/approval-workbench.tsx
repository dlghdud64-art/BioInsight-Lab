    ...
    <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center" onClick={e => { if (e.target === e.currentTarget) onClose(); }}>
      <div className="bg-[#1C2028] border border-bd rounded-xl shadow-2xl w-full max-w-3xl max-h-[88vh] overflow-hidden flex flex-col">
        <div className="flex items-center justify-between px-5 py-3 border-b border-bd bg-[#252A33]">
          <div className="flex items-center gap-3"><div className={`flex items-center justify-center w-7 h-7 rounded-lg border ${isDecided ? "bg-emerald-600/15 border-emerald-500/25" : "bg-yellow-600/15 border-yellow-500/25"}`}>{isDecided ? <Check className="h-4 w-4 text-emerald-400" /> : <ShieldCheck className="h-4 w-4 text-yellow-400" />}</div><div><h2 className="text-sm font-semibold text-slate-100">{isDecided ? "승인 판단 완료" : "승인 워크벤치(Approval Workbench)"}</h2><span className="text-[10px] text-slate-500">{state.requestReference} · {state.selectedVendor}</span></div></div>
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4 space-y-4">
          {/* A. Decision Header */}
          <div className="px-3 py-2 rounded-md bg-blue-600/[0.04] border border-blue-500/15"><span className="text-[9px] font-medium text-blue-400 uppercase tracking-wider block mb-0.5">이관 패키지(Handoff Package)</span><span className="text-[10px] text-blue-200">공급사(Vendor): {handoffPackage.selectedVendorId} · 품목(Items): {handoffPackage.selectedOptionIds.length}개 · 근거(Rationale): {handoffPackage.selectionReasonCodes.join(", ") || "미입력"}</span></div>
          {/* B. Decision Mode */}
          ...
          {state.decisionMode === "return_for_revision" && <div><span className="text-[9px] text-slate-500 block mb-1">보완 요청 이유</span><div className="flex flex-wrap gap-1">{RETURN_REASONS.map(r => { const isActive = state.returnReasonCodes.includes(r.code); return <button key={r.code} type="button" disabled={isDecided} onClick={() => toggleReason(r.code, "return")} className={`text-[9px] px-2 py-0.5 rounded border transition-all ${isActive ? "bg-yellow-600/15 border-yellow-500/30 text-yellow-400" : "border-bd/40 text-slate-500"}`}>{r.label}</button>; })}</div></div>}
          ...
          {/* D. Impact Preview */}
          {state.decisionMode && <div className="px-3 py-2 rounded-md border border-bd/40 bg-[#252A33]"><span className="text-[9px] text-slate-500 block mb-0.5">다음 단계 예고</span><span className="text-[10px] text-slate-300">{state.decisionMode === "approve" ? "발주 전환 진입(PO Conversion Entry)으로 이동" : state.decisionMode === "return_for_revision" ? "비교 검토(Compare Review) / 요청 재개(Request Reopen)로 복귀" : "승인 흐름(chain) 종료"}</span></div>}
        ...
        <div className="px-5 py-3 border-t border-bd bg-[#181E28]">
          ...
          <div className="flex gap-2">
            ...
            {!isDecided ? <Button size="sm" className={`flex-1 h-8 text-[10px] font-medium ${state.decisionMode === "approve" ? "bg-emerald-600 hover:bg-emerald-500 text-white" : state.decisionMode === "return_for_revision" ? "bg-yellow-600 hover:bg-yellow-500 text-white" : state.decisionMode === "reject" ? "bg-red-600 hover:bg-red-500 text-white" : "bg-slate-700 text-slate-400"}`} onClick={confirmDecision} disabled={!state.decisionMode || (state.decisionMode === "approve" && !validation?.canApprove) || (state.decisionMode === "return_for_revision" && !validation?.canReturn) || (state.decisionMode === "reject" && !validation?.canReject)}>{state.decisionMode === "approve" ? <><ShieldCheck className="h-3 w-3 mr-1" />승인 확정</> : state.decisionMode === "return_for_revision" ? <><RotateCcw className="h-3 w-3 mr-1" />보완 요청</> : state.decisionMode === "reject" ? <><Ban className="h-3 w-3 mr-1" />반려</> : "결정 모드 선택"}</Button> : <div className="flex-1 text-center text-[10px] text-emerald-400 py-2">판단 완료 — {state.nextDestination}</div>}
          </div>
        </div>
      </div>
    </div>
