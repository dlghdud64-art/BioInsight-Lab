...
export default function SafetyManagerPage() {
  ...
  // §safety-redesign 상단정합 — 저장 상태 바 제거(시안 §0). activeFrame persistence(hydration+PATCH effect)는
  //   보존하되, 화면 상단의 저장상태 표시 파생(applied/pending/saved/failureReason/boundaryLabel)은 미사용 → 제거.
  ...
  return (
    <div className="min-h-screen bg-white p-4 md:p-8 pt-4 md:pt-6">
      ...
      <div className="max-w-full mx-auto space-y-5">
        ...
        {/* §safety-redesign 상단정합 — 저장 상태 바 제거(시안 §0, 상단 공간 차지 클러터).
            activeFrame persistence(hydration+PATCH effect)는 보존, 화면 표시 바만 제거. */}

        {/* ═══ KPI 카드 4개 (호버 상세 팝업 포함) ═══ */}
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4">
          {/* 전체 화학물질 */}
          <div className="group/kpi relative rounded-xl border border-slate-200 bg-white p-3.5 md:p-4 hover:shadow-md hover:border-blue-200 transition-all cursor-default">
            <div className="flex items-center justify-between mb-2">
              <div className="flex items-center gap-1.5">
                <FileText className="h-4 w-4 text-blue-500" />
                <span className="text-xs text-slate-400">전체 화학물질</span>
              </div>
              <span className="hidden sm:inline-flex items-center text-[10px] font-semibold px-2 py-0.5 rounded-full bg-blue-50 text-blue-600">대장 등록</span>
            </div>
            <p className={`text-2xl md:text-3xl font-extrabold ${totalCount > 0 ? "text-slate-900" : "text-slate-400"}`}>{totalCount}</p>
            <p className="text-xs text-slate-500 mt-1">재고 대장 기준</p>
            {/* 호버 팝업 */}
          ...
          {/* 고위험 물질 */}
          <div className="group/kpi relative rounded-xl border border-slate-200 bg-white p-3.5 md:p-4 hover:shadow-md hover:border-red-200 transition-all cursor-default">
            <div className="flex items-center justify-between mb-2">
              <div className="flex items-center gap-1.5">
                <AlertTriangle className="h-4 w-4 text-red-500" />
                <span className="text-xs text-slate-400">고위험 물질</span>
              </div>
              <span className="hidden sm:inline-flex items-center text-[10px] font-semibold px-2 py-0.5 rounded-full bg-slate-100 text-slate-500">미분류</span>
            </div>
            <p className={`text-2xl md:text-3xl font-extrabold ${highRiskCount > 0 ? "text-slate-900" : "text-slate-400"}`}>{highRiskCount}</p>
            <p className={`text-xs font-medium mt-1 ${highRiskCount > 0 ? "text-red-500" : "text-slate-400"}`}>{highRiskCount > 0 ? "즉시 조치 필요" : "분류 대기"}</p>
            {/* 호버 팝업 */}
            <div className="absolute left-0 right-0 top-full mt-2 z-30 opacity-0 translate-y-1 pointer-events-none group-hover/kpi:opacity-100 group-hover/kpi:translate-y-0 group-hover/kpi:pointer-events-auto transition-all duration-200 ease-out">
              <div className="mx-2 rounded-xl bg-slate-900 text-white p-4 shadow-xl">
          ...
          {/* MSDS 미등록 */}
          <div className="group/kpi relative rounded-xl border border-slate-200 bg-white p-3.5 md:p-4 hover:shadow-md hover:border-yellow-200 transition-all cursor-default">
            <div className="flex items-center justify-between mb-2">
              <div className="flex items-center gap-1.5">
                <FileWarning className="h-4 w-4 text-yellow-500" />
                <span className="text-xs text-slate-400">MSDS 미등록</span>
              </div>
              <span className="hidden sm:inline-flex items-center text-[10px] font-semibold px-2 py-0.5 rounded-full bg-yellow-50 text-yellow-700">규정 준수 불가</span>
            </div>
            <p className={`text-2xl md:text-3xl font-extrabold ${msdsMissingCount > 0 ? "text-slate-900" : "text-slate-400"}`}>{msdsMissingCount}</p>
            <p className={`text-xs font-medium mt-1 ${msdsMissingCount > 0 ? "text-yellow-600" : "text-slate-400"}`}>보완 필요</p>
            {/* 호버 팝업 */}
            <div className="absolute left-0 right-0 top-full mt-2 z-30 opacity-0 translate-y-1 pointer-events-none group-hover/kpi:opacity-100 group-hover/kpi:translate-y-0 group-hover/kpi:pointer-events-auto transition-all duration-200 ease-out">
              <div className="mx-2 rounded-xl bg-slate-900 text-white p-4 shadow-xl">
          ...
          {/* 최근 점검 */}
          <div className="group/kpi relative rounded-xl border border-slate-200 bg-white p-3.5 md:p-4 hover:shadow-md hover:border-emerald-200 transition-all cursor-default">
            <div className="flex items-center justify-between mb-2">
              <div className="flex items-center gap-1.5">
                <Calendar className="h-4 w-4 text-emerald-500" />
                <span className="text-xs text-slate-400">최근 점검</span>
              </div>
              <span className="hidden sm:inline-flex items-center text-[10px] font-semibold px-2 py-0.5 rounded-full bg-slate-100 text-slate-500">점검 이력 없음</span>
            </div>
            <p className={`text-2xl md:text-3xl font-extrabold ${recentInspCount > 0 ? "text-slate-900" : "text-slate-400"}`}>{recentInspCount}</p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          ...
          {/* §safety-redesign 상단정합 — 안전 판단 요약 패널(시안 §3): 도넛+범례+가장 시급 배너+준비도 바+CTA. sticky. */}
          <div className="rounded-xl border border-slate-200 bg-white p-5 lg:sticky lg:top-20 lg:self-start">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-base font-bold text-slate-900">안전 판단 요약</h3>
              <span className="text-xs text-slate-400">오늘 기준</span>
            </div>
            ...
            <div className="flex flex-col items-center mb-4">
              <div className="relative w-44 h-44">
                ...
                <div className="absolute inset-0 flex flex-col items-center justify-center">
                  <span className={`text-4xl font-extrabold ${safetyScore > 0 ? "text-slate-900" : "text-slate-400"}`}>{safetyScore}%</span>
            ...
            <div className="space-y-1.5 mb-4">
              <div className="flex items-center gap-2 text-sm">
                ...
                <span className="text-slate-600 flex-1">즉시 조치</span>
                <span className={`font-bold ${immediateCount > 0 ? "text-slate-900" : "text-slate-400"}`}>{immediateCount}건</span>
              </div>
              <div className="flex items-center gap-2 text-sm">
                <span className="w-2.5 h-2.5 rounded-full bg-yellow-500 flex-shrink-0" />
                <span className="text-slate-600 flex-1">문서 보완</span>
                <span className={`font-bold ${docRemCount > 0 ? "text-slate-900" : "text-slate-400"}`}>{docRemCount}건</span>
              </div>
              <div className="flex items-center gap-2 text-sm">
                <span className="w-2.5 h-2.5 rounded-full bg-emerald-500 flex-shrink-0" />
                <span className="text-slate-600 flex-1">정상</span>
                <span className={`font-bold ${compliantCount > 0 ? "text-slate-900" : "text-slate-400"}`}>{compliantCount}건</span>
              </div>
            </div>

            {/* 가장 시급 배너 */}
            {msdsMissingCount > 0 && (
              <div className="mb-4 rounded-lg bg-yellow-50 border border-yellow-200 px-3 py-2.5">
                <p className="text-xs font-semibold text-yellow-800">MSDS 미등록 {msdsMissingCount}종</p>
                <p className="text-[11px] text-yellow-700 mt-0.5">등록만 완료하면 안전 지수가 회복됩니다.</p>
              </div>
            )}

            {/* 준비도 바 2종 (canonical 충족률) */}
            <div className="space-y-2.5 mb-4">
              <div>
                <div className="flex items-center justify-between text-xs mb-1">
                  <span className="text-slate-500">GMP 준비도<span className="text-slate-400"> · MSDS+점검 완료율</span></span>
                  <span className="font-bold text-slate-700 tabular-nums">{gmpReadiness}%</span>
                </div>
                <div className="h-1.5 rounded-full bg-slate-100 overflow-hidden">
                  <div className="h-full rounded-full bg-blue-500 transition-all duration-500" style={{ width: `${gmpReadiness}%` }} />
                </div>
              </div>
              <div>
                <div className="flex items-center justify-between text-xs mb-1">
                  <span className="text-slate-500">KOSHA 준비도<span className="text-slate-400"> · MSDS 보유율</span></span>
                  <span className="font-bold text-slate-700 tabular-nums">{koshaReadiness}%</span>
                </div>
                <div className="h-1.5 rounded-full bg-slate-100 overflow-hidden">
                  <div className="h-full rounded-full bg-emerald-500 transition-all duration-500" style={{ width: `${koshaReadiness}%` }} />
                </div>
              </div>
            </div>
          ...
          <div id="ai-action-queue" className="rounded-xl border border-slate-200 bg-white p-5">
            ...
            {/* §safety-redesign 상단정합 — 전체 보기 푸터(시안 §4): 화학물질 대장 테이블로 scroll(실 동작). */}
            {queueItems.length > 0 && (
              <button
                type="button"
                onClick={() => document.getElementById("safety-chem-list")?.scrollIntoView({ behavior: "smooth", block: "start" })}
                className="w-full mt-4 pt-3 border-t border-slate-100 text-xs font-semibold text-blue-600 hover:text-blue-700 inline-flex items-center justify-center gap-1"
              >
                전체 {queueItems.length}건 보기 <ArrowRight className="h-3 w-3" />
              </button>
            )}
          </div>
        </div>

        {/* §safety-redesign 상단정합 — 안전 지수 트렌드(7일) 제거(시안 기준, mock 데이터). */}

        {/* ═══ 화학물질 목록 ═══ */}
        <div id="safety-chem-list" className="flex gap-4">
