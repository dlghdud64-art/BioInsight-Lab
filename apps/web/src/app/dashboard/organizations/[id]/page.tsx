    ...
    <div className="space-y-6">
      ...
      <Tabs defaultValue="members" className="space-y-4">
        <TabsList className="bg-slate-800 p-1 rounded-lg">
          <TabsTrigger value="dashboard" className="data-[state=active]:bg-slate-900 data-[state=active]:text-slate-100 text-slate-400">
            조직활동
          </TabsTrigger>
          <TabsTrigger value="members" className="data-[state=active]:bg-slate-900 data-[state=active]:text-slate-100 text-slate-400">
            멤버
          </TabsTrigger>
          {isAdmin && (
            <TabsTrigger value="settings" className="data-[state=active]:bg-slate-900 data-[state=active]:text-slate-100 text-slate-400">
              관리
            </TabsTrigger>
          )}
        </TabsList>
                ...
                <div className="max-h-[480px] overflow-y-auto">
                  {organizationLogs.map((log, idx) => {
                    ...
                    return (
                      <div
                        key={log.id}
                        className={`flex items-start gap-4 p-4 transition-colors hover:bg-slate-800/30${idx < organizationLogs.length - 1 ? "border-b border-slate-800" : ""}`}
                      >
            ...
            <div className="flex items-center justify-between flex-wrap gap-3">
              ...
              <div className="relative w-48">
                ...
                <Input
                  ...
                  className="pl-9 h-9 text-sm border-slate-700"
                />
              </div>
            </div>
            {/* 상태별 요약 카운터 */}
            ...
            {filteredTeamMembers.length === 0 ? (
              <Card className="shadow-none border-slate-800 bg-slate-900">
                <CardContent className="py-12 text-center">
                  <Mail className="h-12 w-12 text-slate-300 text-slate-400 mx-auto mb-4" />
                          ...
                          <TableRow key={member.id} className="border-slate-800">
                            <TableCell className="py-4">
                              <div className="flex items-center gap-3">
                                <Avatar className="h-9 w-9 shrink-0 border border-slate-700">
                                  <AvatarFallback className="bg-slate-800 text-slate-400 bg-slate-800 text-slate-400 text-sm font-medium">
                                    {member.initial}
                                  </AvatarFallback>
                                </Avatar>
                                <div>
                            ...
                            <TableCell className="py-4">
                              {canEditRole && rawMember ? (
                                ...
                                >
                                  <SelectTrigger className="w-[140px] h-9 text-sm border-slate-700">
                                    <SelectValue />
                                  </SelectTrigger>
                                  <SelectContent>
                            ...
                            {isAdmin && (
                              <TableCell className="py-4 text-right">
                                {member.rawRole === "OWNER" ? (
                                  ...
                                  <Lock className="h-4 w-4 text-slate-300 text-slate-400 mx-auto" />
              ...
              <Card className="shadow-none border-slate-800 bg-slate-900">
                ...
                <CardContent className="space-y-4">
                  ...
                  <div className="flex items-center gap-4">
                    <div className="w-24 h-24 shrink-0 rounded-full border border-slate-700 bg-slate-800 overflow-hidden flex items-center justify-center">
                      {logoPreviewUrl ? (
                        <img
                          src={logoPreviewUrl}
                          alt="조직 로고"
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex rounded-md border border-slate-700 overflow-hidden">
                      <span className="inline-flex items-center px-3 text-sm text-slate-400 bg-slate-800 border-r border-slate-700 shrink-0">
                        bio-insight.lab/
                      </span>
                      <Input
                        id="edit-slug"
                        ...
                        placeholder="my-lab"
                        className="rounded-none border-0 focus-visible:ring-0 focus-visible:ring-offset-0 bg-slate-800 text-white"
                      />
                    </div>
                    {slugStatus === "checking" && (
        ...
        <DialogContent className="sm:max-w-[480px] bg-slate-900 border-slate-800">
          ...
          <Tabs defaultValue="email" className="mt-2">
            <TabsList className="grid w-full grid-cols-2 bg-slate-800">
              <TabsTrigger value="email" className="data-[state=active]:bg-slate-900 data-[state=active]:text-slate-100 text-slate-400">
                이메일 초대
              </TabsTrigger>
              <TabsTrigger value="org" className="data-[state=active]:bg-slate-900 data-[state=active]:text-slate-100 text-slate-400">
                협력사 연결
              </TabsTrigger>
            </TabsList>
  ...
  if (otherOrgs.length === 0) {
    return (
      <div className="text-center py-8 text-slate-400">
        <Building2 className="h-10 w-10 mx-auto mb-3 text-slate-300 text-slate-400" />
    ...
    <div className="space-y-3">
      ...
      <div className="space-y-2 max-h-[240px] overflow-y-auto">
        {otherOrgs.map((org) => (
          <div
            key={org.id}
            className="flex items-center justify-between rounded-lg border border-slate-700 p-3 hover:bg-slate-800/50 transition-colors"
          >
            <div className="flex items-center gap-3">
            ...
            <Button
              size="sm"
              variant="outline"
              className="shrink-0 text-xs border-slate-800 text-slate-400 border-slate-700 text-slate-500"
              disabled
            >
              Coming Soon
            </Button>
