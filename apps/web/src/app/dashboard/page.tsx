...
export default function DashboardPage() {
  ...
  if (status === "loading") {
    return (
      <div className="p-4 pt-4 md:p-8 md:pt-6 space-y-4">
        ...
        <div className="h-[72px] rounded-xl bg-slate-800 animate-pulse" />
        <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
          {[1, 2, 3, 4].map((i) => (
            <div key={i} className="h-[100px] md:h-[120px] rounded-xl bg-slate-800 animate-pulse" />
          ))}
        </div>
      </div>
    );
  ...
  const renderNotificationIcon = (type: string) => {
    switch (type) {
      case "alert":
        return <div className="flex-shrink-0 rounded-md bg-red-950/40 p-2"><AlertTriangle className="h-4 w-4 text-red-400" /></div>;
      case "quote":
        return <div className="flex-shrink-0 rounded-md bg-blue-950/40 p-2"><FileText className="h-4 w-4 text-blue-400" /></div>;
      case "delivery":
        return <div className="flex-shrink-0 rounded-md bg-green-950/40 p-2"><Truck className="h-4 w-4 text-green-400" /></div>;
      default:
        return null;
    }
  };
  ...
  return (
    <div className="p-4 pt-4 md:p-8 md:pt-6 space-y-3 md:space-y-4 overflow-x-hidden">
      ...
      {/* ═══ 1순위: 오늘의 우선 작업 ═══ */}
      <div className="rounded-xl border border-[#2a2a2e]/50 bg-[#1a1a1e] shadow-sm overflow-hidden">
        <div className="px-4 py-2.5 border-b border-[#2a2a2e]/50 flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              {stats.activeQuotes > 0 && (
                <Link href="/dashboard/quotes?status=PENDING">
                  <Button variant="outline" size="sm" className="h-7 text-[11px] px-2.5 border-blue-200 text-blue-400 hover:bg-blue-950/30">
                    견적 검토하기
                  </Button>
                </Link>
              )}
              {stats.lowStockAlerts > 0 && (
                <Link href="/dashboard/inventory?filter=low">
                  <Button variant="outline" size="sm" className="h-7 text-[11px] px-2.5 border-red-200 text-red-400 hover:bg-red-950/30">
                    재고 확인하기
                  </Button>
                </Link>
              )}
        ...
        {hasActionItems ? (
          <div className="divide-y divide-[#2a2a2e]/50 sm:divide-y-0 sm:grid sm:divide-x" style={{ gridTemplateColumns: `repeat(${actionCount}, 1fr)` }}>
            {stats.lowStockAlerts > 0 && (
              <Link href="/dashboard/inventory?filter=low" className="flex items-center gap-3 px-4 py-3 hover:bg-[#222226]/30 transition-colors group">
            ...
            {stats.activeQuotes > 0 && (
              <Link href="/dashboard/quotes?status=PENDING" className="flex items-center gap-3 px-4 py-3 hover:bg-[#222226]/30 transition-colors group">
            ...
            {stats.expiringCount > 0 && (
              <Link href="/dashboard/inventory" className="flex items-center gap-3 px-4 py-3 hover:bg-[#222226]/30 transition-colors group">
            ...
            {stats.undecidedCompareCount > 0 && (
              <Link href="/compare" className="flex items-center gap-3 px-4 py-3 hover:bg-[#222226]/30 transition-colors group">
                <div className={`rounded-md p-2 flex-shrink-0 ${stats.compareStats.slaBreachedCount > 0 ? "bg-red-950/40" : "bg-purple-950/40"}`}>
                  <GitCompare className={`h-4 w-4 ${stats.compareStats.slaBreachedCount > 0 ? "text-red-400" : "text-purple-400"}`} />
                </div>
                <div className="min-w-0 flex-1">
      ...
      <div className="md:hidden space-y-3 pb-20">
        ...
        <div className="grid grid-cols-2 gap-3">
          <Link href="/dashboard/inventory">
            <Card className="overflow-hidden cursor-pointer transition-all hover:shadow-md border-[#2a2a2e]/50 shadow-sm bg-[#1a1a1e]">
              <CardContent className="p-3">
                <div className="flex items-center justify-between mb-1.5">
                  <span className="text-[10px] font-semibold text-slate-400 uppercase tracking-wider">등록 품목</span>
                  <div className="rounded-full p-1 bg-blue-950/50"><Package className="h-3 w-3 text-blue-400" /></div>
                </div>
          ...
          <Link href="/dashboard/inventory?filter=low">
            <Card className={`overflow-hidden cursor-pointer transition-all hover:shadow-md shadow-sm ${stats.lowStockAlerts > 0 ? "border-red-900/50 bg-red-950/20" : "border-[#2a2a2e]/50 bg-[#1a1a1e]"}`}>
              <CardContent className="p-3">
                <div className="flex items-center justify-between mb-1.5">
                  <span className="text-[10px] font-semibold text-red-400 uppercase tracking-wider">재고 부족</span>
                  <div className="rounded-full p-1 bg-red-950/50"><AlertTriangle className="h-3 w-3 text-red-400" /></div>
                </div>
                <div className="text-2xl font-bold text-red-400">{stats.lowStockAlerts}</div>
                <p className="text-[10px] text-red-400 mt-0.5 leading-tight">{getStockInsight()}</p>
              </CardContent>
            </Card>
          </Link>
            ...
            <Card className="overflow-hidden cursor-pointer transition-all hover:shadow-md border-[#2a2a2e]/50 shadow-sm bg-[#1a1a1e]">
              <CardContent className="p-3">
                <div className="flex items-center justify-between mb-1.5">
                  ...
                  <div className="rounded-full p-1 bg-emerald-900/20"><DollarSign className="h-3 w-3 text-emerald-400" /></div>
                </div>
            ...
            <Card className="overflow-hidden cursor-pointer transition-all hover:shadow-md border-[#2a2a2e]/50 shadow-sm bg-[#1a1a1e]">
              <CardContent className="p-3">
                <div className="flex items-center justify-between mb-1.5">
                  ...
                  <div className="rounded-full p-1 bg-violet-950/50"><FileText className="h-3 w-3 text-violet-400" /></div>
                </div>
        ...
        {/* 3순위: 빠른 실행 */}
        <Card className="shadow-sm border-[#2a2a2e]/50 bg-[#1a1a1e]">
          ...
          <CardContent className="p-3 pt-0 grid grid-cols-2 gap-2">
            ...
            <Link href="/app/quote" className="block">
              <Button variant="outline" className="w-full h-10 justify-start text-xs gap-2 bg-blue-950/20 border-blue-900/50 text-blue-400 hover:bg-blue-950/30">
            ...
            <Link href="/dashboard/inventory" className="block">
              <Button variant="outline" className="w-full h-10 justify-start text-xs gap-2 bg-[#111114] border-[#333338] text-slate-300 hover:bg-[#222226]">
                <TrendingDown className="h-3.5 w-3.5 flex-shrink-0" />
                재고 차감
              </Button>
            </Link>
            <Link href="/dashboard/inventory" className="block">
              <Button variant="outline" className="w-full h-10 justify-start text-xs gap-2 hover:bg-[#222226]">
        ...
        {/* 4순위: 최근 알림 */}
        <Card className="bg-[#1a1a1e] border-[#2a2a2e]/50 shadow-sm">
          <CardHeader className="pb-2 p-3">
          ...
          <CardContent className="p-3 pt-0 space-y-1">
            {notifications.map((n) => (
              <Link key={n.id} href={n.href} className={`flex items-start gap-2.5 p-2 rounded-lg transition-colors ${n.unread ? "bg-blue-950/30" : "hover:bg-[#222226]"}`}>
          ...
          <Card className="bg-[#1a1a1e] border-[#2a2a2e]/50 shadow-sm">
            ...
            <CardContent className="p-3 pt-0 space-y-2">
              {stats.recentPurchases.slice(0, 3).map((p, i) => (
                <div key={p.id || `p-${i}`} className="flex items-center gap-2.5 py-1.5 border-b border-[#2a2a2e]/30 last:border-0">
                  <div className="flex h-7 w-7 shrink-0 items-center justify-center rounded-md bg-blue-950/50 text-blue-400">
      ...
      <div className="hidden md:grid md:grid-cols-7 md:gap-4">
        ...
        <div className="md:col-span-5 space-y-4">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            <Link href="/dashboard/inventory">
              <Card className="h-[124px] flex flex-col justify-between overflow-hidden cursor-pointer transition-all hover:shadow-md border-[#2a2a2e]/50 shadow-sm rounded-xl bg-[#1a1a1e]">
                <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-1 p-4">
                  ...
                  <div className="rounded-full p-1.5 bg-blue-950/50 flex-shrink-0"><Package className="h-3.5 w-3.5 text-blue-400" /></div>
                </CardHeader>
            ...
            <Link href="/dashboard/inventory?filter=low">
              <Card className={`h-[124px] flex flex-col justify-between overflow-hidden cursor-pointer transition-all hover:shadow-md shadow-sm rounded-xl ${stats.lowStockAlerts > 0 ? "border-red-900/40 bg-red-950/20" : "border-[#2a2a2e]/50 bg-[#1a1a1e]"}`}>
                <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-1 p-4">
                  <CardTitle className="text-xs font-semibold text-red-400 uppercase tracking-wider">재고 부족</CardTitle>
                  <div className="rounded-full p-1.5 bg-red-950/50 flex-shrink-0"><AlertTriangle className="h-3.5 w-3.5 text-red-400" /></div>
                </CardHeader>
                <CardContent className="p-4 pt-0">
                  <div className="text-2xl font-bold text-red-400">{stats.lowStockAlerts}</div>
                  <p className="text-[11px] text-red-400 mt-0.5 leading-tight">{getStockInsight()}</p>
                </CardContent>
              </Card>
            </Link>
            <Link href="/dashboard/purchases">
              <Card className="h-[124px] flex flex-col justify-between overflow-hidden cursor-pointer transition-all hover:shadow-md border-[#2a2a2e]/50 shadow-sm rounded-xl bg-[#1a1a1e]">
                <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-1 p-4">
                  ...
                  <div className="rounded-full p-1.5 bg-emerald-900/20 flex-shrink-0"><DollarSign className="h-3.5 w-3.5 text-emerald-400" /></div>
                </CardHeader>
                <CardContent className="p-4 pt-0">
            ...
            <Link href="/dashboard/quotes?status=PENDING">
              <Card className="h-[124px] flex flex-col justify-between overflow-hidden cursor-pointer transition-all hover:shadow-md border-[#2a2a2e]/50 shadow-sm rounded-xl bg-[#1a1a1e]">
                <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-1 p-4">
                  ...
                  <div className="rounded-full p-1.5 bg-violet-950/50 flex-shrink-0"><FileText className="h-3.5 w-3.5 text-violet-400" /></div>
                </CardHeader>
                <CardContent className="p-4 pt-0">
                  <div className="text-2xl font-bold text-slate-200">{stats.activeQuotes}</div>
          ...
          <Card className="overflow-hidden bg-[#1a1a1e] border-[#2a2a2e]/50 shadow-sm rounded-xl">
                ...
                <div className="space-y-0 divide-y divide-[#2a2a2e]/50">
                  {stats.recentPurchases.slice(0, 5).map((p, i) => (
                    <div key={p.id || `p-${i}`} className="flex items-center gap-3 py-2.5 first:pt-1">
                      <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-md bg-blue-950/50 text-blue-400">
        ...
        <div className="md:col-span-2 space-y-4">
          ...
          <Card className="bg-[#1a1a1e] border-[#2a2a2e]/50 shadow-sm rounded-xl">
            <CardHeader className="pb-2 p-4">
              <CardTitle className="text-sm font-semibold text-slate-100">업무 바로가기</CardTitle>
              <p className="text-[11px] text-slate-500 mt-0.5">검색 → 비교 → 견적 → 재고 등록</p>
            </CardHeader>
            <CardContent className="p-4 pt-0">
              {/* 메인 액션 흐름 */}
              <div className="space-y-0.5">
                <Link href="/app/search" className="flex items-center gap-2.5 px-2 py-2.5 rounded-lg hover:bg-blue-950/30 transition-colors group">
                  ...
                  <div className="flex-1 min-w-0">
                    <span className="block text-sm font-semibold text-slate-200 group-hover:text-blue-400">시약·장비 검색</span>
                    <span className="block text-[11px] text-slate-500">500만+ 품목 검색</span>
                  </div>
                  <ChevronRight className="h-3.5 w-3.5 text-slate-300 flex-shrink-0 group-hover:text-blue-400 transition-colors" />
                </Link>
                <Link href="/app/compare" className="flex items-center gap-2.5 px-2 py-2.5 rounded-lg hover:bg-indigo-950/30 transition-colors group">
                  <div className="flex h-7 w-7 items-center justify-center rounded-md bg-indigo-950/50 flex-shrink-0">
                    <GitCompare className="h-3.5 w-3.5 text-indigo-400" />
                  </div>
                  <div className="flex-1 min-w-0">
                    <span className="block text-sm font-semibold text-slate-200 group-hover:text-indigo-400">제품 비교</span>
                    <span className="block text-[11px] text-slate-500">스펙·가격 비교 목록</span>
                  </div>
                  <ChevronRight className="h-3.5 w-3.5 text-slate-300 flex-shrink-0 group-hover:text-indigo-400 transition-colors" />
                </Link>
                <Link href="/app/quote" className="flex items-center gap-2.5 px-2 py-2.5 rounded-lg hover:bg-violet-950/30 transition-colors group">
                  <div className="flex h-7 w-7 items-center justify-center rounded-md bg-violet-950/50 flex-shrink-0">
                    <TrendingUp className="h-3.5 w-3.5 text-violet-400" />
                  </div>
                  <div className="flex-1 min-w-0">
                    <span className="block text-sm font-semibold text-slate-200 group-hover:text-violet-400">견적 요청하기</span>
                    <span className="block text-[11px] text-slate-500">공급사에 견적 발송</span>
                  </div>
                  <ChevronRight className="h-3.5 w-3.5 text-slate-300 flex-shrink-0 group-hover:text-violet-400 transition-colors" />
                </Link>
                <Link href="/dashboard/inventory" className="flex items-center gap-2.5 px-2 py-2.5 rounded-lg hover:bg-emerald-950/30 transition-colors group">
                  <div className="flex h-7 w-7 items-center justify-center rounded-md bg-emerald-950/50 flex-shrink-0">
                    <Plus className="h-3.5 w-3.5 text-emerald-400" />
                  </div>
                  <div className="flex-1 min-w-0">
                    <span className="block text-sm font-semibold text-slate-200 group-hover:text-emerald-400">재고 등록</span>
                    <span className="block text-[11px] text-slate-500">입고 품목 등록 및 관리</span>
                  </div>
                  <ChevronRight className="h-3.5 w-3.5 text-slate-300 flex-shrink-0 group-hover:text-emerald-400 transition-colors" />
                </Link>
                <Link href="/dashboard/inventory" className="flex items-center gap-2.5 px-2 py-2.5 rounded-lg hover:bg-[#222226]/30 transition-colors group">
                  <div className="flex h-7 w-7 items-center justify-center rounded-md bg-slate-800/50 flex-shrink-0">
                    <TrendingDown className="h-3.5 w-3.5 text-slate-400" />
                  </div>
                  <div className="flex-1 min-w-0">
                    <span className="block text-sm font-semibold text-slate-200 group-hover:text-slate-100">재고 차감</span>
                    <span className="block text-[11px] text-slate-500">출고·사용 기록</span>
                  </div>
                  <ChevronRight className="h-3.5 w-3.5 text-slate-300 flex-shrink-0 group-hover:text-slate-500 transition-colors" />
                </Link>
              </div>
              {/* 보조 액션 */}
              <div className="mt-3 pt-3 border-t border-[#2a2a2e]/50 space-y-0.5">
                ...
                <Link href="/dashboard/inventory" className="flex items-center gap-2.5 px-2 py-2 rounded-lg hover:bg-[#222226]/40 transition-colors group">
                  ...
                  <span className="text-sm text-slate-400 group-hover:text-slate-100">재고 관리</span>
          ...
          <Card className="bg-[#1a1a1e] border-[#2a2a2e]/50 shadow-sm rounded-xl">
            ...
            <CardContent className="space-y-1.5 pt-0">
              {notifications.map((n) => (
                <Link key={n.id} href={n.href} className={`flex items-start gap-2.5 p-2.5 rounded-lg transition-colors group ${n.unread ? "bg-blue-950/30 hover:bg-blue-950/50" : "hover:bg-[#222226]"}`}>
                  {renderNotificationIcon(n.type)}
          ...
          <Card className="bg-[#1a1a1e] border-[#2a2a2e]/50 shadow-sm rounded-xl">
            ...
            <CardContent className="p-4 pt-0 space-y-2">
              ...
              <Link href="/dashboard/quotes?status=RESPONDED" className="flex items-center gap-3 p-2.5 rounded-lg border border-green-900/40 bg-green-950/20 hover:bg-green-950/40 transition-colors group">
                <div className="rounded-md bg-green-950/60 p-1.5 flex-shrink-0">
              ...
              {/* 응답 대기 */}
              <Link href="/dashboard/quotes?status=PENDING" className="flex items-center gap-3 p-2.5 rounded-lg border border-slate-700/40 hover:bg-[#222226]/30 transition-colors group">
              ...
              {/* 지출 분석 바로가기 */}
              <Link href="/dashboard/analytics" className="flex items-center gap-2.5 px-2.5 py-2 rounded-lg hover:bg-blue-950/20 transition-colors group">
          ...
          {stats.lowStockItems.length > 0 && (
            <Card className="bg-[#1a1a1e] border-red-900/50 shadow-sm rounded-xl">
              ...
              <CardContent className="p-4 pt-0 space-y-1.5">
                {stats.lowStockItems.slice(0, 3).map((item) => (
                  <div key={item.id} className="flex items-center justify-between py-1.5 border-b border-[#2a2a2e]/30 last:border-0">
                    <div className="flex-1 min-w-0">
