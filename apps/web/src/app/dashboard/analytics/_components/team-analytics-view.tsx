...
function getStatusStyle(status: TeamSpendingData["status"]) {
  switch (status) {
    case "안정":
      return { bg: "bg-emerald-950/20", text: "text-emerald-400", border: "border-emerald-900/40" };
    case "주의":
      return { bg: "bg-amber-950/20", text: "text-amber-400", border: "border-amber-900/40" };
    case "초과 위험":
      return { bg: "bg-red-950/20", text: "text-red-400", border: "border-red-900/40" };
    case "검토 필요":
    ...
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        ...
        <div className={`rounded-xl border p-4 shadow-none ${riskTeams > 0 ? "border-amber-200/60 bg-amber-950/10 border-amber-900/30" : "border-slate-800/60 bg-[#161d2f] border-slate-800/50"}`}>
          ...
          <div className={`text-xl font-bold ${riskTeams > 0 ? "text-amber-400" : "text-slate-100"}`}>
            {riskTeams}개
          </div>
      ...
      {riskTeamList.length > 0 && (
        <Card className="rounded-xl border-amber-900/30 bg-amber-950/10 shadow-none">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        {/* 팀별 사용액 비교 */}
        <Card className="rounded-xl border-slate-800/50 shadow-none bg-[#161d2f]">
          <CardHeader className="pb-2 p-4">
            <CardTitle className="text-sm font-semibold text-slate-200">팀별 사용액 비교</CardTitle>
            <p className="text-[11px] text-slate-500 mt-0.5">배정 예산 대비 현재 사용액</p>
          </CardHeader>
          <CardContent className="p-4 pt-0">
        ...
        <Card className="rounded-xl border-slate-800/50 shadow-none bg-[#161d2f]">
          <CardHeader className="pb-2 p-4">
            ...
            <p className="text-[11px] text-slate-500 mt-0.5">소진율 기준 내림차순 정렬</p>
          </CardHeader>
          <CardContent className="p-4 pt-0">
            <ResponsiveContainer width="100%" height={240}>
            ...
            <Table className="min-w-[800px]">
              ...
              <TableBody>
                {TEAM_DATA.map((team) => {
                  ...
                  return (
                    <TableRow key={team.id} className="border-slate-800/30 hover:bg-slate-800/20 transition-colors">
                      ...
                      <TableCell className="py-2.5 text-center">
                        <div className="flex flex-col items-center gap-1">
                          <span className={`text-xs font-semibold ${team.burnRate >= 90 ? "text-red-400" : team.burnRate >= 75 ? "text-amber-400" : "text-slate-400"}`}>
                            {team.burnRate}%
                          </span>
                      ...
                      <TableCell className="py-2.5 text-center">
                        <div className="flex items-center justify-center gap-1">
                          ...
                          <span className={`text-xs font-medium ${team.monthChange > 0 ? "text-rose-400" : team.monthChange < 0 ? "text-emerald-400" : "text-slate-400"}`}>
