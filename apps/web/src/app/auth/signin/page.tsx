...
function SignInContent() {
  ...
  // §auth-callback-guard — 로그인 연타 차단(B). 매 클릭이 state/PKCE 쿠키를 덮어쓰면
  //   먼저 떠난 Google 콜백의 state ≠ 현재 쿠키 → CallbackRouteError(화면 error=Configuration).
  //   제출 중 disable 로 단일 OAuth 개시만 허용(dead button 아님 — 진행중 라벨 노출).
        ...
        <div className="flex-1 flex items-center justify-center px-6 sm:px-12 lg:px-16">
          <div className="w-full max-w-[376px] -translate-y-4">
            ...
            <div className="bg-white rounded-[24px] border border-slate-200/70 p-8 shadow-[0_10px_30px_rgba(15,23,42,0.06)]">
              ...
              {/* Google — first action */}
              <Button
                className="w-full font-semibold text-[15px] rounded-xl transition-all bg-slate-900 text-white hover:bg-slate-800 hover:shadow-lg mb-6 disabled:opacity-60 disabled:cursor-not-allowed"
                style={{ height: 50 }}
                disabled={submitting}
                onClick={() => {
              ...
              >
                {submitting ? (
                  <span className="mr-3 h-5 w-5 shrink-0 inline-block animate-spin rounded-full border-2 border-white/40 border-t-white" />
                ) : (
