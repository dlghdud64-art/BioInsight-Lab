...
export async function GET(
  ...
  try {
    ...
    // #api-inventory-inspection-info-leak — isOwner OR isOrgMember 분기 (GET).
    //   기존 `userId !== session.user.id && !organizationId` 는 organizationId
    //   있는 row 를 어떤 user 든 통과시켜 multi-tenant info leak 위험 (C cluster
    //   sweep #2). vendor-requests cluster 패턴 정확히 mirror.
    {
...
export async function POST(
  ...
  try {
    ...
    // #api-inventory-inspection-info-leak — isOwner OR isOrgMember 분기 (POST).
    {
      const isOwner = inventory.userId === session.user.id;
      let isOrgMember = false;
      if (!isOwner && inventory.organizationId) {
