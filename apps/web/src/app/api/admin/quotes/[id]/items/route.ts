...
export async function PATCH(
  ...
  try {
    ...
    const quote = await db.quote.findUnique({
      where: { id: quoteId },
      include: { items: true },   // §drift D1 — Quote 의 QuoteListItem 관계명은 items
    });
      ...
      items.map((item: any) =>
        db.quoteListItem.update({
          ...
          data: {
            ...
            raw: {
              ...(quote.items.find((li: { id: string; raw?: any }) => li.id === item.id)?.raw as object || {}),
              costPrice: item.costPrice,
              adminNotes: item.adminNotes,
            },
            lineTotal: item.unitPrice
              ? item.unitPrice *
                (quote.items.find((li: { id: string; quantity?: number | null }) => li.id === item.id)?.quantity || 1)
              : null,
          },
        })
      )
