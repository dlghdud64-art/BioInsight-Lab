...
export async function GET(request: NextRequest) {
  try {
    ...
    const { getQuotesByUser } = await import("@/lib/api/quotes");
    let quotes = await getQuotesByUser(session.user.id);
    ...
    if (sortBy === "newest") {
      ...
      quotes.sort((a, b) => new Date(a.createdAt).getTime() - new Date(b.createdAt).getTime());
    } else if (sortBy === "status") {
      const statusOrder = ["PENDING", "SENT", "RESPONDED", "COMPLETED", "CANCELLED"];
      quotes.sort((a, b) => {
