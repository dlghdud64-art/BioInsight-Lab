    ...
    <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center" onClick={e => { if (e.target === e.currentTarget) onClose(); }}>
      <div className="bg-[#1e2024] border border-bd rounded-xl shadow-2xl w-full max-w-3xl max-h-[88vh] overflow-hidden flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4 space-y-4">
          ...
          <div>
            ...
            <div className="mt-2 space-y-1.5">
              {([
                ...
                return (
                  <div key={item.label} className={`flex items-center gap-3 px-3 py-2.5 rounded-md border ${item.status === "confirmed" ? "border-emerald-500/20 bg-emerald-600/[0.03]" : item.status === "not_available" ? "border-red-500/15 bg-red-600/[0.03]" : "border-bd/40 bg-[#252729]"}`}>
          ...
          {/* Receiving readiness */}
          <div className={`px-3 py-2.5 rounded-md border ${validation?.canOpenReceivingPreparationReentry ? "border-emerald-500/20 bg-emerald-600/[0.03]" : "border-bd/40 bg-[#252729]"}`}>
