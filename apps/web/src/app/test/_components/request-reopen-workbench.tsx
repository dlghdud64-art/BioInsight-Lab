...
import { X, Check, AlertTriangle, ArrowRight, ArrowLeft, FileText, Building2, RefreshCw, Pencil } from "lucide-react";
import { type RequestReopenState, type RequestReopenObject, createInitialRequestReopenState, buildRequestReopenVendorTargetPlan, buildRequestReopenLinePlan, validateRequestReopenBeforeRecord, buildRequestReopenObject, buildRequestSubmissionReopenHandoff } from "@/lib/ai/request-reopen-engine";
...
export function RequestReopenWorkbench({ open, onClose, handoff, onReopenRecorded, onRequestSubmissionReopenHandoff, onReturnToCompareReopen }: RequestReopenWorkbenchProps) {
  ...
  const simulateSetup = useCallback(() => {
    setReopenState(prev => prev ? { ...prev, reusedVendorTargetIds: prev.requestCandidateIds.slice(0, 1), addedVendorTargetIds: ["new_vendor_1"], reusedRequestLineIds: prev.requestCandidateIds, rewrittenRequestLineIds: [], requestConditionDeltaSummary: "수량 증가 + 긴급도 상향", missingDecisionCount: 0, substatus: "ready_for_request_submission_reopen" } : prev);
  }, []);
    ...
    <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center" onClick={e => { if (e.target === e.currentTarget) onClose(); }}>
      <div className="bg-[#1e2024] border border-bd rounded-xl shadow-2xl w-full max-w-3xl max-h-[88vh] overflow-hidden flex flex-col">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <div className="flex items-center gap-2 text-[10px] mt-0.5">
                <span className="text-slate-400">Vendor <span className="text-slate-200 font-medium">{(reopenState.reusedVendorTargetIds.length + reopenState.addedVendorTargetIds.length) || "—"}</span></span>
                <span className="text-slate-600">·</span>
                <span className="text-slate-400">Lines <span className="text-slate-200 font-medium">{(reopenState.reusedRequestLineIds.length + reopenState.rewrittenRequestLineIds.length) || "—"}</span></span>
                <span className="text-slate-600">·</span>
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4 space-y-4">
          ...
          {reopenState.reusedVendorTargetIds.length === 0 && reopenState.addedVendorTargetIds.length === 0 && (
            <Button size="sm" variant="ghost" className="w-full h-7 text-[9px] text-blue-400 hover:text-blue-300 border border-blue-500/20" onClick={simulateSetup}>Vendor + Line 설정 시뮬레이션</Button>
          )}
        ...
        <div className="px-5 py-3 border-t border-bd bg-[#1a1c1f]">
          <div className="flex items-center gap-3 text-[10px] mb-2.5">
            <span className="text-slate-500">Vendor <span className="text-slate-300 font-medium">{reopenState.reusedVendorTargetIds.length + reopenState.addedVendorTargetIds.length}</span></span>
            <span className="text-slate-600">·</span>
            <span className="text-slate-500">Lines <span className="text-slate-300 font-medium">{reopenState.reusedRequestLineIds.length + reopenState.rewrittenRequestLineIds.length}</span></span>
            <span className="text-slate-600">·</span>
