...
export function HeroSection() {
  return (
    <section className="border-b border-slate-800 bg-gradient-to-b from-white via-slate-50/30 to-white py-8 md:py-16 lg:py-20">
      <div className="mx-auto max-w-6xl px-4 md:px-6 grid grid-cols-1 gap-4 md:gap-8 lg:grid-cols-[minmax(0,1fr)_minmax(0,1.2fr)] lg:items-center">
        ...
        <div className="space-y-3 md:space-y-5">
          <div className="space-y-2.5 md:space-y-3">
            <h1 className="text-2xl md:text-4xl lg:text-5xl font-bold leading-tight tracking-tight text-slate-100">
              검색·비교·견적 요청을
          ...
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2.5 md:gap-3">
            <Link href="/test/search" className="flex-1 sm:flex-initial">
              <Button className="w-full sm:w-auto bg-indigo-600 text-white hover:bg-indigo-700 shadow-none hover:shadow-xl transition-all text-sm md:text-base px-5 md:px-6 py-5 md:py-6 h-auto min-h-[44px]">
                검색 시작하기
            ...
            <Link href="/test/quote" className="flex-1 sm:flex-initial">
              <Button
                variant="outline"
                className="w-full sm:w-auto border-2 border-slate-700 bg-slate-900 text-slate-300 hover:bg-slate-900 hover:border-slate-400 text-sm md:text-base px-5 md:px-6 py-5 md:py-6 h-auto min-h-[44px]"
              >
                견적 요청 리스트
              </Button>
            </Link>
