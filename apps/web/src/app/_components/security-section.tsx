import { Card, CardContent } from "@/components/ui/card";
import { Shield, Lock, Server } from "lucide-react";

// SecuritySection 컴포넌트 - 연구/구매 워크벤치 스타일
export function SecuritySection() {
  return (
    <section id="security" className="py-8 md:py-10 border-b border-slate-800 bg-slate-900">
      <div className="mx-auto max-w-6xl px-4 md:px-6">
        <h2 className="text-base md:text-lg font-semibold tracking-tight text-slate-100 mb-3">
          보안
        </h2>
        <div className="flex overflow-x-auto snap-x snap-mandatory gap-4 pb-4 scrollbar-hide md:grid md:grid-cols-3 md:gap-6 md:overflow-visible md:pb-0 -mx-4 px-4 md:mx-0 md:px-0">
          <Card className="bg-slate-900 border border-slate-800 rounded-xl shadow-none hover:shadow-none hover:-translate-y-1 transition-all duration-200 min-w-[85%] snap-center shrink-0 md:min-w-0 md:shrink">
            <CardContent className="flex items-start gap-2.5 p-6">
              <div className="flex h-7 w-7 items-center justify-center rounded-md bg-emerald-900/40 flex-shrink-0">
                <Shield className="h-3.5 w-3.5 text-emerald-400" strokeWidth={1.5} />
              </div>
              <div className="space-y-0.5 min-w-0 flex-1">
                <h3 className="text-sm font-semibold text-slate-100">Paste-only 모드</h3>
                <p className="text-xs leading-snug text-slate-400">
                  프로토콜/데이터시트는 파일로만 업로드하여 시스템에만 전송합니다.
                </p>
              </div>
            </CardContent>
          </Card>

          <Card className="bg-slate-900 border border-slate-800 rounded-xl shadow-none hover:shadow-none hover:-translate-y-1 transition-all duration-200 min-w-[85%] snap-center shrink-0 md:min-w-0 md:shrink">
            <CardContent className="flex items-start gap-2.5 p-6">
              <div className="flex h-7 w-7 items-center justify-center rounded-md bg-slate-800 flex-shrink-0">
                <Lock className="h-3.5 w-3.5 text-slate-300" strokeWidth={1.5} />
              </div>
              <div className="space-y-0.5 min-w-0 flex-1">
                <h3 className="text-sm font-semibold text-slate-100">공유 링크 보안</h3>
                <p className="text-xs leading-snug text-slate-400">
                  암호화 기반 공유 링크. 검색엔진 노출 방지, 만료/비활성화 지원.
                </p>
              </div>
            </CardContent>
          </Card>

          <Card className="bg-slate-900 border border-slate-800 rounded-xl shadow-none hover:shadow-none hover:-translate-y-1 transition-all duration-200 min-w-[85%] snap-center shrink-0 md:min-w-0 md:shrink">
            <CardContent className="flex items-start gap-2.5 p-6">
              <div className="flex h-7 w-7 items-center justify-center rounded-md bg-indigo-100 flex-shrink-0">
                <Server className="h-3.5 w-3.5 text-indigo-400" strokeWidth={1.5} />
              </div>
              <div className="space-y-0.5 min-w-0 flex-1">
                <h3 className="text-sm font-semibold text-slate-100">프라이빗 배포</h3>
                <p className="text-xs leading-snug text-slate-400">
                  Enterprise 플랜에서 자체 서버 배포 옵션을 제공합니다.
                </p>
              </div>
            </CardContent>
          </Card>
        </div>
      </div>
    </section>
  );
}
