...
export function DashboardSidebar({ isMobileOpen: externalIsMobileOpen, onMobileOpenChange }: DashboardSidebarProps = {}) {
  ...
  const SidebarContent = () => (
    <div className="h-full flex flex-col">
      ...
      <div className="flex-1 overflow-y-auto p-3 md:p-4 pt-8 md:pt-8">
        ...
        {/* 데스크톱 헤더 */}
        <h2 className="hidden lg:block text-xs lg:text-sm font-semibold text-slate-900 dark:text-slate-100 mb-6">메뉴</h2>
        ...
        <div className="space-y-6">
          {sidebarGroups.map((group, groupIndex) => (
            <div key={groupIndex}>
              <h3 className="text-xs font-bold text-gray-400 dark:text-slate-500 uppercase tracking-wider mb-2 px-2 md:px-3 mt-6 first:mt-0">
                {group.label}
              </h3>
              <nav className="space-y-1">
                {group.items.map((item) => {
                  ...
                  return (
                    ...
                    >
                      <Icon className={cn("h-5 w-5 flex-shrink-0", isActive ? "text-blue-600 dark:text-blue-400" : isInventory ? "text-blue-500 dark:text-blue-400" : "text-slate-500 dark:text-slate-400")} />
                      <span className="truncate whitespace-nowrap">{item.title}</span>
                      {item.badge && (
      ...
      {/* 하단 고정 영역 (서비스 홈으로) */}
      <div className="px-3 md:px-4 py-3 md:py-4 border-t border-slate-200 dark:border-slate-800 flex-shrink-0 mt-auto">
        <Link 
          href="/" 
          onClick={() => setIsMobileOpen(false)}
          className="flex items-center gap-3 rounded-lg px-2 md:px-3 py-2 text-xs md:text-sm font-medium text-slate-500 dark:text-slate-400 transition-all hover:text-blue-600 dark:hover:text-white hover:bg-blue-50 dark:hover:bg-slate-800"
        >
          <House className="h-5 w-5 flex-shrink-0 text-slate-500 dark:text-slate-400" />
  ...
  return (
    <>
      ...
      <aside className="hidden lg:block w-64 min-w-[16rem] min-h-screen bg-white dark:bg-slate-950 border-r border-slate-200 dark:border-slate-800 shrink-0 z-30">
        <SidebarContent />
      </aside>
      ...
      <aside
        className={cn(
          "fixed top-0 left-0 h-full w-64 min-w-[16rem] bg-white dark:bg-slate-950 border-r border-slate-200 dark:border-slate-800 z-50 mobile-sidebar transition-transform duration-300 lg:hidden shrink-0",
