      ...
      <div className="max-w-5xl mx-auto px-4 md:px-6">
        ...
        <div className="hidden md:block">
          <div className="relative">
            {/* Connection line */}
            <div className="absolute top-6 left-6 right-6 h-px bg-slate-200" />
              ...
              {FLOW_STAGES.map((stage, idx) => {
                ...
                return (
                  <div key={stage.title} className="relative flex flex-col items-center text-center px-2">
                    {/* Node */}
                    <div className="relative z-10 w-12 h-12 rounded-md border border-slate-200 bg-white flex items-center justify-center mb-3">
                      <Icon className="h-5 w-5 text-slate-700" strokeWidth={1.8} />
                    </div>
                    <span className="text-xs font-semibold text-slate-800 mb-0.5">{stage.title}</span>
                    <span className="text-[10px] text-slate-500 leading-tight mb-1">{stage.desc}</span>
                    <span className="text-[9px] text-slate-400 leading-tight">{stage.detail}</span>
                  </div>
                );
              })}
            </div>
        ...
        <div className="md:hidden space-y-0 border rounded-md overflow-hidden">
          {FLOW_STAGES.map((stage, idx) => {
            const Icon = stage.icon;
            return (
              <div key={stage.title} className="flex items-center gap-3 px-3 py-2.5 border-b last:border-b-0 bg-white">
                <div className="flex-shrink-0 w-8 h-8 rounded-md border border-slate-200 bg-slate-50 flex items-center justify-center">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[11px] text-slate-500 leading-tight">{stage.desc}</p>
                </div>
        ...
        <div className="mt-10 md:mt-14 border rounded-md bg-slate-50/50 px-4 py-3">
              ...
              return (
                <div key={cap.label} className="flex items-center gap-2.5">
                  ...
                  <div>
                    ...
                    <span className="text-[10px] text-slate-400 ml-1.5">{cap.desc}</span>
                  </div>
                </div>
              );
            })}
