...
// 카드 surface 통일 — signal은 아이콘만
const CARD_BG = "#1A2232";
const CARD_BORDER = "#252F3E";
const CARD_BORDER_HOVER = "#333F52";
...
// 시스템 증거 — 단일 neutral 톤, 아이콘만 미세 구분
const PROOF_ICON_COLOR = "#8A99AF";
const SYSTEM_PROOFS = [
    ...
    <section className="py-20 md:py-28" style={{ backgroundColor: "#141A24", borderBottom: "1px solid #222C3A" }}>
      <div className="max-w-[1240px] mx-auto px-4 md:px-6">
          ...
          {CONSOLE_LAYERS.map((layer) => {
            ...
            return (
              ...
              >
                ...
                <p className="text-[12px] leading-relaxed mb-1.5" style={{ color: "#B0BCCC" }}>{layer.purpose}</p>
                {layer.support && (
                  <p className="text-[11px] leading-relaxed mb-2" style={{ color: "#8A99AF" }}>{layer.support}</p>
                )}
                <p className="text-[11px] font-semibold" style={{ color: "#7A8A9E" }}>{layer.items}</p>
              </div>
            );
          })}
        </div>
        ...
        <div className="rounded-lg px-5 py-5" style={{ backgroundColor: CARD_BG, border: `1px solid ${CARD_BORDER}` }}>
          <p className="text-[11px] font-bold uppercase tracking-widest mb-3.5" style={{ color: "#8A99AF" }}>
            System Evidence
          </p>
            ...
            {SYSTEM_PROOFS.map((proof) => {
              ...
              return (
                <div key={proof.text} className="flex items-start gap-2.5 py-1.5">
                  ...
                  <span className="text-[13px] leading-relaxed" style={{ color: "#B0BCCC" }}>{proof.text}</span>
                </div>
              );
            })}
          </div>
