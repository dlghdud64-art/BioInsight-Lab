...
export default function PricingPage() {
    ...
    if (isAnnual && plan.priceAnnualPerMonth != null) {
      return (
        ...
        >
          ...
          <span className="inline-flex items-center rounded-md bg-green-900/20 px-1.5 md:px-2.5 py-0.5 md:py-1 text-[10px] md:text-xs font-semibold text-green-700 ring-1 ring-green-600/10">
            10% 할인
          </span>
        </span>
      );
  ...
  return (
    <MainLayout>
          ...
          <div className="container mx-auto px-4 py-6 md:py-16">
            <div className="max-w-7xl mx-auto">
              ...
              <div className="flex items-center justify-center mb-4 md:mb-12">
                <div className="bg-slate-800 rounded-full p-0.5 md:p-1 inline-flex">
                  <button
                    type="button"
                    ...
                    className={cn(
                      ...
                      !isAnnual
                        ? "bg-slate-900 shadow-none text-slate-100"
                  ...
                  >
                    연간
                    <span className="bg-slate-200 text-slate-400 text-[10px] md:text-xs px-1.5 md:px-2 py-0.5 rounded-full font-medium">
                      -10%
                    </span>
                  </button>
                </div>
                  ...
                  return (
                    <Card
                      ...
                      className={cn(
                        ...
                        isSelected && "ring-2 ring-blue-500 ring-offset-2 bg-blue-950/20/50 border-blue-300",
                        !isSelected && "hover:border-slate-700 hover:bg-slate-900/50",
                        isRecommended && !isSelected && "border-2 border-blue-800 shadow-2xl md:scale-105 z-10",
                    ...
                    >
                      ...
                      {plan.badge && (
                        <div className="absolute -top-3 md:-top-4 left-1/2 -translate-x-1/2 z-10">
                          <Badge className="bg-blue-600 text-white px-3 md:px-4 py-1 md:py-1.5 text-[10px] md:text-xs font-bold shadow-none whitespace-nowrap">
                            {plan.badge}
                          </Badge>
                        </div>
                      )}
                        ...
                        <div className="hidden md:flex justify-center mb-3">
                          <div
                            className={cn(
                              ...
                              isSelected || isRecommended ? "bg-blue-900/30" : "bg-slate-800"
                            )}
                          >
                            <Icon
                              className={cn(
                                "h-6 w-6",
                                isSelected || isRecommended ? "text-blue-400" : "text-slate-500"
                              )}
                            />
                          </div>
                        </div>
                      ...
                      <CardContent className="flex-1 flex flex-col pt-0 px-2.5 md:px-6 pb-3 md:pb-6" onClick={(e) => e.stopPropagation()}>
                        ...
                        {hasMore && (
                          <button
                            type="button"
                            className="md:hidden flex items-center justify-center gap-1 text-[11px] text-slate-400 hover:text-slate-400 mb-3 transition-colors"
                            onClick={(e) => {
                              e.stopPropagation();
                        ...
                        {isSelected ? (
                          ...
                          <Button
                            variant="outline"
                            className="w-full border-slate-800 hover:bg-slate-900 text-slate-500 hover:text-slate-200 h-9 md:h-10 text-sm"
                            onClick={(e) => {
                              e.stopPropagation();
                              handleCardSelect(plan.id);
                            }}
              ...
              <div className="mb-8">
                ...
                <div className="md:hidden overflow-x-auto -mx-4 px-4">
                  <table className="w-full text-[11px]">
                    <thead>
                      <tr className="border-b border-slate-800 sticky top-0 bg-slate-900">
                        ...
                        <th className="text-center py-2 px-1 font-semibold text-blue-700 bg-blue-950/20/50 w-[15%]">Biz</th>
                    ...
                    <tbody>
                      {comparisonFeatures.map((item, index) => {
                        if ("isCategoryHeader" in item && item.isCategoryHeader) {
                          return (
                            <tr key={`mcat-${index}`} className={cn(
                              "border-t",
                              item.tier === "business" ? "bg-blue-950/20/30" : item.tier === "enterprise" ? "bg-slate-900" : "bg-slate-900/50"
                            )}>
                              <td colSpan={5} className="py-1.5 px-1">
                                <div className="flex items-center gap-1">
                                  <span className={cn(
                                    "text-[10px] font-bold uppercase tracking-wider",
                                    item.tier === "business" ? "text-blue-400" : item.tier === "enterprise" ? "text-slate-500" : "text-slate-400"
                                  )}>{item.label}</span>
                                  {item.tier === "business" && <span className="text-[8px] font-semibold text-blue-400 bg-blue-900/30 px-1 rounded">Biz+</span>}
                        ...
                        const mCell = (val: boolean | string, highlight?: boolean) => (
                          <td className={cn("text-center py-1.5 px-1", highlight && "bg-blue-950/20/20")}>
                            {typeof val === "boolean" ? (val ? <Check className="inline h-3 w-3 text-green-400" /> : <span className="text-slate-300">—</span>) : <span className="font-medium text-slate-300">{val}</span>}
                          </td>
                        );
                        return (
                          <tr key={`mrow-${index}`} className="border-b border-slate-800">
                      ...
                      <Table>
                        <TableHeader>
                          <TableRow className="bg-slate-900 border-b border-slate-800">
                            ...
                            <TableHead className="text-center font-semibold py-3 bg-blue-950/20 text-blue-800">Business</TableHead>
                        ...
                        <TableBody>
                          {comparisonFeatures.map((item, index) => {
                              ...
                              return (
                                <TableRow
                                  ...
                                  className={cn(
                                    ...
                                    isBusinessTier ? "bg-blue-950/20/50" : isEnterpriseTier ? "bg-slate-800/70" : "bg-slate-900/50"
                                  )}
                                >
                                  <TableCell colSpan={5} className="py-2 pl-5">
                                    <div className="flex items-center gap-2">
                                      <span className={cn("text-[11px] font-bold uppercase tracking-widest", isBusinessTier ? "text-blue-400" : isEnterpriseTier ? "text-slate-500" : "text-slate-400")}>
                                        {item.label}
                                      </span>
                                      {isBusinessTier && <span className="text-[10px] font-semibold text-blue-400 bg-blue-900/30 border border-blue-800 px-1.5 py-0.5 rounded-full">Business+</span>}
                                      {isEnterpriseTier && <span className="text-[10px] font-semibold text-slate-500 bg-slate-200 border border-slate-700 px-1.5 py-0.5 rounded-full">Enterprise</span>}
                                    </div>
                                  </TableCell>
                                </TableRow>
                              );
                            ...
                            const renderCell = (value: boolean | string) =>
                              typeof value === "boolean" ? (value ? <Check className="h-4 w-4 text-green-400 mx-auto" /> : <span className="text-slate-300 text-lg leading-none">—</span>) : <span className="text-sm font-medium text-slate-300">{value}</span>;
                            return (
                              <TableRow key={index} className="hover:bg-slate-900/40 transition-colors">
                                ...
                                <TableCell className="text-center py-2.5 bg-blue-950/20/20">{renderCell(dataItem.business)}</TableCell>
      ...
      <div className="fixed bottom-0 left-0 right-0 z-40 bg-slate-900/95 border-t border-slate-800 shadow-[0_-4px_12px_rgba(15,23,42,0.08)]">
        {/* ── 모바일 바 ── */}
        <div className="md:hidden flex items-center justify-between px-3 py-2 gap-2">
          <div className="flex items-center gap-2 min-w-0">
            ...
            {isAnnual && <Badge variant="secondary" className="text-[9px] px-1.5 py-0 bg-green-900/20 text-green-700 shrink-0">연간</Badge>}
          </div>
          <div className="flex items-center gap-1.5 shrink-0">
            ...
            <Button
              size="sm"
              variant="outline"
              className="h-8 px-3 text-xs border-slate-800"
        ...
        <div className="hidden md:flex mx-auto max-w-[90rem] px-8 py-4 items-center justify-between gap-x-12">
          ...
          <div className="flex items-center shrink-0">
            <Button
              className="px-8 h-11 text-base font-semibold bg-blue-600 hover:bg-blue-700 whitespace-nowrap shadow-none"
              onClick={() => (window.location.href = "/support")}
            >
              요금 & 도입 문의
            </Button>
