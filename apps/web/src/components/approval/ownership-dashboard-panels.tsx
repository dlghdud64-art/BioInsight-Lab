        ...
        {sorted.slice(0, 8).map((owner) => (
          ...
          >
            ...
            <div className="flex gap-3 text-[10px]">
              ...
              {owner.escalationCount > 0 && <span className="text-yellow-400">에스컬 {owner.escalationCount}</span>}
...
export function OverloadedOwnerPanel({ data, onRebalanceClick, className }: OverloadedOwnerPanelProps) {
  ...
  return (
    <div className={cn("rounded border border-yellow-500/20 bg-yellow-500/5 p-4 space-y-2", className)}>
      <div className="flex items-center justify-between">
        <h3 className="text-xs font-medium text-yellow-300">과부하 담당자</h3>
        <span className="text-xs tabular-nums text-yellow-400">{data.totalOverloaded}명</span>
      </div>

      <div className="space-y-1.5">
        {data.recommendedRebalance.map((rec) => (
          <div key={rec.fromOwnerId} className="rounded bg-yellow-500/5 p-2 space-y-1">
            <div className="flex items-center justify-between">
              <span className="text-xs text-slate-700">{rec.fromOwnerName}</span>
              {rec.excessCount > 0 && (
                <span className="text-[10px] text-yellow-400">+{rec.excessCount}건 초과</span>
              )}
            </div>
            <p className="text-[10px] text-yellow-400/70">{rec.suggestedAction}</p>
            {onRebalanceClick && (
              <button
                onClick={() => onRebalanceClick(rec.fromOwnerId)}
                className="text-[10px] text-yellow-300 hover:text-yellow-200 underline"
              >
                재배정 검토
              </button>
            )}
...
const HEALTH_CONFIG: Record<string, { color: string; label: string; border: string }> = {
  ...
  attention: { color: "text-yellow-400", label: "주의", border: "border-yellow-500/20" },
    ...
    <div className={cn("rounded border bg-slate-900/50 p-3 space-y-2", health.border, className)}>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 text-xs">
        <div>
          ...
          <p className={cn("text-sm font-semibold tabular-nums", coverage.coverageRate >= 90 ? "text-emerald-400" : coverage.coverageRate >= 70 ? "text-yellow-400" : "text-red-400")}>
        ...
        <div>
          ...
          <p className={cn("text-sm font-semibold tabular-nums", coverage.overloadedCount === 0 ? "text-slate-400" : "text-yellow-400")}>
