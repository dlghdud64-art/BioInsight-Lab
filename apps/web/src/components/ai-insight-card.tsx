        ...
        <div className="px-3 pb-3 md:px-5 md:pb-4 pt-1 space-y-2.5 border-t border-indigo-200">
          ...
          <div className="flex flex-wrap items-center gap-1.5 pt-2">
            ...
            {getComparisonCriteria().slice(0, 4).map((c, i) => (
              <Badge key={i} variant="outline" className="text-[10px] bg-pn/60 text-indigo-700 border-indigo-200 px-2 py-0.5">
                {c}
              </Badge>
            ))}
          </div>
          ...
          <div className="flex flex-wrap gap-1.5">
            {getSuggestedQueries().map((sq, idx) => (
              <a
                ...
                className="text-[10px] px-2.5 py-1 rounded-full bg-pn/80 text-indigo-700 border border-indigo-200 hover:bg-indigo-100 transition-colors"
              >
                {sq}
              </a>
            ))}
