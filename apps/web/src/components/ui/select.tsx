...
>(({ className, children, position = "popper", ...props }, ref) => (
  <SelectPrimitive.Portal>
    <SelectPrimitive.Content
      ref={ref}
      className={cn(
        // §mobile-logs P4 (진단 ④) — 회색 채색 토큰 제거(bg-popover 흰 패널 복원) +
        //   패널 그림자 시안 토큰(0 12px 32px rgba(15,23,42,.14) — shadow-md 대체).
        "relative z-50 max-h-96 min-w-[8rem] overflow-hidden rounded-md border bg-popover text-popover-foreground shadow-[0_12px_32px_rgba(15,23,42,0.14)] data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 data-[side=bottom]:slide-in-from-top-2 data-[side=left]:slide-in-from-right-2 data-[side=right]:slide-in-from-left-2 data-[side=top]:slide-in-from-bottom-2 border-bd",
        position === "popper" &&
...
>(({ className, children, ...props }, ref) => (
  <SelectPrimitive.Item
    ...
    className={cn(
      ...
      // §mobile-logs P4 (진단 ④) — 항목 44px 터치 높이 + 선택(✓) 강조(#eff6ff/#1d4ed8) —
      //   전역(데스크톱 포함, 핸드오프 §3 명시). §11.73 hover/highlighted 톤 보존.
      "relative flex w-full min-h-[44px] cursor-pointer select-none items-center rounded-sm py-1.5 pl-8 pr-2 text-sm outline-none transition-colors duration-100 focus:bg-blue-50 focus:text-blue-700 data-[highlighted]:bg-blue-50 data-[highlighted]:text-blue-700 data-[state=checked]:bg-[#eff6ff] data-[state=checked]:text-[#1d4ed8] data-[state=checked]:font-semibold data-[disabled]:pointer-events-none data-[disabled]:opacity-50",
      className
    )}
    {...props}
  >
