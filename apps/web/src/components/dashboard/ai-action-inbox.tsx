...
const CARD_CONFIG: Record<string, CardConfig> = {
  QUOTE_DRAFT: {
    ...
    cta: "견적 요청 검토하기",
    badgeLabel: "즉시 확인 필요",
    badgeClass: "bg-red-950/40 text-red-400 border-red-800",
  ...
  STATUS_CHANGE_SUGGEST: {
    ...
    cta: "상태 변경안 검토하기",
    badgeLabel: "상태 변경 제안",
    badgeClass: "bg-purple-950/40 text-purple-400 border-purple-800",
  ...
  REORDER_SUGGESTION: {
    ...
    cta: "재발주 우선순위 보기",
    badgeLabel: "재고 위험",
    badgeClass: "bg-orange-950/40 text-orange-400 border-orange-800",
  ...
  EXPIRY_ALERT: {
    ...
    cta: "임박 품목 확인하기",
    badgeLabel: "오늘 처리 권장",
    badgeClass: "bg-yellow-950/40 text-yellow-400 border-yellow-800",
...
const STAGE_CONFIG: Record<string, { label: string; className: string }> = {
  QUOTE_DRAFT: {
    label: "검토 필요",
    className: "bg-orange-950/40 text-orange-400 border-orange-800",
  },
  VENDOR_EMAIL_DRAFT: {
    label: "검토 필요",
    className: "bg-orange-950/40 text-orange-400 border-orange-800",
  ...
  STATUS_CHANGE_SUGGEST: {
    label: "승인 필요",
    className: "bg-purple-950/40 text-purple-400 border-purple-800",
  },
  REORDER_SUGGESTION: {
    label: "조치 필요",
    className: "bg-red-950/40 text-red-400 border-red-800",
  },
  EXPIRY_ALERT: {
    label: "확인 필요",
      ...
      <div className="rounded-xl border border-slate-800/50 bg-[#161d2f] shadow-none overflow-hidden">
        ...
        <div className="px-4 py-2.5 border-b border-slate-800/50 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <Badge
              variant="secondary"
              className="h-5 min-w-[20px] px-1.5 text-[10px] font-bold bg-blue-900/40 text-blue-300"
            >
              {pendingCount}
            </Badge>
          </div>
        ...
        <div className="divide-y divide-slate-800/50">
          {items.map((item) => {
            ...
            return (
              <div
                key={item.id}
                className={`px-4 py-3 border-l-[3px] ${config.borderColor} hover:bg-slate-800/20 transition-all duration-200`}
              >
                <div
