    ...
    return (
      <Card className="border border-slate-800 shadow-none bg-slate-900 border-slate-800">
        <CardContent className="py-6 px-5 flex items-center gap-4">
          ...
          <a href="/dashboard/settings/budget" className="ml-auto shrink-0 text-xs text-blue-400 hover:underline">
            예산 등록 →
          </a>
        </CardContent>
      </Card>
    ...
    <Card className={`border border-slate-800 shadow-none overflow-hidden ${hasWarning ? "bg-red-950/20" : "bg-slate-900"}`}>
      <CardContent className="p-0">
          ...
          <div className="flex-1 p-3 sm:p-5 space-y-2 sm:space-y-3">
            ...
            <div className="flex justify-between items-center gap-2">
              ...
              {effectiveBudgets.length > 1 ? (
                ...
                >
                  <SelectTrigger className="h-8 w-[180px] text-xs border-slate-700">
                ...
                <div className="px-2 py-1.5 rounded-full bg-slate-800/60 border border-slate-700/80 text-[11px] text-slate-300">
            ...
            <div className="space-y-1 pt-1">
              <div className="flex items-center justify-between text-[11px] text-slate-400">
                ...
                <span className={`font-semibold ${usageRate >= 80 ? "text-red-400" : "text-slate-100"}`}>
                  {usageRate}%
                </span>
              </div>
              <Progress
                value={usageRate}
                className={`h-2.5 ${usageRate >= 80 ? "[&>div]:bg-red-500 bg-red-950/40" : "bg-slate-800/60"}`}
              />
        ...
        {selectedBudget.hasWarning && selectedBudget.warningMessage && (
          <div className="mx-3 mb-3 sm:mx-4 sm:mb-4 rounded-lg bg-amber-950/30 border border-amber-800/40 px-3 py-1.5 sm:px-4 sm:py-2 flex items-start gap-2">
            <AlertTriangle className="h-3.5 w-3.5 sm:h-4 sm:w-4 text-amber-400 shrink-0 mt-0.5" />
        ...
        {!selectedBudget.hasWarning && (
          <div className="mx-3 sm:mx-4 rounded-lg bg-slate-800/50 border border-slate-700 px-3 py-1.5 sm:px-4 sm:py-2 flex items-start gap-2">
        ...
        <div className="mx-3 mb-3 mt-2 sm:mx-4 sm:mb-4 sm:mt-3 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2">
          ...
          <div className={otherWarningCount > 0 ? "sm:ml-auto" : "w-full flex justify-end"}>
            <Button
              ...
              className="border-emerald-800 text-emerald-400 hover:bg-emerald-950/50 hover:border-emerald-700 text-xs h-8 px-3"
            >
              {isDownloading ? (
                <>
