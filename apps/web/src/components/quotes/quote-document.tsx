...
export function QuoteDocument({ quote, companyInfo }: QuoteDocumentProps) {
  ...
  return (
    <div className="bg-slate-900 print:bg-slate-900" id="quote-document">
      {/* A4 용지 스타일 */}
      <div className="max-w-[210mm] mx-auto bg-slate-900 border border-slate-700 shadow-none print:shadow-none print:border-0">
        <div className="p-8 md:p-12 print:p-12 space-y-8">
          {/* 헤더: 견적번호, 날짜, 공급자 정보 */}
          <div className="border-b-2 border-gray-800 pb-6">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-6">
              <div>
                <h1 className="text-3xl font-bold text-slate-100 mb-2">견 적 서</h1>
              ...
              <div className="text-right">
                <div className="text-lg font-bold text-slate-100 mb-2">{company.name}</div>
              ...
              <table className="w-full border-collapse">
                <thead>
                  <tr className="bg-slate-800 border-b-2 border-slate-700">
                    <th className="px-4 py-3 text-left text-sm font-bold text-slate-100 border-r border-slate-700 w-12">
                      No.
                    </th>
                    <th className="px-4 py-3 text-left text-sm font-bold text-slate-100 border-r border-slate-700">
                      품명
                    </th>
                    <th className="px-4 py-3 text-left text-sm font-bold text-slate-100 border-r border-slate-700 w-32">
                      규격
                    </th>
                    <th className="px-4 py-3 text-center text-sm font-bold text-slate-100 border-r border-slate-700 w-20">
                      수량
                    </th>
                    <th className="px-4 py-3 text-right text-sm font-bold text-slate-100 border-r border-slate-700 w-28">
                      단가
                    </th>
                    <th className="px-4 py-3 text-right text-sm font-bold text-slate-100 border-r border-slate-700 w-32">
                      공급가액
                    </th>
                    ...
                    items.map((item, index) => {
                      ...
                      return (
                        <tr key={item.id} className="border-b border-slate-800 hover:bg-slate-900">
                          <td className="px-4 py-3 text-sm text-slate-100 border-r border-slate-800">
                            {lineNumber}
                          </td>
                          <td className="px-4 py-3 text-sm text-slate-100 border-r border-slate-800">
                          ...
                          <td className="px-4 py-3 text-sm text-slate-400 border-r border-slate-800">
                            {item.product?.spec || "-"}
                          </td>
                          <td className="px-4 py-3 text-sm text-slate-100 text-center border-r border-slate-800">
                            {item.quantity}
                          </td>
                          <td className="px-4 py-3 text-sm text-slate-100 text-right border-r border-slate-800">
                            {unitPrice > 0 ? `₩${unitPrice.toLocaleString()}` : "-"}
                          </td>
                          <td className="px-4 py-3 text-sm text-slate-100 text-right border-r border-slate-800 font-medium">
          ...
          <div className="flex justify-end">
            <div className="w-full md:w-80 space-y-2">
              ...
              <div className="flex justify-between items-center py-3 border-t-2 border-gray-800 mt-2">
                <span className="text-base font-bold text-slate-100">총 견적 금액</span>
          ...
          <div className="border-t-2 border-gray-800 pt-6 space-y-6">
            ...
            <div className="text-center py-8 border-t border-slate-700">
              ...
              <div className="flex justify-end mt-12">
                <div className="text-right">
                  <div className="text-lg font-bold text-slate-100 mb-2">{company.name}</div>
                  <div className="text-sm text-slate-400 mb-8">대표이사: 홍길동</div>
                  <div className="border-2 border-gray-400 w-32 h-32 mx-auto flex items-center justify-center text-xs text-slate-400">
                    인
                  </div>
                </div>
              </div>
