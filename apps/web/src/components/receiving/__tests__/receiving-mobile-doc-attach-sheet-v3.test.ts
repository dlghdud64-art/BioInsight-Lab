...
describe("§receiving-doc-attach-canonical — 모바일 시트(승격 보존)", () => {
  it("same-canvas 바텀 시트 폼팩터 유지 + 모바일 전용", () => {
  ...
  it("로컬 복제 truth 없음 — 데모 store dispatch 주입 제거", () => {
    expect(read(SHEET)).not.toMatch(/attachReceivingDocument/);
    expect(read(PAGE)).not.toMatch(/onAttach=\{attachReceivingDocument\}/);
  });

  it("터치 타깃 44px 유지(모바일 접근성)", () => {
    expect(read(SHEET)).toMatch(/min-h-\[44px\]/);
  });
});
