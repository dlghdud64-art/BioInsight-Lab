...
export function ProductCard({
  ...
  return (
    <div className="bg-pn rounded-lg shadow-sm hover:shadow-md border border-bd hover:-translate-y-0.5 transition-all duration-300 overflow-hidden group">
      {/* 수직 스택 레이아웃 */}
      <div className="p-4 space-y-3">
        {/* 썸네일 + 제품명 */}
        <div className="flex items-start gap-3">
          <div className="w-16 h-16 shrink-0 rounded-md border border-bd bg-el overflow-hidden flex items-center justify-center">
            {!showFallback ? (
              <img
                src={imageSrc}
                alt={product.name}
        ...
        <div className="flex flex-wrap items-center gap-1.5">
          {keySpecs.length > 0 && keySpecs.map((spec, idx) => (
            <Badge
              key={idx}
              variant="outline"
              className="bg-el text-slate-400 text-xs px-2 py-0.5 rounded border-0 font-normal"
            >
              {spec.value}
            </Badge>
          ))}
        ...
        {/* 가격 & 액션 */}
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3 md:gap-2 pt-2 border-t border-bd">
          {/* 가격 */}
