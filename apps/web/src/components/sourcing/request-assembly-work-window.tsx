        ...
        <div className="shrink-0 border-b border-slate-600/30 bg-[#22252b]">
          ...
          <div className="px-6 pb-3">
            <div className="flex items-center gap-2 mb-2">
              ...
              {incompleteLines.length > 0 && (
                <span className="text-xs text-yellow-400 font-medium ml-1">불완전 {incompleteLines.length}</span>
              )}
            </div>
                    ...
                    className={`shrink-0 flex items-center gap-2 h-9 px-3 rounded-lg border text-xs font-medium transition-all ${
                      isSelected
                        ...
                        : line.isComplete
                          ...
                          : "border-yellow-500/20 bg-yellow-600/[0.04] text-yellow-200 hover:border-yellow-500/30"
                    }`}
                  >
                ...
                <section>
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <span className={`text-xs px-2 py-0.5 rounded-md border ${
                      line.isComplete
                        ...
                        : "border-yellow-500/25 bg-yellow-600/10 text-yellow-300"
                    }`}>
                  ...
                  <div className={`px-5 py-4 rounded-xl border transition-all ${
                    line.isComplete
                      ...
                      : "border-yellow-500/25 bg-yellow-600/[0.04]"
                  }`}>
                    <div className="mb-4">
                            ...
                            className={`flex-1 h-9 rounded-md text-sm font-medium border transition-all ${
                              isActive
                                ? u === "critical" ? "bg-red-600/15 border-red-500/30 text-red-300"
                                  : u === "urgent" ? "bg-yellow-600/15 border-yellow-500/30 text-yellow-300"
              ...
              <div>
                ...
                <div className="space-y-1.5">
                  ...
                  {validation.warnings.map((w, i) => (
                    <div key={`warn-${i}`} className="flex items-start gap-2 px-3 py-2 rounded-lg bg-yellow-600/[0.06] border border-yellow-500/15">
                      <AlertTriangle className="h-3.5 w-3.5 text-yellow-400 shrink-0 mt-0.5" />
                      <span className="text-xs text-yellow-200">{w}</span>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div className="space-y-2">
                ...
                <div className="flex items-center justify-between px-3 py-2 rounded-lg border border-slate-600/20 bg-[#2a2e35]">
                  ...
                  <span className="text-xs font-semibold text-slate-200">
                    ...
                    <span className={incompleteLines.length > 0 ? "text-yellow-300" : "text-slate-400"}>{incompleteLines.length}</span>
                  </span>
                </div>
                <div className="flex items-center justify-between px-3 py-2 rounded-lg border border-slate-600/20 bg-[#2a2e35]">
                  <span className="text-xs text-slate-400">긴급도</span>
                  <span className={`text-xs font-semibold ${
                    urgency === "critical" ? "text-red-300" : urgency === "urgent" ? "text-yellow-300" : "text-blue-300"
                  }`}>
        ...
        <div className="shrink-0 px-6 py-4 border-t border-slate-600/30 bg-[#1e2126]" style={{ borderBottomLeftRadius: "16px", borderBottomRightRadius: "16px" }}>
          ...
          <div className="flex items-center gap-4 text-xs mb-3">
            ...
            {incompleteLines.length > 0 && (
              <>
                <span className="text-slate-600">·</span>
                <span className="text-yellow-400 font-medium">불완전 {incompleteLines.length}</span>
              </>
            )}
