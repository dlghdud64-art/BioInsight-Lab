...
- ❌ **연결되지 않은 소스의 카운트·필터를 0 으로 보여주기 금지 — 표시 자체를 하지 않는다** (아래 조항)

---

## 🛑 연결되지 않은 소스는 0 을 보여주지 않는다 (2026-09-24 · 호영님 판정)

**0 은 「재봤는데 없다」 로 읽힌다. 연결이 없으면 재본 적이 없는 것이고, 그 둘은 다른 사실이다.**

```
연결됨 · 오늘 0건    → 0 을 보여준다. 참이다.
연결 안 됨           → 0 을 보여주지 않는다. **컨트롤·카운트 자체를 만들지 않는다.**
```

실측 2026-09-24(§inbox-unconnected-filters · 운영 작업함): 모듈 알약이
「전체 · 견적 · 발주 · 입고 · 재고위험」 5개였는데 이 화면에 항목을 넣는 생산자는
`real-quote-inbox.ts` 하나뿐이고 `sourceModule` 은 `'quote'` 고정이었다. 셋은 **항상 0**이다.

🛑 **필터를 지우는 것이 「없는 척」 이 되면 안 된다.** 무엇이 아직 연결되지 않았는지는
   **문구로 계속 밝힌다**(그 화면의 「발주 · 입고 · 재고 위험은 아직 이 작업함에 연결되지 않았습니다」).
   조항이 요구하는 것은 **침묵이 아니라 거짓 0 의 제거**다.

⚠️ **옵션이 1개만 남으면 그 컨트롤도 지운다.** 「전체 / 견적」 두 알약은 같은 집합을 내므로
   눌러도 아무 일이 없다 — dead button 금지 조항에 그대로 걸린다.

🔑 지우기만 하면 다음 사람이 배선 없이 목록에 다시 넣는다. **연결 목록을 상수로 두고,
   그 집합이 생산자가 실제로 대입하는 값과 같은지 sentinel 이 함께 본다.**
   선언만 늘고 배선이 없으면 RED 다(§인프라를 만들면 같은 커밋에서 배선한다 와 같은 뿌리).

⚠️ 지워진 필터 키가 **옛 북마크·링크의 URL** 에 남아 있다. 그대로 복원하면 0건 화면에
   **그 필터를 끌 알약이 없는** 막다른 길이 된다 → 목록에 없는 키는 전체로 떨어뜨린다.

📌 판정 대기(같은 날 실측 · 호영님 지시 범위 밖이라 손대지 않음):
   `owned_by_me`(모든 생산 지점이 `owner: undefined`) · `escalated` · `awaiting_approval` ·
   `awaiting_internal_review` 도 생산 분기가 없어 항상 0이다. 「차단」 KPI·상태 알약은
   `blockedReason` 대입이 **0곳**이라 같다. 이건 소스 축이 아니라 **상태 축**이라 조항 적용 여부가
   따로 판정돼야 한다.
...
- 0건 비활성 톤: `bg-white border-dashed border-slate-200` + text `text-gray-400`
  (2026-09-21 개정 · 호영님 판정. 0건은 "꺼짐/불가" 가 아니라 **"아직 안 채워진 자리"** 다 —
   회색 채움은 전자로 읽히고 점선은 후자로 읽힌다. 구 `bg-gray-50 border-gray-200` 는 은퇴.
   아이콘 박스(0건 `bg-slate-50`)는 **카드와 다른 요소**다 — 둘을 한 토큰 grep 으로 재지 말 것.
   실측: 센티널 7자리가 파일 전체 grep 으로 주석·아이콘 박스에 걸려 카드를 한 번도 재지 않았다.)
...
- 데이터 없는 KPI/위젯은 축소 + 비활성 톤(카드 = 흰 배경 점선 테두리 · §1 참조)
...
```
1회  §approval-gate-single-source  사유 유니온을 세미콜론 없이 핀 → 멤버를 **더 붙여도** 부분 일치로 통과
2회  §approval-gate-single-source  필터 함수의 **시그니처만** 핀 → 본문을 비워도 통과(전량이 새어 나감)
```

둘 다 프로브가 잡았다. 조항이 아니라 **프로브 목록**에 넣는 이유가 여기 있다 —
쓸 때는 매번 「이건 충분하다」 고 느끼고, 비워 보면 아닌 것이 드러난다.

지킬 것: 단언 하나를 쓸 때마다 그 짝으로 **「비워도 통과하나」** 프로브를 1개 만든다.
```
선언을 핀했으면   →  본문을 비운다        (function f(){ return X; } → function f(){ return; })
목록을 핀했으면   →  목록을 비운다·늘린다  (Set([...]) → Set([]) · 멤버 추가)
호출을 핀했으면   →  인자를 상수로 바꾼다  (f(realInput) → f(null))
```
...
🛑 **순서를 절차로 고정한다 — 「적용 확인 → 주입 → RED」.** 기억에 맡기면 다음이 난다.
   2026-09-12 하루에 **2회 더** 무효 프로브가 났고, 둘 다 적용 확인이 잡았다(치환은 조용히 실패한다).
```
1회  셸 cwd    레포 루트에서 상대 경로로 치환 → 파일 없음 → 아무것도 안 바뀌었는데 GREEN
     처방: 파일을 건드리는 명령은 `cd <절대경로> || exit 1` 과 `pwd` 로 시작한다
2회  EOL 불일치 앵커를 
 으로 잡았는데 대상이 CRLF(csrf-route-registry) → 치환 0 → GREEN
     처방: 앵커는 대상 파일의 EOL 을 읽어서 만든다  const eol = s.includes("
") ? "
" : "
"
```
확인 방법은 **결과에 드러나는 것**으로 한다 — `git diff --stat` · 치환 건수 출력 · `grep -c <주입 토큰>`.
"주입했다" 는 로그는 확인이 아니다(치환 실패해도 그 줄은 찍힌다).

🛑 **정규식·백슬래시가 들어가는 스크립트는 heredoc·`node -e` 로 만들지 않는다. 파일로 쓴다.**
   2026-09-12 하루에 **3회** 같은 형태로 깨졌다 — 셸·heredoc 을 거치며 백슬래시가 한 겹씩 사라진다.
```
1회  node -e 안의 정규식      SyntaxError 로 즉사(그나마 눈에 보임)
2회  heredoc 의 /BS/g        BS 한 겹 소실 → 잘못된 정규식이 조용히 다른 것을 매칭
3회  heredoc 의 toMatch(/.../) 앵커  BS 소실 → ANCHOR MISS · 치환 0
```
   매번 "치명적이지 않다" 로 넘어갔지만 2회차는 **프로브를 무효로 만들 뻔했다**(EOL 불일치 건과 같은 결말).
   → 스크립트는 편집 도구로 파일에 직접 쓰고, 백슬래시가 꼭 필요하면 `String.fromCharCode(92)` 나
     줄 단위 치환으로 우회한다. 앵커 EOL 은 대상 파일에서 읽는다.
...
### 🛑 비교식 하나로 갈림을 판정하지 않는다 — **분기 전체를 읽는다** (2026-09-20 신설 · 같은 형태 4회)

검출기가 무는 것은 명제의 **형태**이고, 값이 갈리는지는 **그 비교식에 도달하기까지
무엇이 걸러졌는가**가 정한다. 둘을 섞으면 없는 결함을 만들고 있는 결함을 놓친다.

실측 2026-09-20 (§reorder-need 인라인 복제 31곳 분류 중):

    scan/page.tsx:336           truthy 가드 → "정본과 갈린다" 로 분류
                                🛑 앞 분기가 qty <= 0 → "재고 없음" 이라 그 경우를 먼저 잡는다. 값 일치.
    use-inventory-ai-panel:158  같은 가드 → "정본 준수로 즉시 치환" 지시
                                🛑 다음 줄이 qty / safetyStock 이다. 치환하면 "안전 수준의 NaN%".
                                   가드가 0나누기를 겸하고 있었다.

같은 형태 4회: ① 「한 줄만 불일치」(ORGANIZATION 판정 · 시간축 누락) ② `:373` 을 동작 자리로
단언(독블록이었다) ③ 「유일한 라이브 호출자」(래퍼를 안 셌다) ④ 이번 건.
뿌리가 같다 — **조각 하나를 읽고 전체를 말했다.**

절차: 비교식을 찾으면 (1) 그 분기에 도달하는 조건 (2) 앞선 형제 분기 (3) 직후 사용처를
함께 읽는다. 셋 중 하나라도 안 읽었으면 "갈린다" 고 쓰지 않고 **"형태가 다르다" 까지만** 쓴다.

### 🛑 **시안은 조항 위에 있지 않다** (2026-09-12 신설)

시안(디자인 제안)과 조항이 충돌하면 **조항이 이기거나 호영님께 올린다.** 시안이 조용히 이기는 경로를 막는다.

🛑 **"구현이 시안과 일치하는가" 만 보는 검증은 조항 위반을 통과시킨다.**
```
실측 2026-09-12(§amber-token-ratchet)
  8/21  호영님이 관리자 역할 점 #b45309 를 거부하고 slate-900 으로 결정 (같은 파일 주석에 기록)
  9/08  §mobile-residual-5 1e 가 시안 토큰(퍼플·앰버)으로 되돌림
  9/08  smoke 4-2 에서 operator 가 "4색이 시안값과 일치" 만 확인하고 유지 판정 — 조항 대조 누락
  결과  amber 금지 조항(§11.302) 위반이 4일간 land · amber-token-ratchet 이 그동안 RED
        sentinel 까지 그 위반값을 핀하고 있었다(검사가 위반을 요구하는 형태)
```
지킬 것: 시안 대조는 **조항 대조 뒤에** 온다. 색·문구·구조가 조항(신호등 팔레트 · em dash · 금지 토큰)에
닿으면 그 조항을 먼저 찾는다. 충돌하면 구현 전에 상신한다 — 판정은 호영님 몫이고, 조용한 채택은 금지다.

⚠️ 반대 방향은 이미 적혀 있다 — 「조항이 기존 계약을 깨면 그 조항이 틀린 것이다」(§타이포 구분자).
   둘은 짝이다: 조항이 계약을 깨면 조항을 고치고, 시안이 조항을 깨면 시안을 고친다. **조용히 넘기지 않는다.**

### 🛑 **래칫류는 기준선 RED 원장에 올리지 않는다** (2026-09-12 신설)

래칫의 존재 이유가 **GREEN 을 유지하는 것**이다. 기준선 RED 더미에 들어간 래칫은 꺼진 래칫이다.

```
실측 2026-09-12  amber-token-ratchet 이 4일간 RED (67 > baseline 66)
                 그 안에 §11.302 amber 금지 위반 1건이 숨어 있었다
                 좁힌 게이트 축(ops+security+regression) 밖이라 매 커밋 보고에서 안 보였다
래칫 전수 N = 5  (amber-token-ratchet · enforcement-handle-close · phantom-model-call ·
                 audit-org-unresolved-ratchet · org-scope-callers-inventory)
```

지킬 것
- 원장(`apps/web/docs/red-ledger.json`)에는 **sentinel 만** 담는다. 래칫이 RED 면 등재가 아니라 **즉시 처리**다.
- 래칫 판별은 파일명이 아니라 **성격**으로 한다: 헤더에서 스스로 래칫이라 선언 · 상한 상수(BASELINE·CEILING·
  LEGACY·RATCHET·CAP) · 상한 판정(toBeLessThanOrEqual · 초과 시 throw). 본문에서 남의 래칫을 인용한 것은
  래칫이 아니다(실측 오탐 1건).
- **GREEN 인 래칫까지 세어 분모 N 을 남긴다.** "N 개 중 M 개가 꺼져 있었다" 가 이 발견의 정확한 형태이고,
  N 을 모르면 다음에 또 꺼졌을 때 못 본다. `npm run red-ledger` 가 매번 N 과 M 을 같이 찍는다.

- 🛑 **기준선을 갱신할 때 새로 들어가는 RED 는 그 자리에서 귀속한다** (2026-09-25 신설 · 호영님 판정).
  **기준선에 조용히 들어간 RED 는 영원히 「신규」가 되지 않는다.**
  게이트 차분(`cmp-gate base.json new.json`)은 base 에 이미 RED 인 것을 신규로 세지 않는다.
  내 커밋이 만든 RED 가 다음 회차의 base 가 되는 순간 비교 대상에서 빠지고, 그 뒤로는 몇 번을 재도 조용하다.

  실측 2026-09-24: `meta/sentinel-identifier-boundary` ②(레거시 목록 유령 경로 0)가 §po-ui-removed
  (`33fc705f`)에서 꺼졌는데 — 테스트 5개를 은퇴시키며 목록에 유령이 생겼다 — 그 뒤 gate102·104·105
  **세 번의 차분이 전부 「신규 RED 0」** 을 냈다. 병렬 세션도 같은 차분 방식이라 양쪽 다 못 봤고,
  동료 세션이 `red-ledger` 를 돌려서야 드러났다.
  🔑 위 「래칫류는 기준선 RED 원장에 올리지 않는다」 가 막으려던 형태를 **측정 방식이 무력화**하고 있었다.

  지킬 것
  · 커밋 전 게이트에 **`npm run red-ledger` 를 항상 포함**한다. 래칫 N/M 판정은 그쪽이 정본이고 차분은 보조다.
  · 기준선을 새로 찍을 때 **그 회차에 늘어난 RED 를 한 건씩 귀속**한다 — 내 커밋인지, 남의 커밋인지, 원래 있던 것인지.
    귀속하지 않은 RED 를 base 로 승격시키지 않는다.
  · 파일을 은퇴·삭제했으면 **그 경로를 인용하는 레지스트리·레거시 목록**도 같은 커밋에서 지운다. 유령 경로가 래칫을 끈다.

  ⚠️ 이건 §「검사 결과 초기값은 합격이 아니다」 가 **게이트 자체에 적용된 두 번째 사례**다(호영님).
     그럴듯한 값(「신규 0」)을 성공 신호로 읽지 않는다 — 그 0이 **무엇을 안 보고 나온 0인지**를 먼저 묻는다.

### 🛑 **개수는 명제가 아니다** — 집합·관계를 핀한다 (2026-09-12 신설)

개수는 집합의 **그림자**다. 그림자를 핀하면 두 방향으로 틀린다.

```
① 상쇄로 우연히 맞는다   하나 빠지고 하나 늘면 총계는 그대로다 → 구성이 바뀌었는데 GREEN
② 정당한 변경마다 막힌다  라우트 하나 지울 때마다 숫자를 손으로 고쳐야 한다
```

실측 2026-09-12(§baseline-red-ledger 파일럿 · csrf-batch10):
```
exempt 수 9      기대 9 · 실측 9  → GREEN 인데 구성이 달랐다
                 send-link 제거(-1) · pricing-assistant 추가(+1) 가 상쇄
highRisk 17      실측 15 → 상시 RED. 숫자만 고치면 다음 삭제에서 또 선다
```

🔑 처방은 값 갱신이 아니라 **핀의 층위 교체**다.
```
❌ expect(stats.exempt).toBe(9)
❌ expect(stats.exemptReasons).toEqual({ ... 총계 맵 ... })
✅ expect(정렬된 집합 리터럴).toEqual([
     '/api/pricing-assistant :: public_stateless_llm',   // 9053f580 (2026-06-28) 추가
     ...
     // 제거됨: '/api/templates/[id]' 22fa9ddd
   ])
```
- 무엇이 늘고 줄었는지가 **diff 에서 읽힌다**. 개수만 보면 '왜 17 이 15 가 됐나' 를 매번 다시 판다.
- 목록을 고칠 때는 **그 변경을 만든 커밋**을 같은 줄에 적는다. 근거 없는 갱신은 §fixture 필드 지위 분리의
  "검사가 못 찾으니 anchor 를 갱신한다" 와 같은 위반형이다.
- 적용 범위: 레지스트리·enum·라우트 목록·파일 목록·허용 목록 등 **열거 가능한 모든 것**.
  열거가 불가능할 때만(수천 건 레거시) 개수 래칫을 쓰고, 그때는 래칫임을 명시한다.

⚠️ 이것은 아래 「상수를 참조하는 단언」의 일반형이다 — 그쪽은 심볼이 사라지면 비교가 무의미해지는 경우이고,
   이쪽은 **총계가 구성을 가리는** 경우다. 처방은 같다: 값 집합을 리터럴로 고정한다.
...
실측 2026-09-20 — 같은 형태가 대시보드 입고 칩에 있었다. 이쪽은 계산식이 아니라 **테이블**이 갈렸다:

    칩    /api/dashboard/summary   inventoryRestock.count({ userId })      상태 무관 전량
    화면  /dashboard/receiving     receiving-drafts?status=AWAITING_REPLY,PENDING_REVIEW,APPROVED

두 개의 다른 엔티티다. `InventoryRestock` 이 N건이어도 `ReceivingDraft` 가 0이면
칩은 "이상 없음" 이고 여는 화면은 비어 있다.

🛑 **호영님 판정 2026-09-20 — 입고 정본은 `ReceivingDraft` 다.**
receiving 페이지는 이미 자기 주석에 canonical 로 선언하고 있었고, 대시보드만 다른 테이블을 셌다.
→ 고칠 곳은 `api/dashboard/summary/route.ts` 의 `receive` 모듈 하나.
  구현은 대시보드 표면 소유 세션이 맡는다(§병렬 세션 2).
...
### 🛑 가드를 세우기 전에 **그 아래 저장층이 무엇인지** 확인한다 (2026-09-21 신설 · 실측 3건)

fail-closed 를 닫아도 그 판정을 떠받치는 저장층이 휘발이면, 가드는 **빈 상자 위에 선다.**
"판별 불가를 통과로 세지 않는다" 를 지켜도 대부분의 경우 검사할 것이 없어서 지나간다.

실측 2026-09-20~21:

    ① ai-pipeline 복구·감사 체인
       bootstrap.ts:37  PERSISTENCE_PROVIDER 기본 "MEMORY"
                 :52-54 PRISMA 는 overrides.prismaClient 필수 · 그렇게 부르는 live 호출자 0
       Vercel prod env 에 PERSISTENCE_PROVIDER 없음(24개 전량 조회) → MEMORY 확정
       서버리스라 인스턴스가 바뀌면 그 순간 없다. RECOVERY_AUDIT_HOPS 의 "복구 실행 프로세스에서
       재검증" 이라는 해소 조건은 사실상 같은 요청 안에서만 성립한다.

    ② CSRF 위반 기록
       event-provenance-engine.ts:130 모듈 내 배열 · :238 push · :241 2000건 링 버퍼
       DB 참조 0 · 외부 싱크 0 · flush 0
       report_only 는 "기록만" 이 아니라 사실상 아무것도 하지 않는다.
       csrf-status 의 telemetry 도 같은 배열을 본다 → 우회 관측 경로 없음.

    ③ 그 결과 위험도는 설정값과 무관하게 말할 수 있다
       report_only 면 차단도 기록도 없고, enforce 여도 위반 기록은 휘발이다.
       어느 쪽이든 위반이 있었는지 사후에 알 수 없다.

절차: fail-closed 를 닫기 전에 (1) 그 판정이 읽는 저장층의 실제 구현 (2) prod 에서 그 구현이
무엇으로 해석되는지 (3) 판정 결과가 남는 곳이 요청·인스턴스 경계를 넘는지 — 셋을 먼저 잰다.
넘지 못하면 가드를 세우지 말라는 뜻이 아니라, **가드가 무엇 위에 섰는지 함께 기록**하라는 뜻이다.

⚠️ 그리고 fail-closed 는 **"던진다" 가 아니다.** 2026-09-21 CSRF 모드 건에서 내가 처음 낸 권고는
   "인식 못 하는 값이면 기동 시 에러" 였는데, 그 함수는 미들웨어가 **요청마다** 부른다 —
   던지면 오타 하나가 프로덕션 전체를 500 으로 만든다. **가드가 제품을 죽이면 그게 더 큰 결함이다.**
   택한 형태: 모드는 안전한 기본값에 머물되 인식 실패 사실을 잃지 않는다
   (console.error = 비휘발 싱크 + 상태 엔드포인트가 recognized 를 노출).
   권고를 낼 때 그 권고가 실행되면 무엇이 죽는지까지 세는 것이 판단의 일부다.

⚠️ 반대 방향도 같다 — **가드를 고치는 배포가 prod 동작을 조용히 켜서도 안 된다.**
   같은 건의 첫 구현은 env 값을 trim 한 뒤 모드를 골랐다. 이전 구현은 완전 일치만 인정했으므로,
   `" soft_enforce\n"` 처럼 공백이 붙은 prod 값이 있었다면 **이 배포 하나로 report_only → soft_enforce
   로 켜진다.** prod 값은 sensitive 라 읽을 수 없어 확인도 불가였다(운영 세션이 커밋 전 게이트에서 발견).
   → trim 하지 않는다. 모드는 **완전 일치**로만 고르고, 공백 붙은 허용값·빈 값·공백만 있는 값은
     전부 이전과 같이 report_only + recognized=false(키는 있는데 먹지 않는 상태 · 미설정과 구분).
   모드 전환은 **값을 고치는 사람이 의도해서** 일어나야 한다. 해석 규칙 변경이 대신 일으키면 안 된다.
...
### 이 게이트가 열려야 되살아나는 것 — 의존 항목 (2026-09-25 · 호영님 판정)

게이트가 닫혀 있는 동안 **제품에서 내린** 문구·표면이다. 여기 적어 두지 않으면 게이트를 여는 사람이
이것들을 되살릴 줄 모른다. 게이트를 열 때 이 목록을 함께 처리한다.

```
① 퍼블릭 결재 문구                §approval-gate-single-source · 랜딩 파이프라인 4단계 · /intro 조직 운영 2곳
   복원 조건  BUSINESS SKU 결제 → in_app_approval 도달이 **prod 에서 확인**된 뒤
   왜 내렸나  결재는 FREE 에서만 막힌 게 아니라 **모든 사용자에게** 막혀 있다(정책 none · 게이트 닫힘 ·
              webhook 이 TEAM 만 씀 · ADMIN 0명이면 또 400). 방문자는 워크스페이스가 없어
              판정 함수(canRequestApproval)를 부를 수 없으므로 로그인 뒤 표면과 달리 **가릴 수가 없다.**
   🛑 요금제 라벨로 가리지 않는다 — 「어떤 요금제가 결재를 켠다」 는 전제가 오늘 거짓이라
      그 라벨 자체가 또 하나의 약속이 된다(호영님).

② 전환 자리 요금제 안내           §order-entry-removed
   복원 조건  ①과 같다. 「업그레이드하면 결재를 쓸 수 있다」 는 문장은 그 경로가 실재할 때만 참이다.

③ 결재 판정의 **금액 구간 불일치**   §approval-gate-single-source 자기 한계 1
   오늘 상태  판정 함수가 전원 거짓이라 결재 CTA 가 어디에도 뜨지 않는다 → **도달 불가**다.
              결재가 켜지는 날 바로 드러난다.
   무엇이 갈리나  라우트는 금액 tier 로 결재자 후보를 좁히고 high tier(기본 1,000만원↑)에서
              self_admin 을 **막는다**(escalation). 화면 축은 「결재자가 한 명이라도 있는가」 까지만 본다
              → 워크스페이스 ADMIN 이 요청자 본인뿐 + 금액 high tier = 화면은 보여주고 라우트는 400.
   닫는 방법  **같은 함수의 두 호출 방식**이다. 따로 판정하지 않는다(호영님).
              · 금액을 아는 자리(견적 상세 CTA) → 금액 인자를 넣은 전체 판정을 부른다
              · 금액을 모르는 자리(KPI 라벨 · 퍼널)   → 지금처럼 결재자 존재까지만 본다
```

⚠️ 로그인 뒤 표면은 **내리지 않았다** — `lib/approval/approval-capability.ts` 판정 뒤에 뒀다.
   결재가 실제로 켜지는 날 저절로 보인다. 되살릴 일이 없으므로 이 목록에 없다.
...
- 🛑 **동승은 커밋 단위만이 아니라 파일 단위로도 일어난다** (2026-09-24 신설 · 호영님 판정).
  `git add <경로>` 는 훅을 고르지 않는다 — 내가 A 부분을 고친 파일에 상대 세션이 B 부분을
  미커밋으로 고쳐 뒀으면 **B 도 내 커밋으로 나간다.**
  실측: `33fc705f`(§po-ui-removed)가 `budget/[id]/page.tsx` 의 상대 세션 `fetch → csrfFetch`
  수정을 무기록으로 실어 냈다(상대 세션이 `326ea393` 에 사후 기록해 복구).
  게이트는 워킹트리를 재므로 **측정은 됐다** — 무사한 것과 지켜진 것은 다르다. 커밋 본문이 실린
  것보다 좁으면 나중에 그 수정을 찾는 사람이 엉뚱한 커밋을 파고 revert 범위도 틀어진다.
  → 스테이징 **전에** `git diff -- <내 경로>` 로 내가 기억하는 변경만 있는지 본다(`--stat`
  증감폭만으로는 못 잡는다). 섞여 있으면 **커밋 본문에 적는다** — 훅 단위로 가르려 하지 말 것
  (`git add -p` 는 대화형이라 못 쓰고, 파일을 되돌리면 상대 작업이 사라진다 · §병렬 세션 2).

- 🛑 **파일 삭제는 HEAD 축과 워킹트리 축을 둘 다 봐야 게이트에 잡힌다** (2026-09-24 신설).
  `git ls-tree` 로 재는 단언은 **HEAD** 를 읽는다. `git rm` 으로 스테이징만 된 상태에서는 HEAD 에
  파일이 그대로 있어 **커밋 전 게이트가 GREEN** 이고, 커밋이 HEAD 를 옮긴 **뒤에야** RED 가 된다.
  실측: `33fc705f` 가 그 형태로 RED 1건을 land 시켰다(§11.162 「발주 landing 보존」).
  → 삭제 명제는 `existsSync`(워킹트리)와 `ls-tree`(HEAD) 를 **둘 다** 단언한다.
  같은 뿌리의 일반형은 §"X를 쓰는가" 에 grep 으로 답하지 않는다 — **질문에 맞는 축으로 잰다.**
...
- 🛑 **대기 사유는 승인만이 아니다 — 게이트 대기 커밋도 같다** (2026-09-20 실측 추가).
  미검증 커밋이 main 에 서 있는 동안 병렬 세션의 push 가 그것을 실어 나른다.
  실제: `8f386e1f` · `77694f58` (2026-09-19) 와 `93e25ef5` (2026-09-20) — 이 세션이
  게이트 전에 main 에 커밋했고, 다른 세션이 자기 커밋(`e52846a3`)을 push 할 때 함께
  올라갔다. 사후 게이트가 GREEN 이라 결과는 무사했지만, **무사한 것과 지켜진 것은 다르다.**
  main 은 Vercel 자동 배포 대상이므로 RED 였다면 그대로 프로덕션에 나갔다.

- ⚠️ 그런데 **공유 워킹 카피에서는 위의 `wip/<slug>` 가 실행 불가하다.** 브랜치 전환은
  같은 카피를 쓰는 다른 세션의 작업 트리와 기준선을 파괴한다(§병렬 세션 1·2 와 충돌).
  따라서 공유 카피를 쓰는 세션에서 실효 방어는 둘뿐이다:
  ① 커밋과 게이트 사이 간격을 최소로 — 커밋 즉시 운영 세션에 측정을 요청한다.
  ② **push 하는 세션이 동승분 보고 의무를 진다**(바로 위 항목). 위 사례에서 push 한
  세션은 동승분을 표기하지 않았다 — 의무는 커밋한 쪽이 아니라 미는 쪽에 붙는다.
