...
- 긴급/주의 (낮은 재고, 만료 임박, 검토 필요): muted amber `#b45821` 톤 — 배지 `bg-amber-weak text-amber`, 큰 카드 `bg-amber-weak border-amber-line text-amber` (호영님 2026-06-30 확정)
- 정상: `bg-emerald-100 text-emerald-700` (배지)
- 정보 (실행 가능 CTA): `bg-emerald-600 text-white` (primary), `bg-blue-600 text-white` (분석/검토)
- ✅ 주의색 = muted amber `#b45821`(채도 낮춤, 눈 편함). ❌ 쨍한 yellow(`bg-yellow-*`)·orange 금지 — 밝은 노랑 눈피로 피드백으로 brownish amber 로 이전(호영님 2026-06-30, 예전 예산 이상탐지 결정 연장). 토큰: `--amber #b45821 / --amber-weak #fdf3ec / --amber-line #f3d4bf`. 의미=주의·만료임박(위험=rose, 정상=emerald 와 구분)
