...
- 🛑 **재고 상태 색 판정표 (2026-09-26 · 호영님 판정 · §inventory-state-tone)** — 구 「낮은 재고 = yellow」 를 이 표로 정정.
  ...
  | emerald | 정상 | 정상 |
  | 중립(slate) | 상태 아님 | **라벨 재출력**(해야 할 작업) · 판별 불가 |

  · 3줄 추가 판정(2026-09-27): 보관 위치 미지정은 「당장 조치할 일은 아니지만 **비어 있는 정보**라서 주시」 ·
    입고 대기는 emerald 로 두면 우선 처리 큐에서 「정상」 으로 읽혀 **뜻이 반대** · 라벨 재출력은 상태가 아니다.
  · 🛑 **건수가 0이면 중립이다** (라이브 실측 2026-09-26: 「만료 임박 0」 칩이 yellow 였다).
    0건인데 주의색을 띠면 「볼 게 있다」 고 거짓말한다 → 정본 `inventoryToneClassForCount(state, count)`.
    §11.283a 가 흰 KPI 카드에서 이미 잠근 규칙이고, 표면마다 다시 쓰이고 있던 것을 정본으로 모았다.
  · 🛑 **조치 버튼은 상태색을 쓰지 않는다** — 재발주 · 교체 주문 · 출고 · 입고 전부 중립이다.
    버튼이 「주시」 색을 입으면 같은 행의 red 배지와 어긋나고, blue·emerald 는 §9 의 다른 축이다.
    보존되는 것은 「솔리드 채움 hover + 흰 글자」(대비)이고 중립 톤으로 유지한다.
...
### 2-b. 병합·rebase 전에 **생성 파일은 HEAD 로 되돌린다** (2026-09-27 신설 · 호영님 판정)

```
apps/web/src/generated/**   병합·rebase 전에 미커밋 변경을 git restore 로 되돌린다
                            red-ledger 가 다시 만들 수 있는 파일이라 되돌려도 유실이 아니다
                            병합 후 red-ledger 로 재생성하고, **결과가 HEAD 와 다를 때만** 커밋에 싣는다
```

왜 — 2026-09-27 실측: `migration-manifest.json` 의 미커밋 변경이 병합을 막았고
(`Aborting · Index was not unstashed · Merge with strategy ort failed`), 상대 세션이 같은 파일을
커밋해 둔 상태였다. 그 경고를 보고 autostash 사고로 오해했는데 실제로는 stash 가 만들어지지 않았고
(전 항목 3주 전) HEAD 도 움직이지 않았다 — **확인하고 나서야 유실이 아님을 알 수 있었다.**
생성 파일은 두 세션이 동시에 재생성하므로 이 형태는 계속 난다. 미리 되돌리면 병합이 막히지 않는다.

⚠️ 경고 문구를 본 즉시 확인할 것: `git stash list --format="%gd %cr"`(새 항목이 있나) ·
   `git log --oneline -2`(HEAD 가 옮겨졌나) · `git status`(사라진 파일이 무엇인가). 셋을 보기 전에
   「유실됐다」 고도 「무사하다」 고도 말하지 않는다.

🔑 `package-lock.json` 은 다르다 — 그쪽은 **블롭이 CRLF** 라서 `*.json text eol=lf` 와 영구히 어긋나
   상시 더러웠다(`git restore` 로도 안 가라앉는다). `git add --renormalize` 로 블롭을 LF 로 바꿔
   2026-09-27 에 끝냈다(내용 차이 0 실측). 다시 더러워지면 그건 **누군가의 실제 변경**이다.
