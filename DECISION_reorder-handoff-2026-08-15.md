...
### §scope-from-verdict — 확인 범위는 **판정 기준에서 역산**한다

> 범위 < 판정 이면 조치자가 **범위를 넓히거나 판정을 좁혀야** 하는데,
> **둘 다 조치자 권한 밖**이다 → **반드시 멈춘다.**

실례 (2026-08-16 §2 아이콘 분리):
```
확인 범위   1c 2슬롯
판정 기준   label 값에 U+1F50D / U+2709 **전량 0**
실측        1b.cta.secondary 에 🔍 잔존 → 판정 미충족
```
조치자가 임의로 1b 를 고쳤으면 **범위를 스스로 넓힌 것**이고,
판정을 1c 한정으로 읽었으면 **판정을 스스로 좁힌 것**이다. 멈춘 게 맞다.
